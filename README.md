...
|---|---|---|
| **"I said use long-term solutions, not patches" — 30 turns later the Agent patches again** | Turn 1: you say "use the cleanest solution," Agent answers "got it." Turn 50: "let me patch this quickly." Your preference got diluted by new content. | Inject baseline at session start, compact anchor per turn with drift markers — your directions get first attention every turn |
...
| **Agent sees a reminder → reacts defensively or rationalizes around it** | LLMs trained to please users — when faced with a violation reminder, the first reaction is to self-justify or find the shortest patch around it, not to genuinely correct | Rules phrased as "the user hopes…" agreement, not "you must X" verdict — Agent's first reaction shifts from "let me defend" to "let me realign" |
...
| **"I want to add a rule but writing yaml is heavy / my phrasing doesn't change Agent behavior"** | You know what behavior you want, but writing the rule is its own chore — wrong `violation_keywords` format triggers false positives, wrong tone makes the Agent defensive | Type `/pinrule <natural language>`; the skill refines, validates, confirms, writes — ~30 seconds end-to-end |
