...
- 操作系统：**macOS / Linux**（Windows 兼容未实测，wrapper 用 Unix shebang，
  Windows 用户建议先 WSL 跑）
...
审批后 karma 在 codex CLI 才触发。Claude Code 这条线没 approval gate
任何用法都立即生效。
````
...
karma 默认装的 5/7 条 sticky 是「跨用户合理」的中性版本，**不一定是你的真正偏好**。
...
表示生效，但对「逐步确认型」用户是 noise。**按你真正偏好选**。
...
- **Stop hook 实时干预**：Agent 沉默式停下时让继续生成下一步（防死循环 safeguard）
- **累积强制 block**：同规则反复违反 → 要求 fix 根因或让用户介入；可在
...
- **确实有假阳**（误拦你的合法操作） — 表格 cell 引用术语 / `python -c` 内字符串字面 / commit message 描述违反字眼等场景都可能误拦。遇到时按 `karma audit` 看 `⚠️ 可能假阳` 标记，给作者反馈
- **确实有假阴**（漏拦真违反） — 用户故意伪装（假数字「9999 测试通过」/ kebab-case 包裹 ML jargon 等）regex 分不清。karma 信任用户不蓄意作弊
- **`karma audit` 修后 0 触发 ≠ 根本性 fix 正确** — 可能只是 fix 过宽把真阳吃了。dogfooding 数据是嫌疑提示不是 truth
...
可选个人 sticky（全权委托型用户）：
- `keep-pushing-no-stop`（**元层**）完成一波后立即推下个，不停下等用户决定。Stop hook 配合 `decision=block` 让 Agent 不立即停下继续生成（safeguard：单 turn 累积 ≥ 2 次 `stop_block_max_per_turn` 默认值后让 Agent 停下）。v0.4.41 加 user prompt 上文叫停字眼整 turn 豁免（sticky #8 例外清单工程层 enforced）。
...
karma v2 「事后审计」架构有天花板 — Agent 容易学到「怎么不被 hook 拦」而不是「按 sticky 行为」。v3 往「proactive 教练」方向演化，已落地 5 步：

- **v0.4.24 中段注入 anchor** — PostToolUse hook 在每次 tool call 后注入最近触发过的 sticky 简化提醒。Claude Code 协议层验证 `additionalContext` 接受 — sticky 不再只在 user prompt 头部一次注入
- **v0.4.25 字面多样性元行为监测** — `karma audit --with-fix-timeline` 加 🎭 字面试探标记。Agent 用多种 snippet 字面变体触发同一 sticky 时（如 keep-pushing 91% 多样性）标出来，让用户看到 Agent 在反向工程绕 check 还是改行为
- **v0.4.26→v0.4.27 反思式语气改造** — keep-pushing / chinese-plain 两条表达风格类规则的 suggested_fix 改反思式（自检你是不是合理停下 / 技术专名必须保留）。`long-term` 补丁 + `non-blocking` sleep 等工程行为类规则保持命令式（防 Agent 自我合理化）
...
- **v0.4.31~33 根因修复** — subagent_start.py ensure_ascii 子 Agent 收乱码 / bypass_karma `json.dump\b` regex 缺 word boundary 假阳 / strip_shell_quoted_literals 复合 shell 嵌套（heredoc + indirect Step 顺序错导致 release notes markdown 反引号路径漏剥）
- **v0.4.34~39 闭环架构 — 按当前模型自动适应阈值** — 用户洞察连击驱动：
  ...
  - **v0.4.35 model_threshold 表** — 用户「衰减拐点 70K-200K 不是 8K，差 10x，建议至少 60K + 自动适应不用手动调」。Opus 80K / Sonnet 60K / Haiku 30K / 老模型 8K 向后兼容 / 未知 fallback 60K
  - **v0.4.36~38 hook payload 路径尝试** — SessionStart payload 含 model（v0.4.36），但 user_prompt_submit / PreToolUse / PostToolUse / Subagent* 都没（v0.4.38 dogfooding 验证 7 turn state.model 仍 None 证明）
  - **v0.4.37 子 Agent model 捕获** — 用户「自己试一下就知道答案」。manual run 实验发现：tool_name 是 "Agent" 不是 "Task"，tool_input 含 model 字段。主 PreToolUse(Agent) 入队 pending → SubagentStart pop 写子 Agent state.model
  - **v0.4.39 根本路径 — transcript_path** — 用户「怎么查 model 你不是就能查么」+「我随时 /status 都能看到当前 model」。深挖发现所有 hook payload 含 transcript_path → reverse scan jsonl 找最后非合成 model 字面 → karma 权威路径，不依赖 payload 含 model 字段。dogfooding 生效证据：state.model='claude-opus-4-7' 写入
...
- **v0.4.41 keep_pushing 加 user_prompt 上下文叫停检测** — dogfooding 触发：用户「不用啦感谢，休息吧」明确叫停但反思 hook 反复触发。根因：keep_pushing.check 只看 Agent response 末尾，看不到 user prompt 上文。sticky #8 例外清单字面（停 / 不用了 / 明天再说 / 先到这 / 算了 / 晚安 / 够了等）从文本声明变工程层 enforced：stop.py 加 `_read_last_user_prompt` + checks `run_checks` 加 user_prompt 入参透传 + keep_pushing `_USER_STOP_HINT_RE` 整 turn 豁免
- **v0.4.42 用户元层 4 任务一波落地 + 核心设计哲学转向** — 接力 session 元层 3 问触发：「真字狂魔」副作用根因 / 「想不出深度推进点」就宣告饱和 / 跨 session 数据混淆。4 任务：① 源头文档「真X」前缀清理 615 → ~140 治 in-context mimicry。② 规则文本「合作默契」语气重写三批次（包装文本 + sticky.yaml 8 条 + 8 个 check 共 14 处 suggested_fix）。③ chinese-plain 工程层临时撤（保留 preference + 代码供恢复）。④ stats/audit/doctor 加 `get_current_session_id()` 按 mtime 选主 Agent session-state + 「本 ses / 历史」对照列。**核心设计哲学转向**：从「规则系统监督 Agent」改「合作默契邀请 Agent」 — 让 Agent 看到提醒第一反应是「调整对齐」而非「防御 / 绕过」

**实测效果对比**（本机 dogfooding）：1 turn 累积 ~60K token 场景下，v0.4.32 (8K 阈值) 触发 7+ 次中段提醒，v0.4.39 (opus 80K 阈值) 触发 0 次 — **7x+ 频率下降，「Agent 防御性写作扭曲」副作用根因消除**。

后续观察方向：跨场景真实用户使用 + 验证 transcript_path 路径在长 session（jsonl 几 MB 级别）性能 + 跨模型场景。
...
karma v2 已完成 M0-M5（多 backend 横向扩展）+ v3 演化（中段注入 / 字面多样性监测 / 反思式语气 / SessionStart baseline / PreCompact 落盘 / SubagentStart 装机 / 子 Agent 独立 state + 按当前模型自动适应阈值）+ 多轮评审 Agent 交叉评审 + 多轮 dogfooding 修真 bug，**392 个测试全绿 (v0.4.42)**。三家 AI 客户端（Claude Code / Codex CLI / Gemini CLI）实测装机 / 卸装 / hook 触发全跑通。
