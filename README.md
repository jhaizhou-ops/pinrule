...
> **长任务中段提醒频率 7x+ 下降。「真字狂魔」副作用根因消除。392 个测试全过。**
>
...
**目录**：[痛点](#你遇到的真问题) · [30 秒上手](#30-秒上手) · [原理](#为什么有效4-个机制) · [实测干预效果](#实测干预效果v0432--v0439) · [性能](#性能量化对照) · [自定义](#自定义你自己的核心方向) · [装机](#装机详情) · [8 个 hook 真生效功能](#8-个-hook-真生效功能) · [karma 不做的事](#试过但放弃的karma-不做的事) · [FAQ](#faq) · [文档导航](#文档导航)
...
| **Agent 看到提醒激发防御反应 / 找借口合理化** | 「⚠️ 上次违反！」红警示 → Agent 堆「真字狂魔」类前缀防御性自证（单 response 30+ 次「真X」前缀） | 「合作默契」语气替代「请始终遵守」规则系统，第一反应是「调整对齐」非「绕过」 |
...
当代 Claude 真衰减拐点 **70K-200K**（不是早期模型 8K），karma `model_threshold.py` 按模型自适应（Opus 80K / Sonnet 60K / Haiku 30K）。
...
「请始终遵守」激活 fight-or-flight 防御反应 → Agent 找借口合理化绕过。「跟你协作的真人用户希望...」激活 cooperation → Agent 第一反应是调整对齐。

**Test**: Agent 看到「⚠️ 上次违反！」会堆「真字」前缀防御性自证吗？v0.4.41 之前是 30+ 次/response，v0.4.42 改合作默契语气后是 0 次。

### 4. 协议层 schema 真合规

v0.4.43 / v0.4.44 系统性根治早期 3 个「stop 类」hook（Stop / SubagentStop / PreCompact）错用 `hookSpecificOutput.additionalContext` 真 bug（协议不支持，长期被 Claude Code 静默拒绝）。

**Test**: Claude Code UI 报 `Expected schema` 错吗？v0.4.44 后答案是 No。

---

## 实测干预效果（v0.4.32 → v0.4.39）

作者本机连续一年 dogfooding 跑 karma 自身开发，模型自适应阈值架构落地真效果：

| 场景 | v0.4.32（8K 阈值） | v0.4.39（按模型自适应） | 改善 |
|---|---|---|---|
| 1 turn 累积 ~60K context 中段提醒频率 | 7+ 次 / turn | 0 次 / turn | **7x+ 频率下降** |
| Agent 防御性「真字狂魔」副作用 | 30+ 次/response 堆「真X」前缀 | 0 次 | **根因消除** |
| sticky 跨 compact 保留 | compact 后压成模糊词 | PreCompact + SessionStart 两端夹击 | **100% 跨保留** |
| hook 输出 schema 合规率 | Stop / SubagentStop / PreCompact 全错（v0.4.43 + v0.4.44 fix）| 8 个 hook 全合规 | **100%** |
| 装机生效频率 | Claude Code 8 / Codex 4 / Gemini 4 = 16 个 hook 全 ✓（Codex 0.130 需 `/hooks` 审批）|
...
所有 hook 输出都协议层 schema 合规（v0.4.43 / v0.4.44 系统性根治）。
...
继承 [karma v1 失败教训](https://github.com/jhaizhou-ops/karma-v1) + 6 周 dogfooding 验证：
...
| **「请始终遵守 / 立即按 fix / 不要再犯」类警示词** | v0.4.42 dogfooding 真验证 — 激活 Agent 防御反应导致「真字狂魔」副作用，换合作默契语气后副作用消除 |
...
karma `data/sticky.dev.example.yaml` 的 7 条是作者一年 dogfooding 累积的真痛点 — **但不是给你照搬的**。读完 `karma sticky list` 后保留映射到你真实翻车现场的，其余删掉换成你自己的真痛点。
...
- [docs/RULES_REDESIGN_PROPOSAL.md](./docs/RULES_REDESIGN_PROPOSAL.md) — v0.4.42 「合作默契」语气设计提案
