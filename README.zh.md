...
>
> 纯工程零 LLM 依赖，hook 响应 60ms 以内。**中文 + 英文用户都覆盖（v0.8.0）**；加新语言只需给每个 signal 目录提交一个 `.txt`，零 Python 代码，零 LLM 在循环里。
>
> ---
>
...
| **质量门禁** | lint / 类型检查 / 死代码 / 452 个单元测试，全绿 | 加上持续真实环境 dogfooding |
| **hook 延迟** | < 60ms（`user_prompt_submit` 实测 ~49ms） | AI 客户端协议预算 200ms |
| **每 turn token 消耗** | 头部 ~400 token + 中段刷新 ~60 token | 60K context 的 1% 以内 |
...
| **用户语言** | 中文 + 英文（v0.8.0 / v0.8.1），可扩展 | 6 个检测信号全外部化到 `data/signals/<name>/{zh,en}.txt`（平面字眼）或 `.yaml`（cartesian 模板 + 词集）。加新语言 = ~6 个小文件，零 Python 代码 |
