...
> **运行时**：纯工程 · 零 LLM · 零联网 · 零运行时依赖 · hook 通常 50-70ms · token 占比约 2%。（场景规则包调研由你的 Agent 完成，见下面 Path B。）
...
`pinrule init` 在 `~/.pinrule/` 装默认规则 + 自动给所有检测到的客户端（Claude / Codex / Cursor / Hermes）装 hook。后续如果有新客户端就跑 `pinrule install-hooks` 补上 hook。
...
- **持续注入** 你给 Agent 设定的规则：每个 session 开始时注入完整规则；每轮对话头部带一个精简锚点（rule id + 一句核心方向 + 上一轮偏离过的规则标记）；长上下文累积到注意力衰减拐点时，自动重新注入完整规则。
...
| **支持客户端** | Claude / Codex / Cursor / Hermes — [加新 backend](./pinrule/backends/HOWTO.zh.md) |
...
> **边界**：pinrule 运行时不调 LLM、不联网；场景调研由你的 Agent 完成，pinrule 只负责校验、写入和本地运行规则。
