...
| **质量门禁** | lint / 类型检查 / 死代码 / **845 个单元测试**，全绿 (CI: 4 matrix job ubuntu+macos × py3.11+3.12) | 加上持续真实环境 dogfooding |
...
Memory 系统擅长「记得用户上周说过 X」。pinrule 擅长「用户的 7 条核心方向每 turn *自动*注入到 prompt 头部, 不管 Agent 主动去查不查 — 真违反实时拦截」。
