...
> **让 AI 在长任务里不忘掉你的规则。纯工程, 零 LLM, 通常 50-70ms hook 延迟.**
...
### Cursor：Agent Transcripts（回复级检查）

**什么时候需要：** 规则注入、改文件前 Read、Shell/MCP 拦截等**不依赖** transcript。  
**需要 transcript 的：** `keep-pushing`、`chinese-plain`、`loud-failure` 等「看完 Agent 上一句回复再判」的规则。  
另：`afterAgentResponse` hook 直接带 `text` 字段，**不依赖** `transcript_path` 也能扫上一句回复。

**重要：桌面 Agent 一般不用你手动开开关。**  
`pinrule install-hooks --backend cursor` + 在 **Agent / Composer** 里正常聊几句后，Cursor 会自动写  
`~/.cursor/projects/<workspace>/agent-transcripts/<session>.jsonl`，并在 hook stdin 里带上 `transcript_path`。  
本机 dogfood：`stop` / `afterAgentResponse` **100%** 带路径；`sessionStart` **总是 null**（正常，起手时文件还没创建）。

**无法用 CLI 可靠「打开」：** 未在 `~/Library/Application Support/Cursor/User/settings.json` 或公开文档里找到可写的 `transcript` 配置项；`cursor` CLI 也无对应子命令。`pinrule` **不能**像 Codex `trusted_hash` 那样自动改 Cursor 内部开关。

**怎么确认（给 Agent / 用户一条命令）：**

```bash
pinrule doctor    # macOS：解析最新 Cursor Hooks 日志里的 transcript_path 统计
```

或手动：底部 **Output → Hooks**，在 `stop` / `preToolUse` 的 INPUT 里应看到非 null 的 `.jsonl` 路径。

**若 `stop` 也长期全是 null：** 检查 Cursor **隐私模式**是否禁止本地保留 Agent 对话；用真实项目文件夹开 Agent（不要只靠无 workspace 的窗口）；Reload 后新开 Composer。Claude Code / Codex 用户**不用**这一步。
...
| **token 累积消耗** | 1.8K SessionStart baseline（一次性, 但每 turn re-send）+ 每 turn anchor **只列本 session 违反过的规则**（v0.13.0+; 没违反 = 0 anchor）+ 累积达模型衰减拐点（Opus 60K / Sonnet 40K / Haiku 30K）自动 refresh | **真 dogfood 实测（30 sessions, v0.13.0 上线后）: 60% 工作 session = 0 anchor token (干净没漂移, 完全 passthrough)；~40% 有违反的 session 比 v0.12.x 省 45-80%；真 median 每 session 累积违反 1 条（v0.13.0 上线时假设 3-5 条, 真实远低于）。**sessionStart baseline + PostToolUse 中段重注入覆盖 anti-decay, anchor 纯聚焦 drift 信号 |
