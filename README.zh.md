...
> **让 AI 在长任务里跟你的规则保持对齐 — 纯工程零 LLM, hook 响应 < 60ms.**
...
**目录**：[痛点](#你遇到的问题) · [10 秒上手](#0-依赖纯工程10-秒上手) · [`/karma` 自然语言录入规则](#karma-自然语言--agent-替你写规则) · [使用效果](#使用效果) · [原理](#为什么有效) · [性能](#性能) · [8 个 hook 全覆盖](#8-个-hook-位置全覆盖) · [karma 不做的事](#试过但放弃的karma-不做的事) · [诚实边界](#诚实的工具边界) · [FAQ](#faq) · [心智模型](#心智模型) · [文档导航](#文档导航)
...
仓库内一份 Markdown source of truth 在 [`skills/karma/SKILL.md`](./skills/karma/SKILL.md)；`karma install-skill` 命令自动处理 Markdown → TOML 转换给 Gemini commands 路径。
...
| **质量门禁** | lint / 类型检查 / 死代码 / 620+ 个单元测试，全绿 | 加上持续真实环境 dogfooding |
| **hook 延迟** | < 60ms（`user_prompt_submit` 实测 ~49ms） | AI 客户端协议预算 200ms |
| **每 turn token 消耗** | UserPromptSubmit 注入 ~490 token 精简 anchor；完整规则 baseline（~1.8K token）SessionStart 一次注入 + 累积达模型衰减拐点（Opus 60K / Sonnet 40K / Haiku 30K）自动补 | 100 turn 跨 1M Opus context 占 ~8% |
...
对话生命周期时序图, 每个 hook 在哪里 fire 一眼能看清:

```
session 起手 / compact 重起
        │
        ▼
 ┌─ SessionStart ──┐  注入完整规则 baseline (compact 重起时读 snapshot)
 │                 │
 ▼                 │
 ...
 │ 精简 anchor +      │  │ 子 Agent 继承规则 │    │ 临时 state 销毁 │
 │ 偏离标记           │  └─────────────────┘    └────────────────┘
 └────────────────────┘
        │
        ▼
 Agent 调 Bash / Edit / Write
        │
        ▼
 ┌─ PreToolUse ──┐  扫命令 + session 行为时序; 命中 → deny
 │               │
 ▼               │
 tool 真跑       │
        │        │
        ▼        │
 ┌─ PostToolUse ─┐  跟踪状态 + 累积到衰减拐点中段全量 reinject
 │               │
 ▼               │
 Agent 出回应    │
        │        │
        ▼        │
 ┌─ Stop ────────┐  强提醒 + 推进 nudge + 短期意图话术拦截
                              │
                              ▼
 ┌─ PreCompact ──┐  落盘完整规则 → SessionStart(source=compact) 重读
                  (客户端 auto-compact 长 context 时 fire)
```
...
- [docs/CODEX_BACKEND.zh.md](./docs/CODEX_BACKEND.zh.md) — Codex backend 所有权边界跟契约（双语）
