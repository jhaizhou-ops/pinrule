...
| **token 累积消耗** | 1.8K SessionStart baseline（一次性, 但每 turn re-send）+ 每 turn 新加 ~490 anchor（每个 anchor 留在 conversation 里, 后续每 turn 都重发 — LLM 无状态, 每次调用都从头重读完整 history）+ 累积达模型衰减拐点（Opus 60K / Sonnet 40K / Haiku 30K）自动 refresh | **典型工程 session 占总 API input token ~10-15%；长 500-turn dogfood 占 1M context window ~25%。**设计取舍: 每 turn 新 anchor 让最新 drift marker 立刻可见。v0.13.0 delta-only anchor 优化（压到 ~2%）在 backlog |
