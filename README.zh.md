...
>
> 纯工程零 LLM 依赖，hook 响应 60ms 以内。**中文 + 英文用户都覆盖**；加新语言只需给每个 signal 目录提交一个 `.txt`，零 Python 代码，零 LLM 在循环里。**v0.9.0 每 turn token 成本降 73%**：SessionStart 全量 baseline + 每 turn 精简 anchor + 衰减拐点累积达阈值时全量补。
>
> ---
>
> **历史版本**：v0.6.0（2026-05-15）经过 18 个 release 的废弃周期后删除了 `karma.sticky` 模块 / `.sticky_id` 属性 / `karma sticky` CLI。迁移机械化（符号 `s/sticky/rule/`，CLI 换 `karma rule`）。老的 `sticky.yaml` 跟 `violations.jsonl` 自动迁移、保持可读 — 详细 cookbook 见 [CHANGELOG v0.6.0](./CHANGELOG.zh.md)。
...
**目录**：[痛点](#你遇到的问题) · [10 秒上手](#0-依赖纯工程10-秒上手) · [`/karma` 自然语言录入规则](#karma-自然语言--agent-替你写规则) · [使用效果](#使用效果) · [原理](#为什么有效) · [性能](#性能) · [8 个 hook 全覆盖](#8-个-hook-位置全覆盖) · [自定义规则](#自定义你自己的核心方向) · [karma 不做的事](#试过但放弃的karma-不做的事) · [FAQ](#faq) · [文档导航](#文档导航)
...
| **「想加规则但 yaml 门槛高 / 我的措辞 Agent 不响应」** | 你知道想要什么行为，但写规则本身是个体力活 — `violation_keywords` 格式错触发假阳，语气错激发防御 | 在 Claude Code / Codex / Gemini CLI 输 `/karma <自然语言>`，karma skill 自动 refine 语气、格式化 keyword、检测跟现有规则重叠、预览注入、跟你确认、写入 — ~30 秒端到端 |
...
Agent 调 Bash / Edit / Write **之前**，karma 扫命令内容 + 关键词，命中违规规则直接 deny 工具，附改进建议：
...
| **源码** | ~5500 行 Python | 可读可改，没有黑魔法 |
| **质量门禁** | lint / 类型检查 / 死代码 / 460 个单元测试，全绿 | 加上持续真实环境 dogfooding |
| **hook 延迟** | < 60ms（`user_prompt_submit` 实测 ~49ms） | AI 客户端协议预算 200ms |
| **每 turn token 消耗**（v0.9.0）| UserPromptSubmit 注入 ~490 token 精简 anchor；完整规则 baseline（~1817 token）SessionStart 一次注入 + 累积达模型衰减拐点（Opus 60K / Sonnet 40K / Haiku 30K）自动补 | 100 turn 跨 1M Opus context 占 ~8%（v0.9.0 前是 18%）|
| **磁盘占用** | < 10MB | 配置 + 历史违规日志 + session 状态 |
| **模型适配** | 按模型衰减拐点的自适应阈值 | 主流模型用各自实测的衰减点 |
| **支持客户端** | Claude Code / Codex CLI / Gemini CLI | 加新 backend 看 [HOWTO](./karma/backends/HOWTO.zh.md) |
...
`~/.claude/karma/rules.yaml`（`karma init` 会复制默认模板）：
...
- id: long-term-fundamental
  preference: |
    ...
    他愿意为长期质量等你多想几分钟。
  violation_keywords:
    - 我先打个补丁          # 「意图前缀 + 动作」格式区分讨论概念 vs 行动声明
    - 先用 workaround
    - 我先硬编码
  violation_checks:
...
**关键设计点**：
- **`violation_keywords` 用「意图前缀 + 动作」格式**（「我先硬编码」而非「硬编码」）— 区分讨论概念 vs 行动声明，避免「不要硬编码」类自然语言讨论被误判
- **软上限 10 条 / 硬上限 12 条** — 超过 12 条 Claude 倾向只做模式匹配「规则存在」不认读，遵循率反而下降
...
| **引入 LLM 依赖** | 延迟和成本都是问题。纯工程让 hook 保持 60ms 以内，装机也可复现 |
...
`data/rules.dev.example.yaml` 的 7 条默认规则是作者自用累积的痛点 — 不是给你照搬的模板。装完跑 `karma rule list` 看默认有哪些，留下映射到你自己翻车现场的，剩下的删掉换成你自己的。
...
- [CHANGELOG.zh.md](./CHANGELOG.zh.md) — 版本变更历史
- [docs/HANDOFF.zh.md](./docs/HANDOFF.zh.md) — 内部开发接力文档
- [docs/RULES_REDESIGN_PROPOSAL.zh.md](./docs/RULES_REDESIGN_PROPOSAL.zh.md) — 「合作默契」语气设计提案 — 核心哲学转向
