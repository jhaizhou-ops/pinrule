...
from pinrule.backends.protocol_adapter import normalize_tool_input, normalize_tool_name
...
def main() -> int:
    try:
        ...
        print(f"pinrule PostToolUse: 输入 JSON 解析失败 ({e})", file=sys.stderr)
        print(json.dumps({}))
        return 0

    # v0.9.16 debug instrumentation: PINRULE_DEBUG_DUMP_PAYLOAD env 设到 path 时
    # append payload (jsonl) — cross-backend audit / 真 schema sampling 用。
    # 默认关，不影响正常运行。
    _dump = os.environ.get("PINRULE_DEBUG_DUMP_PAYLOAD")
    if _dump:
        try:
    ...
    # v0.9.15 cross-backend: tool_name 归一化到 pinrule canonical（Claude 风格） —
    ...
    # envelope → pinrule canonical Edit-shape dict 含 multi_file_targets 让
    ...
    try:
        ...
        print(f"pinrule PostToolUse: 保存 session_state 失败 ({e})", file=sys.stderr)
        additional_context = ""
...
def _build_smart_reinject(session_id: str, state, payload: dict | None = None) -> str:
    ...
    """
    try:
        from pinrule.rule import load as _load_sticky
    ...
    try:
        ...
        # v0.4.35 阈值来源优先级：config.json 显式配置 > 按模型自适应 > DEFAULT 40K
        # (v0.9.0 把 DEFAULT 从 60K 收紧到 40K 时这条注释忘改, v0.10.5 修)
        # 用户 config.json 给 reinject_every_n_tokens 数字 → 强制覆盖
    ...
    from pinrule.rule import format_for_injection
