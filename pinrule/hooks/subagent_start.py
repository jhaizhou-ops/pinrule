"""SubagentStart hook — 子 Agent 启动时注入 sticky baseline（pinrule v3 第六步）。
...
def main() -> int:
    try:
        ...
        print(f"pinrule SubagentStart: 输入 JSON 解析失败 ({e})", file=sys.stderr)
        _passthrough()
        return 0
    ...
    if agent_id and cursor_model:
        try:
            ...
            print(
                f"pinrule SubagentStart: 写子 Agent model 失败 ({e})",
                file=sys.stderr,
            )
    if agent_id:
        try:
            ...
            print(f"pinrule SubagentStart: pop 子 Agent model 失败 ({e})", file=sys.stderr)
            # 失败不阻塞 sticky baseline 注入

    try:
        rule_list = load_sticky()
    except Exception as e:
        print(f"pinrule SubagentStart: 规则加载失败 ({e})", file=sys.stderr)
        _passthrough()
        return 0
