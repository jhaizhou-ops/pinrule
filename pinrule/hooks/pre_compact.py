"""PreCompact hook — compact 前落盘 sticky 完整状态（pinrule v3 第五步）。
...
- 落盘规则完整状态到 `~/.pinrule/pre_compact_snapshot.md`
  保存：完整 rules.json + 最近 5 turn 违反清单 + compact 触发时间 + session_id
...
def _snapshot_path() -> Path:
    return pinrule_home() / SNAPSHOT_FILENAME
...
def main() -> int:
    try:
        ...
        print(f"pinrule PreCompact: 输入 JSON 解析失败 ({e})", file=sys.stderr)
        _passthrough()
        return 0
    ...
    try:
        ...
        print(f"pinrule PreCompact: 规则加载失败 ({e})", file=sys.stderr)
        _passthrough()
        return 0
    ...
    try:
        ...
        if state.turn_count > 0:
            ...
            from pinrule.violations import recent as _recent_ts
    ...
    try:
        ...
        print(f"pinrule PreCompact: 落盘失败 ({e})", file=sys.stderr)
        _passthrough()
        return 0
    ...
    from pinrule.backends.protocol_adapter import detect_backend, emit_context_injection
