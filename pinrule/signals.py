"""pinrule 检测信号字眼的 i18n 加载器。

pinrule 的 regex 检测对象是「用户跟 Agent 的对话文本」— 用户/Agent 用什么
语言对话，pinrule 就要在那个语言层面识别信号。所以信号字眼必须按语言分别
...
- `signals.py` — pinrule 听 Agent / 用户说什么（regex 检测字眼）
"""
