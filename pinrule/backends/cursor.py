...
① **每 turn 注入走 `beforeSubmitPrompt` → `user_prompt_submit`**. Cursor 官方
   ...
   `hookSpecificOutput.additionalContext` 可用**. pinrule v0.12.2+ 装此 hook +
   `emit_context_injection` 对 beforeSubmitPrompt 走 nested shape. 另同步
   `~/.cursor/rules/pinrule-sticky.mdc` (`alwaysApply`) 作模型起手可见的保险层
   (dogfood: hook sessionStart stdout 有、模型起手自检无; Cursor Rules 起手有).

② **stdin payload 用 `conversation_id` 不是 `session_id`**. v0.12.1 起
   `pinrule.hooks._payload.extract_session_id` 做 `session_id → conversation_id`
...
③ **Stop hook 协议从根本不同**. Cursor `stop` 不接受 `{"decision": "block",
   ...
   对应 pinrule keep-pushing 的「stop 时塞反思 prompt 让 Agent 继续推」语义 — Cursor
...
# Cursor → pinrule canonical tool_name 映射.
# Cursor 文档列的 preToolUse tool_name: "Shell|Read|Write|Grep|Delete|Task|MCP:*"
# pinrule 主要消费 canonical: Bash / Read / Write / Edit.
...
_CURSOR_TOOL_MAP: dict[str, str] = {
    ...
    # event. pinrule 的 read_first check 看 Write 等同 Edit 语义 — 在主逻辑层一致.
}
...
class CursorBackend(JsonHooksBackend):
    ...
    # 套用 Claude 的 PascalCase. wrapper basename 保持 pinrule 内部规范让
    # hook 入口模块 (pinrule/hooks/*.py) 跨 backend 完全复用.
    #
    ...
    def is_pinrule_entry(self, entry: dict) -> bool:
        """Recognize pinrule in native flat entries and legacy Claude nested entries."""
        if "pinrule_" in entry.get("command", ""):
            return True
        return super().is_pinrule_entry(entry)
    ...
    def _sync_native_rules_and_reload_hint(self) -> list[str]:
        ...
        logs.append(
            "  → 改 rules 后跑 `pinrule sync-cursor-rules` 刷新 .mdc;"
    ...
    def emit_deny(self, reason: str, payload: dict) -> str:
        ...
        agent_message = 发给 Agent 看的拦截理由 (pinrule 真想塞的 sticky 反思);
        user_message = 弹给用户看的 (pinrule 给两者填同内容, 让用户 + Agent 都看到).
        """
        return json.dumps({
            "permission": "deny",
            "user_message": reason,
    ...
    def emit_stop_block(self, reason: str, payload: dict) -> str:
        ...
        让 Cursor 自动以这段文本作为 user 回复继续 Agent 循环. 这正映射 pinrule
        keep-pushing 的「Agent 想 stop 时塞反思 prompt 让继续推」语义.

        相比 Gemini AfterAgent 没 block 概念被迫返 {} fail-open, Cursor 这个
        协议天然适配 pinrule 干预 — block 不了但能 redirect 继续, 等效 pinrule 想要
        的「不让 Agent 草草收工」效果.
        """
    ...
    def skill_install_targets(self, skill_name: str = "pinrule") -> list[tuple[Path, str]]:
        ...
        pinrule 不能跨所有未来 project 一次装 skill — 返空 list 不装. 用户想用 /pinrule
        自然语言加规则功能, 需手工 cp `data/pinrule-skill/SKILL.md` 到每个目标项目的
        `.cursor/skills/pinrule/`. post_install_message 响亮告知这个限制 (rule #4
    ...
    def post_install_message(self) -> list[str]:
        ...
        msgs.extend([
            ...
            "",
            "    影响: pinrule 核心能力 (sticky 规则注入 + 行为拦截) 完全可用. 但是",
            "    `/pinrule <自然语言>` 这种用 skill 走捷径加规则的功能在 Cursor 不能",
