...
v0.10.x 曾决定不接 PermissionRequest, 因为 pinrule 不想做第二套权限审批系统。
v0.15.0 native-first refactor 改为安装这个 native event, 但只把它路由到现有
pre_tool_use wrapper / _tool_gate, 不新增独立 check engine:

- 命中 pinrule 规则: 用 Codex PermissionRequest 官方 shape
  `{hookSpecificOutput: {hookEventName: "PermissionRequest",
  decision: {behavior: "deny", message}}}` 拦截。
- 未命中 pinrule 规则: 仍输出 `{}` fail-open，让 Codex 自己继续正常 permission
  approval 流程；pinrule 不替用户自动 allow。
- 仍不把 pinrule 定位成权限审批系统；PermissionRequest 只是 native gate surface
  覆盖, 复用同一套行为规则检查。
...
# Codex → pinrule canonical tool_name 映射
...
def parse_apply_patch_envelope(envelope: str) -> list[dict[str, str]]:
    ...
    @@ hunks or +/- line content (pinrule only needs file paths for state
    tracking).
    """
        ...
        for key in ("command", "input", "patch", "diff"):
            ...
            if isinstance(v, str) and v.strip():
                if key not in verified_keys:
                    print(
                        f"pinrule codex backend: warning — apply_patch 用了未捕获的 wrap "
                        f"key {key!r}, 防御式继续解析. 请把这种 case 报到 GitHub issue "
                        f"让 pinrule 维护者收紧 _extract_codex_patch_text 到真捕获过的 key.",
                        file=_sys.stderr,
                    )
                return v
    return ""
...
def _extract_head_tail_pipe_read_paths(tokens: list[str]) -> list[str]:
    ...
    - `grep -r/-R/--recursive` reads a tree; directory prefixes do not satisfy
      pinrule's per-file read tracking.
    """
...
def codex_hook_trusted_hash(event_key: str, command: str, timeout: int = 30) -> str:
    ...
    algorithm so pinrule can pre-trust the pinrule-owned wrappers it just wrote.
    """
    identity = {
...
class CodexBackend(JsonHooksBackend):
    ...
    def codex_hook_state_entries(
        ...
        """Build `[hooks.state]` entries that pre-trust pinrule-owned wrappers.
        ...
        mirrors Codex's own key/hash derivation for the entries pinrule installs
        into `~/.codex/hooks.json`, and intentionally never inspects or trusts
        non-pinrule hook entries.
        """
        if settings is None:
        ...
        for event_name in self._HOOK_EVENTS:
            ...
            for group_index, entry in enumerate(hooks.get(event_name, [])):
                if not self.is_pinrule_entry(entry):
                    continue
                for handler_index, hook in enumerate(entry.get("hooks", [])):
                    command = hook.get("command", "")
                    if "pinrule_" not in command:
                        continue
    ...
    def trust_pinrule_hooks(self, settings: dict | None = None) -> list[str]:
        """Persist Codex hook trust state for pinrule wrappers in config.toml.

        This is a narrow replacement for the manual `/hooks` review step. It
        only writes `[hooks.state]` entries for pinrule-generated wrapper paths.
        ...
        added = len(missing)
        return [f"Codex pinrule hook trust state 已写入 {config_path} ({added} 新增, {replaced} 更新, {total} 总计)"]

    def save_settings(self, data: dict) -> None:
        super().save_settings(data)
        try:
            self.trust_pinrule_hooks(data)
        except OSError:
    ...
    def normalize_tool_input(
        ...
        """Codex apply_patch (字符串 envelope) → pinrule canonical Edit shape dict.
    ...
    def post_install_message(self) -> list[str]:
        ...
        实测 2026-05-16：本机 codex cli 跑 apply_patch 编辑文件**真成功**，但 pinrule
        violations.jsonl + session-state 完全无新条目 → hook 根本没 fire. 根因
        就是用户没在 TUI `/hooks` 审批. v0.10.2 起 pinrule 按 Codex 0.130 源码
        算法给自己生成的 wrapper 写入 trusted_hash；这里仍提示用户如何核验。
        """
        hooks_dir = self.hooks_dir()
        wrappers = list(dict.fromkeys(
            f"pinrule_{basename}.py" for basename in self._HOOK_EVENTS.values()
        ))
        ...
        msg: list[str] = [
            ...
            "━" * 70,
            "",
            "pinrule 已为自己生成的 Codex wrapper 写入 trusted_hash，正常情况下",
        ...
        msg.extend([
            "",
            "▶ 验证生效:",
            "  安装后随便让 codex 改一个你没先 Read 过的文件 — 应该被 pinrule 🛑 拦截.",
            "  如果还是不拦，跑 `pinrule doctor` 看诊断或来 issue 反馈.",
            "",
            "━" * 70,
        ])
        return msg
