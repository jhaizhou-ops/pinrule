"""Sync pinrule rules into Cursor native `.mdc` rules (model-visible layer).

Cursor hook `sessionStart.additional_context` is confirmed to reach pinrule stdout
...
This module mirrors `format_for_injection()` into `~/.cursor/rules/pinrule-sticky.mdc`
...
_RULE_FILENAME = "pinrule-sticky.mdc"
_MANAGED_MARKER = "<!-- pinrule-managed: do not hand-edit; run `pinrule sync-cursor-rules` -->"
...
def build_mdc_content(rule_text: str, *, id_catalog: str = "") -> str:
    """Wrap pinrule injection text as Cursor always-on rule file."""
    ...
    return (
        "---\n"
        "description: pinrule long-term collaboration agreements (auto-synced)\n"
        "alwaysApply: true\n"
        "---\n\n"
        f"{_MANAGED_MARKER}\n\n"
        f"{body}\n"
...
def sync_cursor_rules(
    *,
    ...
    """Write pinrule rules to Cursor `.mdc` file(s). Returns (paths_written, log_lines)."""
