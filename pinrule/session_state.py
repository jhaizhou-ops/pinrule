...
# Windows 没 fcntl，graceful fallback no-op lock（pinrule classifier 只声明
...
DEFAULT_DIR = pinrule_home() / "session-state"
...
class SessionState:
    ...
    # pinrule 主 PreToolUse 把 model 入队 pending_subagent_models，SubagentStart
...
def _state_path(session_id: str, base_dir: Path | None = None, agent_id: str | None = None) -> Path:
    ...
    v0.4.34（用户「子 Agent 独立 pinrule 监控」架构）：agent_id 给的话 path 加
