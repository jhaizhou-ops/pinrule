...
from pinrule.checks.common import extract_tool_text, is_python_c_command
...
# v0.5.5：宿主语言 -c/-e flag 命令头识别 — 跟 non_blocking / bypass_pinrule 同根因.
# python/node/ruby/perl -c "..." 内 `gold_cases.append` / `if turn_idx >= 400` 等
# 字面是字符串数据不是执行意图. v0.5.3 dogfooding 触发: 探针脚本里
# `r = testset_check(..., content='gold_cases.append(x)')` 被错拦. 同 v0.4.18
# non_blocking sleep 探针根因.
# v0.5.13: _LANG_C_HEAD_RE 已下沉到 pinrule.checks.common.is_python_c_command() —
# 跟 bypass_pinrule / non_blocking 共享同款判定. v0.5.9 Bash heredoc 豁免在
...
def check(*, tool_name: str = "", tool_input: dict | None = None, rule_id: str = "", **_):
    ...
    # 含 `gold_cases.append` / `if turn_idx >= N` 字面是字符串数据不是执行.
    # 跟 non_blocking sleep / bypass_pinrule write 同根因 fix.
    if tool_name == "Bash":
