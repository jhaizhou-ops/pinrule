...
# 终结符前允许 tab/space 缩进 — `<<-` 形式 bash 会剥 tab 缩进，pinrule 也要识别到
...
# 多个 check (testset / bypass_pinrule / non_blocking) 都需要这个判定豁免 — 因为
...
def strip_shell_quoted_literals(cmd: str) -> str:
    ...
    # 触发：`gh release create --notes "$(cat <<'EOF' ...`cat ~/.claude/pinrule/...`
