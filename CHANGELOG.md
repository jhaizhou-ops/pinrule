...
- **External review polish — 6 fixes (8.8/10 friend review)**:
  ...
  - **Uninstall command drops `.venv/bin/`** (bilingual): with `pip install pinrule` as the primary install path, telling people to run `.venv/bin/pinrule uninstall-hooks` was inconsistent. Now `pinrule uninstall-hooks` to match pip-path users.
  - **`/pinrule` skill install boundary clarified** (bilingual): previous wording "Auto-installed on Claude + Codex by `pinrule init`" left the `init` / `install-hooks` / `install-skill` boundary unclear. Now spells out: skill goes in via `install-skill`, hooks via `install-hooks`, `init` runs both as first-run convenience.
  - **zh README sample sentence fix**: L130 demo example still quoted the old pre-v0.16.9 bad phrasing `"跟你协作的是一位这位用户"` (双重「一位+这位」病句) — actual injected text was already fixed in zh.yaml v0.16.9 but the README sample drifted. Updated to `"跟你协作的这位用户，列出了几条长期最看重的方向。"`.
  - **`PINRULE_HOME` sandbox promise lockdown** (6 new tests): README/ARCHITECTURE promise "everything installs under sandbox, real `~/.claude` etc. one byte untouched" is a hard claim. Previous test coverage only verified `pinrule_home()` (data dir); `pinrule_install_root()` (hook wrappers / settings.json / skill files / Cursor rules root) was fully uncovered — one regression and the promise becomes false advertising. New `tests/test_paths.py` covers all 3 backend (Claude/Codex/Cursor) hook+settings paths anchoring under `PINRULE_HOME`, plus a `test_real_home_untouched_when_sandbox_active` guard that asserts no backend path leaks to real home. **851 tests passing (845 + 6 new lockdown)**.
- **HOOK_CONFIGURATION_GUIDE Codex path correction** (bilingual): wrote `~/.codex/config.toml` as the hook-entries path; actual file is `~/.codex/hooks.json` (config.toml only stores `trusted_hash`). Fixed in both EN and zh + the "turn off" Q.
- **ARCHITECTURE `PINRULE_HOME` section refreshed** (bilingual): was stuck describing v0.15.x "data-dir-only" semantics; rewrote with a 5-row table covering all anchors (data dir / hook wrapper / settings.json / skills / Cursor rules) and explicitly distinguished the two helpers `pinrule_home()` and `pinrule_install_root()`. README intentionally does **not** document this — sandbox is a power-user feature, having users discover it organically beats putting it on the homepage.
- **PRD scenario positioning sync** (bilingual): "7 default rules" paragraph stuck on pre-v0.16.7 wording. Updated to reflect bilingual symmetry (v0.16.7) + EN `plain-language-no-jargon` rename (v0.16.8) + `deep-fix-not-bypass` proper name.
- **HOOK_CONFIGURATION_GUIDE bilingual split**: original file was Chinese-only despite `.md` naming convention (we use `.md` for EN, `.zh.md` for Chinese in all other docs). Renamed Chinese content to `HOOK_CONFIGURATION_GUIDE.zh.md`, wrote fresh EN version (human-translated, not machine). Both now have multi-backend table (Claude 8 / Codex 6 / Cursor 12), per-backend hook paths, `PINRULE_HOME` sandbox note, and all 3 official protocol URLs (Claude / Codex / Cursor).
- **README FAQ — pinrule vs memory systems** (bilingual): issue #8 author compared pinrule to mem0 (55k⭐ memory layer). The "pinrule does NOT compete with memory systems" boundary was buried at L368 in the "Tried and rejected" table — first-time visitors from mem0 might miss it. Added a dedicated FAQ entry with a 2-row table contrasting jobs (memory: facts about user, Agent-queried; pinrule: behavioral enforcement, hook-fired) right after the CLAUDE.md differentiation Q.
- **README outdated numbers** (issue #8): `834 unit tests` → **845 unit tests**, `~9.5K LOC` → **~9.7K LOC** (real counts). Issue #8 closed — all 4 review points either already shipped (demo SVG / tagline blockquote / mermaid hook flowchart) or fixed in this batch.
