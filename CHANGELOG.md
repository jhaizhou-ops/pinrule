...
- **ARCHITECTURE PINRULE_HOME section refreshed** (bilingual): was stuck describing v0.15.x "data-dir-only" semantics; rewrote with a 5-row table covering all anchors (data dir / hook wrapper / settings.json / skills / Cursor rules) and explicitly distinguished the two helpers `pinrule_home()` and `pinrule_install_root()`.
