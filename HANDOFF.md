...
`pytest tests/` → **232/232 passed**（M3+M4 加了 157 个新测试，含 Stop matcher 反向约束 2 + turn=None fallback 2 + force_block 豁免 1）
...
1. **触发词假阳性持续观察** — 实战可能还有未发现假阳
2. **sticky.yaml 模板扩** — 当前 7 条，模型支持 10
3. **README.md / PRD.md 同步最新现状**（每次重大改动后追）
4. **跨场景规则集** — 当前只有「软件开发」一个 dev.example.yaml，可加写作 / 研究等场景预设
