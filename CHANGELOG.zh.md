...
记录 pinrule 每个版本的重要变化。版本号遵循 [SemVer](https://semver.org/lang/zh-CN/)。
...
- **README 数字过时** (issue #8): `834 unit tests` → **845**, `~9.5K LOC` → **~9.7K LOC** (实测真数). issue #8 已关 — 4 个 review 点全 addressed (demo SVG / tagline blockquote / mermaid hook 时序图 都已 ship, 数字这次修).
...
karma v1 设计是奖惩/积分系统 — 名字就那时来的. v2 (现在这仓) 完全抛弃了奖惩, 重建成"sticky 行为先验 + 强制 hook 注入". `karma` 这名字继续暗示"有奖惩/RL 系统" (CLAUDE.md L26 自己明确**反对**奖惩) — 名实错位.

外部 8.8/10 v0.15.0 评审没点出名字问题, 但 follow-up 时发现 `pip install karma` 会装到 `Niels Madan / karma 0.1dev` PyPI 占位包 (不是我们的). 改名 `pinrule` 同时解两件事: ① 名实 1:1 ("pin sticky rules; Agent obeys"), ② 全 namespace 干净 (`pip install pinrule` + `github.com/pinrule` + `pinrule.com` + `pinrule.io` 2026-05-17 真查全 free).

### Breaking changes (干净 break, 不自动迁移)

- **Python 包名**: `karma` → `pinrule`. 没发 `karma-rules` 类 alias — 干净 fresh brand.
- **Import 路径**: `import karma` → `import pinrule`.
- **CLI 命令**: `karma init` → `pinrule init` (全 16+ 子命令都改).
- **Hook wrapper 文件名**: `karma_session_start.py` → `pinrule_session_start.py` 等.
- **状态 home 目录**: `~/.karma/` → `~/.pinrule/`.
- **环境变量**: `KARMA_HOME` → `PINRULE_HOME`.
- **Engine check**: `bypass_karma` → `bypass_pinrule` (rule yaml `violation_checks:` 字段).
- **Legacy 迁移路径删了**: `~/.claude/karma` / `~/.cursor/karma` 自动迁移逻辑从 `paths.py` 删 — v0.16.0 fresh brand, 不继承 karma 历史包袱.

### 老 karma 用户迁移

```bash
# 1. 数据目录改名 (如果有 ~/.karma/ 的话)
mv ~/.karma ~/.pinrule

# 2. 拆老 karma hooks (升级前用老 karma CLI 跑)
karma uninstall-hooks
...
- 155 个文件批量 rename + 手工修 corner case (paths.py legacy 逻辑, test fixture, bypass 检测 regex).
- 834 tests 全过 (一个 trusted_hash test update — wrapper basename 改了 → 新 SHA256).
- `pinrule/`, `skills/pinrule/`, `tests/test_bypass_pinrule.py` 全 `git mv` 改 (历史保留).
...
- README + README.zh.md "Claude / Codex / Cursor hook 原生支持" 段加 8 行三端 side-by-side 对照表 (hook 数 / 起手注入 / 工具调用前实时拦截 / Stop 干预 / compact 续命 / 子 Agent 覆盖 / `/pinrule <NL>` 加规则 / 可见性兜底).
...
- `PermissionRequest` 现在安装并自动 trusted，但 pinrule **不变成权限审批系统**：命中规则时返回 Codex 原生 `decision.behavior="deny"`；未命中返回 `{}`，让 Codex 继续自己的正常审批弹窗，避免 pinrule 静默替用户 approve 升权请求。
- `PreToolUse` 与 `PermissionRequest` 复用 `pinrule_pre_tool_use.py`；安装 UX 现在明确显示 6 个 native event / 5 个 wrapper 文件，不再重复生成/提示用户检查同一个 wrapper。
...
诚实边界：Codex docs 仍明确说 `PreToolUse` / `PostToolUse` 还不能拦所有 shell call，`WebSearch` 不覆盖，main-branch generated schemas 可能有当前 release 未支持的 future fields/events。pinrule 只安装 docs 当前 release 明确列出的 surface。

## [0.14.0] — 2026-05-17（minor — 共享 `~/.pinrule` + Cursor 原生能力收口）

### 共享规则库（各端共用）

- 默认 `PINRULE_HOME` 改为 `~/.pinrule/`（与客户端无关）。Claude / Cursor / Codex hooks 读同一份 `rules.yaml`，避免 `~/.cursor/pinrule` 与 `~/.claude/pinrule` 双源注入。
- `pinrule init` 在 `~/.pinrule` 为空时，从 `~/.claude/pinrule/` 自动迁移。
- `pinrule doctor` 检测仍存在的 legacy 第二份 `rules.yaml` 并告警。
- 去掉 Cursor wrapper 里写死的 `PINRULE_HOME=~/.cursor/pinrule`。
...
- `pinrule sync-cursor-visibility`：写入 `pinrule-rules-catalog` skill + `.mdc`（Composer 常不把 hook stdout 放进 `<rules>`）。
- `format_for_injection` 每行带 `[rule-id]`；Cursor 每轮 `beforeSubmitPrompt` 注入 id 目录。
- `python -m pinrule`；`scripts/cursor-install-local.sh` 一键装机。
...
- `pinrule doctor` 正确识别 Cursor 扁平 `{command}` entry (之前误报未引用)
- `pinrule init` 检测 hook 未装全时自动 `install-hooks --backend all`; 有 Cursor 时同步 `pinrule-sticky.mdc`
...
- Cursor install 时 wrapper 默认 `PINRULE_HOME=~/.cursor/pinrule`

## [0.13.2] — 2026-05-17（minor — 砍 Gemini CLI backend，专注 Claude Code / Codex CLI / Cursor）

pinrule 支持的客户端从 4 家收到 **3 家**，专注最常用的 AI 编程客户端. Gemini CLI 装机量小, 维护 4-backend 矩阵 (4× 边角文档, 4× cross-backend audit 成本, 4× 每 release dogfood overhead) 不划算. v0.13.0+ launch-readiness 定位 **Claude Code + Codex CLI + Cursor**.

### 砍了什么

- `pinrule/backends/gemini_cli.py` — 整 backend 文件删
- `pinrule/skill_packaging.py` — Gemini-only Markdown → TOML 转换 helper (只有 Gemini commands 路径用), 整模块删
...
- 所有 `pinrule install-hooks --backend gemini-cli` 跟 `~/.gemini/...` user-facing 提及 (README / PRD / SECURITY / HOWTO 双语)
...
- `pinrule sync-cursor-rules` + `~/.cursor/rules/pinrule-sticky.mdc` (`alwaysApply`) 作 hook 注入不可靠时的起手保险
- `post_tool_use`: Cursor 且 `turn_count==0` 时仍按 `tool_byte_seq` 阈值 reinject
- Cursor wrapper 默认 `PINRULE_HOME=~/.cursor/pinrule`
...
- 端到端: `pinrule install-hooks --backend gemini-cli` 返 "unknown backend" 错 (符合预期); `pinrule doctor` 不再报告 Gemini skill 状态

如果你是现有 Gemini CLI 用户: 留在 v0.13.1 — Gemini backend 最后一个完整维护版本. 后续 Gemini 支持回归走 Codex 同款 contributor-PR pattern (Gemini 侧 maintainer 接手 `pinrule/backends/gemini_cli.py`).
...
v0.12.0 docstring 写过 Cursor "没 UserPromptSubmit 等价 — `beforeSubmitPrompt` 只能 block 不能注入 `additional_context`". Cursor desktop Agent dogfood **证明这是错的**: Cursor `beforeSubmitPrompt` 真接受 `additional_context` 输出, 行为是每 turn 注入点. `_HOOK_EVENTS` 现在 map `beforeSubmitPrompt → user_prompt_submit`, pinrule 每 turn anchor (v0.13.0: 只列违反过的规则) 在 Cursor 也起作用.
...
Cursor `stop` hook 最小 stdin 只 `{status, loop_count}` — 不含 assistant 文本. pinrule response-level check (`keep_pushing_no_stop` / `chinese_plain_no_jargon` / `long_term_response_level` / `loud_failure_with_evidence`) 需上一条 assistant message 才能跑, 没 `transcript_path` 静默 passthrough.
...
**用户能感知**: pinrule token 成本从 **10-15% API input** (v0.12.x) 降到典型工程 session **1-3%**; 长 session context window 占用从 25% 降到 ~5-10%. 优化是真账单降低 — anchor token 节省也会跟着 Anthropic prompt caching 1 折一起传递.
...
- `pinrule.rule.format_anchor_only(rule_list, violated_rule_ids)` — kwarg 从 `recent_violations` 改成 `violated_rule_ids`, 接受 dict[rule_id → turn] (`session_violations()` 返回值) 或 set[rule_id]. 没累积违反时返 `""` (UserPromptSubmit hook 就走 passthrough).
- 新增: `pinrule.violations.session_violations(session_id)` — 返回 session 全程累积 `rule_id → 最近 turn` dict (没 turn 窗口限制; 跟 `recent_turns()` 的 5 turn 窗口互补).
- `pinrule/hooks/user_prompt_submit.py` 调 `session_violations()` 替代 `recent_turns()`.
...
成本降低不削弱 pinrule 的 anti-decay 防线:
...
`pinrule/backends/cursor.py` 现在 override:
...
- `is_pinrule_entry()` — 识别 flat (`command` 含 `pinrule_` 前缀) 跟 legacy Claude nested entries 两种

### Contract test 松绑 (一个 test 从「必须含 nested hooks key」改成「必须含 pinrule_ wrapper 路径」)

`tests/contract/test_backend_contract.py::test_build_event_entry_returns_dict_with_hooks_key` 是 v0.10.1 时代假设 — 当时 Claude / Codex / Gemini 三家都用 nested shape, 当成普适契约. Cursor dogfood 证明跨 backend shape 不是真 invariant. rename 成 `test_build_event_entry_returns_valid_entry`, 只验 dict + `pinrule_` wrapper 路径痕迹. backend-specific shape 留各自 backend test 文件里测.

### v0.12.1 `conversation_id` fallback dogfood 确认有用

Cursor `preToolUse` payload 用 `conversation_id` 不是 `session_id` (`sessionStart` payload 两者都有). `pinrule.hooks._payload.extract_session_id()` fallback 链按设计工作.

### 老实说出未修项 (dogfood report 跟踪, 这波不修)

1. **P1 — `stop` hook 需要 `transcript_path`** 才能 fire `keep_pushing_no_stop` check. Cursor docs 列的 `stop` 最小 stdin 只有 `{status, loop_count}` — 没 assistant 文本. pinrule keep-pushing 通过 `transcript_path` JSONL 读上一条 assistant message; 没这个字段返 `{}` 静默 passthrough. v0.12.4 文档化要求 + 考虑加 graceful skip 路径.
...
pinrule v2 pre-launch — 没有公开 v0.5.0 之前用户需要 migrate. `rule.py` 跟 `cli.py` 从 v0.5.0 (sticky→rule 改名时) 起一直带 `sticky.yaml` legacy fallback + `pinrule init` 自动迁移逻辑. v0.12.2 砍掉这块死代码.

### 砍了什么

- `rule.py`: `_LEGACY_STICKY_PATH` 常量 + `_resolve_default_path()` fallback 函数 + deprecation stderr warning. `DEFAULT_PATH = pinrule_home() / "rules.yaml"` 现在一行了事.
...
- 开发者注释里提到 sticky.yaml 的历史 context (`bypass_pinrule.py` / `description_context.py` 等的 `#` 注释) — 给后续维护者读代码看演进, 不是 user-facing.
- `bypass_pinrule` check 仍然 match `.claude/pinrule` 路径 fragment (任何 pinrule state 文件意外编辑都拦, 不管文件名是啥), 所以 `cp ~/.claude/pinrule/sticky.yaml ~/backup/` 这种绕路企图仍然被抓.

### 验证

- pytest 819 测试全绿 (test fixture 用 `tmp_path / "sticky.yaml"` 是纯字符串字面, 跟 production path resolution 解耦 — 不改也 pass)
- ruff 0 issue, mypy pinrule/ + mypy tests/ 0 error
- 关闭 Task #40 (init migration 条件 bug — 死代码不可能有 bug)

## [0.12.0] — 2026-05-17（minor — Cursor backend 支持，第 4 家 AI 客户端接入）

pinrule 现在能装到 **Cursor IDE 1.7+**（2025-10 发布），跟 Claude Code / Codex CLI / Gemini CLI 平起平坐. `pinrule install-hooks --backend cursor` 写 4 个 hook entry 到 `~/.cursor/hooks.json`, 覆盖 pinrule 规则注入 + 违规拦截全生命周期.

### Cursor 协议级适配

Cursor hooks 协议 (https://cursor.com/docs/hooks) 结构上跟 Claude Code 同源, 但每个 output shape 都有差异. `pinrule/backends/cursor.py` 适配如下:

| 维度 | Cursor 协议 | pinrule 适配 |
|---|---|---|
| Event 名大小写 | camelCase 小开头 (`preToolUse`, `sessionStart`) | `_HOOK_EVENTS` dict literal — 按 Cursor 真期望大小写写进 `hooks.json` |
| Tool 名归一化 | `Shell`（不是 `Bash`） | `normalize_tool_name()` 把 `Shell` → `Bash`, pinrule check 都看 canonical 形 |
...
| Stop hook block | 没 `decision: block` — 用 `{"followup_message": "..."}` auto-continue | `emit_stop_block()` 返 followup_message shape，**正映射** pinrule keep-pushing 反思推进语义 |

Cursor stop hook 协议跟 pinrule 干预模型出奇适配: Gemini `AfterAgent` 没 block 概念 pinrule 被迫返 `{}` fail-open, Cursor `followup_message` 让 pinrule 能 redirect Agent flow 而非只能观察.

### 老实说出两个协议级限制

**没有 UserPromptSubmit 等价.** Cursor `beforeSubmitPrompt` 只能阻断提交, 不能注入 `additional_context`. pinrule 改走 `sessionStart` (一次性 baseline 注入) + `postToolUse.additional_context` (中段重注入). 行为含义: Cursor 用户**不会**在每个 user message header 看到 pinrule sticky 规则重出现 — 规则在 prompt cache + system message 里, 每 turn 模型能看到但不视觉重现.

**没有 home-level global skills 目录.** 据 https://cursor.com/help/customization/skills, Cursor 只支持 `.cursor/skills/` 每个项目根目录. `CursorBackend.skill_install_targets()` 返 `[]`, `post_install_message()` 响亮告知: 想用 `/pinrule` 需手工 cp `skills/pinrule/SKILL.md` 到每个目标项目, 或改走 `pinrule rule add --from-yaml` CLI (CLI 不受影响). 这是 Cursor 协议层限制不是 pinrule 缺陷, 按 `loud-failure-with-evidence` 规则老实说出来.
...
`pre_tool_use.py` + `stop.py` 硬编码中文模板 (「pinrule 拦截：违反」/「Agent 违反」/「建议：」等) — `PINRULE_LOCALE=en` 对这些模板无效, 英文用户看到中英混杂 deny reason. v0.11.4 全部走 `tr()` + 加 6 个 locale key 双语:
...
### PR [#7](https://github.com/jhaizhou-ops/pinrule/pull/7) by @fyn1320068837-source — 153 个新测试覆盖 6 个 untested 模块

第一忠实用户第三次贡献 (#1 #2 之后). Coverage audit 找出 6 个模块没单元测试:

| 文件 | 测试 | 覆盖 |
|---|---|---|
| `test_run_checks.py` | 12 | `run_checks()` 调度: unknown name 静默跳过, exceptions fail-open, `PINRULE_DEBUG` stderr, 多 hit 排序, kwargs 转发 |
...
新顶级 section (在「你遇到的问题」之前), 含 Claude (Opus 4.7) + Codex (GPT 5.5) 跟 pinrule 协作的自评. 第一人称 social proof 来自 pinrule 实际服务的 Agent 自己. 双语.
...
- 5 道 gate: pytest / ruff / mypy `pinrule/` + `tests/` / wheel build / CI 4 matrix
...
Issue [#8](https://github.com/jhaizhou-ops/pinrule/issues/8) 提了 5 个有效 point; 维护者初次回应推迟了 3 个 (「demo GIF 需要 TTY」/「等 PR merge」/「ASCII timeline 够了」). 用户推了 3 次, 每个借口被「先 verify」逐个打回. 结果: 每个借口实际 5 分钟就能解决 (`pip install asciinema termtosvg` + `--command` 非交互 + PINRULE_HOME 隔离). memory `feedback-dont-defer-doable-now` 锁这个教训.

## [0.11.3] — 2026-05-16（minor — `pinrule audit --days N` 时间窗口过滤: dogfood 决策不被老数据稀释）

### 加什么

`pinrule audit` (含 `--by-check`) 加 `--days N` 选项. 只统计最近 N 天的违反, 让 dogfood 决策聚焦 fresh 窗口效果, 不被老数据稀释.
...
```bash
pinrule audit --by-check --days 1        # 最近 24 小时
pinrule audit --by-check --days 7        # 最近 1 周
pinrule audit --days 30 --format md      # 最近 1 月, markdown 表
```
...
`user_prompt_submit.main()` 在 sticky_list 为空时直接 `_output_passthrough; return 0` — 完全跳过 `_advance_turn_state`. 但 model 跟踪 + turn_count 是 **pinrule 系统级 telemetry**, 跟用户有没有装 rules 无关. 本机过因为我 home 有老 `sticky.yaml`; CI 干净 runner 永远空 rules → 永远不写 model.
...
- `monkeypatch.setattr("pinrule.hooks.user_prompt_submit.load", lambda: [])` — 显式模拟空 rules
...
用户最重视的规则是 `deep-fix-not-bypass` (反对 Agent 草草了事不深挖, sticky #1). v0.11.1 给这条 rule 加 L3 时序层 engine pattern, 跟现有 L1 (Bash 字面绕 pinrule 状态) 并存复用同一 `rule_id`.

### 加什么

新增 `pinrule/checks/bypass_pinrule.py` 第二路检测:
...
`long_term.check()` 加 `response: str = ""` 参数, Stop hook 跑 check 时传 response (Stop hook 是 pinrule 看 Agent 整 turn 输出的唯一 hook). 老 tool_input 路径 (Bash/Write/Edit) 不变.
...
`pinrule/backends/_base.py:Backend` 加 2 个新契约方法:
...
v0.10.5 audit sweep 18 个 finding 全部处理完: 10 个在 v0.10.5 + 3 个 v0.10.6 + 5 个 Agent 2 minor 已在 v0.10.5 ship. **6 个连续 v0.10.x release (v0.10.0 → v0.10.6) 构成完整 backend 所有权分工 + 跨平台平价循环**: 架构 (v0.10.0) → codex 3 PR (v0.10.1-3) → pinrule 维护者 parity 推 (v0.10.4) → audit sweep (v0.10.5) → 结构性关闭 (v0.10.6).
...
- README FAQ "Codex 须手动 /hooks 审批" 跟主表格 "立即生效 (v0.10.2 自动信任)" 自相矛盾. 双语改成 "wrapper 由 `pinrule install-hooks --backend codex` 自动信任; 如果 `/hooks` 显示 'modified', Codex 改了 hash 算法 — 手动 approve + 提 issue."
...
- `pinrule/hooks/post_tool_use.py` 现在消费 canonical `tool_input.write_file_paths` (backend-neutral, 跟现有 `read_file_paths` 对称). 任何 backend 的 `normalize_tool_input` 输出这列表会触发 `state.record_edit(path)` 每条 → `last_edit_ts` 推进. 修 codex `sed -i /workspace/src/x.py` 不被 `evidence.check` 看见 (完成词假阳通过). 集成测试 `test_post_tool_use_records_canonical_write_file_paths_advances_last_edit_ts` 锁. **codex backend 后续 (CODEX_BACKEND.md TODO 7)**: codex.normalize_tool_input 当前 `sed -i` 只设 `is_write: True` 没输出 `write_file_paths` — pinrule 端 wiring forward-compat; codex CLI 维护者下个 PR 补字段.

**边界 leak 修** (Agent 2 F1):
- `pinrule/backends/protocol_adapter.py` 删 2 处 `codex` 字面兜底 (`from pinrule.backends.codex import _CODEX_TOOL_MAP` + `REGISTRY["codex"].normalize_tool_input(...)` force-route on `raw_tool_name == "apply_patch"`). `detect_backend()` 通过 `sys.argv[0]` `/.codex/` 字面检测真识别 codex 路由 — 兜底是 vestigial 违反 v0.10.0 "调度层无 backend 字面" 设计自述. 测试改成 mock `sys.argv` 不再依赖兜底.
...
- `pinrule/hooks/pre_compact.py` fallback 数学 fix: `current_turn=999999` + `window=5` 产生 cutoff `999995` 只匹配 turn 999995-999999 (永远不命中真实 session turn 1-100) → `state.turn_count=0` 时 `recent_violation_turns` 永远空, compact-resilience 关键路径 (pre_compact_snapshot.md 「最近 5 turn 违反」段) 失效. fallback 路径改读 ts 维度 `recent(window_sec=24h)`.
- `pinrule/hooks/stop.py` 现在调 `catchup_pending_bg()` via `update_state(try/except + fail-open fallback)`, 跟 Pre/PostToolUse 跟 UserPromptSubmit 一致. 修窗口边缘 case: bg pytest 在最后一个 PostToolUse 之后 Stop hook 之前完成时没 record → `evidence.check` 看 stale `has_recent_test_pass=False` → 完成词被错算 loud-failure 拦.
- `pinrule/hooks/user_prompt_submit.py` strong_reminder 现在写 Violation `turn=current_turn - 1` 不是 `current_turn` — strong_reminder 扫的是**上一**turn 的 assistant response; turn_count 已经在 strong_reminder 之前从 N 推到 N+1.

**Minor regex / docstring 修正** (Agent 1 F1.4 + F1.5 + Agent 3 F3.4 + Agent 2 F5 + F6):
- `pinrule/checks/chinese_plain.py:_PATH_LITERAL_RE` `\w` (Python re 默认 Unicode-aware) 改成显式 `[a-zA-Z0-9./\-_]` ASCII 字符集. 原来吃中文路径段 (`/桌面/某目录/文件.py` 整段被剥), 让中文 ratio 分母变低 → 假阳 `chinese-plain` 拦中文用户用中文路径.
- `pinrule/hooks/post_tool_use.py` 注释写 `DEFAULT 60K` 但 `DEFAULT_THRESHOLD = 40_000` 自 v0.9.0 起 — 改正.
- `pinrule/model_threshold.py` 模块 docstring 列 v0.4.35 阈值 (Opus 80K / Sonnet 60K / Haiku 30K) 但实际 `_MODEL_THRESHOLDS` 是 v0.9.0 + v0.10.4 (Opus 60K / Sonnet 40K / Haiku 30K + 11 个 OpenAI/Codex entry). docstring 更新到现实.
- `pinrule/backends/codex.py:_extract_codex_patch_text` docstring 标 wrap key 哪些真捕获 (只 `input`) 哪些推测 (`patch` / `command` / `diff`), 真 hit 推测 key 时 stderr warning — rule #4 loud-failure-with-evidence + 邀请用户提 issue + 捕获真 payload 让函数收紧.
- `pinrule/model_threshold.py:extract_model_from_transcript` docstring 说明这是 Claude-Code-specific (regex 假设 Claude transcript jsonl shape); 其他 backend 应该 `payload.model` via `model_from_payload`, 不该 fall through 到这.
...
- F2.3 `emit_stop_block(reason, payload)` 契约 — `stop.py` 直 print `{decision: "block", reason}` 不走 backend dispatch. Codex Stop hook 是否接受未验证. pinrule 最强干预 (force_block) 在 codex 下可能静默失效.
...
pinrule 中段 reinject + 按模型自适应阈值原来是 Claude-specific. Codex agent 用 pinrule 时 `gpt-5.5` (1M context 旗舰) 落 DEFAULT 40K 阈值太密扰动表达. v0.10.4 关掉这缺口, 两项改动:

### 统一 `model_from_payload(payload)` — payload.model 优先, transcript fallback

Codex 官方 [hooks docs](https://developers.openai.com/codex/hooks) 明确说每个 command hook stdin 含 `model` 字段 (active model slug), 同时**明确警告** `transcript_path` 格式**不是稳定 hook 接口**. pinrule 之前 user_prompt_submit 和 post_tool_use 直接走 `extract_model_from_transcript()` (v0.4.39 Claude 协议层 limitation 兜底), 错过 codex 稳定信号.

新 `pinrule/model_threshold.py:model_from_payload(payload)` 统一查找:
...
**Codex 行为升级**: 每个 codex hook payload 都含新鲜 model slug (含 `/model` 中途切换后的). pinrule 现在跟切换同步识别模型变化 — `gpt-5.5` agent 立刻拿 120K 阈值, 不用等 transcript 反扫 fallback.
...
| 模型 | Context window | pinrule 阈值 | 理由 |
|---|---|---|---|
...
- **`PreCompact` 不可 hook** — Codex 0.130 hook API 没 PreCompact event. Codex 平台内部有 `enable_request_compression` feature flag 但不暴露为 lifecycle event. pinrule 在 codex 下不能像 Claude 那样 compact 前 snapshot 规则状态.
- **`SubagentStart` / `SubagentStop` 不可 hook** — Codex 平台有 `enable_fanout` / `child_agents_md` feature flag (under development) 但没对应 hook event. pinrule 在 codex 下不能像 Claude Task tool 那样隔离子 Agent state.
- **`PermissionRequest` 不接入** (codex.py ADR-001, v0.10.3): pinrule 已在 PreToolUse 层用 `bypass_pinrule` / `testset` / `read_first` 覆盖危险操作拦截. PermissionRequest 二次拦增加假阳无新维度.
...
### pinrule — user_stop_hints 类 3「协作等候/暂停」
...
- 真捕获的 codex SessionStart payload (PR #4 证据):
  ...
  ```
- 字段跟 Claude SessionStart 完全兼容 — pinrule 通用 `session_start.py` 开箱即用, 不需归一化
- 小发现: Codex 不是 TUI 启动立刻 fire SessionStart, 是第一轮 prompt 之前 fire — 仍功能正确
- `_HOOK_EVENTS` 现在 5 个 event (原 4 个): `SessionStart / UserPromptSubmit / PreToolUse / PostToolUse / Stop`. Codex 0.130 支持 6 个; pinrule 用 5 个 (PermissionRequest 跳过, 没 pinrule 用例).

### exec_command → Bash 归一化 (任务 B)

Codex CLI 跑 shell 全走 `exec_command` tool 名. v0.10.1 只映射 `apply_patch → Edit`, 让 codex shell 调用对 pinrule Bash check (`bypass_pinrule` / `record_bash` / `is_long_task`) 不可见. v0.10.2:
...
**pinrule 历史上最大单次 onboarding 体验提升**. v0.10.0 文档说 Codex 0.130+ 要求每个 hook 必须在 TUI `/hooks` 命令手动审批. Codex CLI 的 PR #4 深入一层: 实现 `CodexBackend.trust_pinrule_hooks()` 复刻 Codex 自家 `trusted_hash` 推导算法, 装机时自动往 `~/.codex/config.toml` 写 `[hooks.state]` entry. 结果: **手动 `/hooks` 审批步骤消除了**.

**安全**: trust writer 只为 pinrule 自家 wrapper 生成 entry (`is_pinrule_entry` 验证 — 跟 uninstall 幂等用同 predicate). 非 pinrule hook (vibe-island bridge / 用户自定义 hook) 永远不碰. Codex 后续升级 hash 算法时, pinrule hash 会回落到 `/hooks` 显示 "modified" 而不是静默信任漂移.

`post_install_message()` 文案重写: 原 "⚠️ 关键 — 必须手动 /hooks 审批", 现 "Codex hook 状态 — pinrule 已写 trusted_hash, 复核可选". README codex alert box 从 "必须手动审批" 翻成 "自动信任, 立即生效".
...
| PreCompact / SubagentStart/Stop | ✅ | ❌ (codex hook API 没暴露对应 event) | pinrule 层不阻塞 — Codex 平台内部**有这些概念** (`enable_request_compression` stable=true 内部上下文压缩, `enable_fanout` / `child_agents_md` 是 sub-agent feature flag under development), 但 Codex 6 个 hook event (SessionStart/PreToolUse/PermissionRequest/PostToolUse/UserPromptSubmit/Stop) **没暴露** compact 或 sub-agent dispatch 作为 hookable lifecycle event. Codex 暴露后再接入. |
| PermissionRequest | N/A | 不用 | pinrule 无用例 |

### pinrule 维护者端配套 (本 commit)
...
- 确认通用 `pinrule/hooks/session_start.py` 处理 codex SessionStart payload 不需改 (字段名兼容)
...
v0.10.0 所有权分工现在跨 2 个连续 PR 验证. Codex CLI 贡献速度快 (真世界 session 捕获作证据, 全面测试覆盖, 甚至 bonus 像 auto-trust 超出明确 ask). pinrule 维护者角色稳: review 边界纪律 + 维护 GitHub 对外文档 + 通用层 backend 改动配套. **跨平台 AI Agent backend 协作模型验证**.

## [0.10.1] — 2026-05-16（patch — codex shell-as-Read 全链路打通 + 跨 backend 契约测试）

**首个 codex-owned PR 合并**: [#3](https://github.com/jhaizhou-ops/pinrule/pull/3) Codex CLI 自己提的 (`feat(codex-backend): detect shell reads from exec_command`). v0.10.0 所有权分工真起作用了 — codex 提的 PR 只动它能改的文件 (`pinrule/backends/codex.py` + `tests/test_codex_backend.py`), pinrule 维护者 review + 做 pinrule 端配套 (通用 `post_tool_use.py` 层消费 canonical `read_file_paths` 字段). 端到端 shell-as-Read 识别真工作: codex agent 跑 `tail -n 20 file.py` → pinrule 记成 Read → 后续 `apply_patch` 同文件不再被 `read_first` 假阳拦. **关掉 v0.9.16 期 codex 用户体验最后一个缺口**.
...
`pinrule/hooks/post_tool_use.py` 消费 canonical `tool_input["read_file_paths"]` 列表 — 对**任何** backend 都生效 (不是 codex 专用). 遍历每条 path 调 `state.record_read()`, 在 per-tool-name 分支之前跑. 设计 backend-neutral: 未来任何 backend (Cursor / Copilot / Cline 等) 的 `normalize_tool_input` 输出 `read_file_paths` 字段也自动生效.
...
PR #3 在 CI 跑 vulture --min-confidence 60 时报 `pinrule/backends/codex.py:144 unused attribute 'whitespace_split'` (本地 80 不报). 这是 stdlib `shlex.shlex.whitespace_split` attribute 被 vulture 误判. `whitelist.py` 加 `_shlex.shlex.whitespace_split` 引用让 CI 干净 (跟现有 `_signals.reset_cache` 同 pattern).
...
1. **Codex 拒绝 `permissionDecision:"allow"` shape** — v0.9.15 假设 Codex 接受 Claude `hookSpecificOutput.allow` shape, 2026-05-16 真测试 codex 0.130 报错 `unsupported permissionDecision:allow`. pinrule 错了 1 个 release.
2. **Codex shell-as-Read 适配缺口** — codex 没独立 `Read` tool, 用 `exec_command` 跑 `tail`/`sed`/`cat` 读文件. pinrule `record_read` 只认 `tool_name == "Read"` → codex shell 读全看不到 → `read_first` 假阳拦 codex 编辑.

用户反馈：「pinrule 的 hook 和判定的设计可能得针对不同的平台有针对性的开发和维护，你主要负责维护 pinrule 主程序和 claude 端，codex 端我让 codex 自行开发和测试」. 同意 — codex 协议细节归对 codex 平台变更信号最快的一方,就是 Codex CLI 自己.

### Major — 架构分工

pinrule 把 backend 所有权当**独立贡献者表面**:

| 文件 | Owner |
|---|---|
| `pinrule/hooks/*.py` 主逻辑 + `pinrule/checks/*.py` engine check + `pinrule/backends/_base.py` Protocol + `pinrule/backends/_json_hooks.py` 基类 + `pinrule/backends/protocol_adapter.py` 调度 + `pinrule/backends/claude_code.py` + `pinrule/backends/gemini_cli.py` | pinrule 维护者 |
| **`pinrule/backends/codex.py`** + **`tests/test_codex_backend.py`**（计划） | **Codex CLI 自己**（Codex session 发 PR） |
| `tests/test_protocol_adapter.py` 跨 backend 契约测试 | pinrule 维护者 |
| `README.md` / `CHANGELOG.md` / `HANDOFF.md` / `ARCHITECTURE.md` / `HOWTO.md` | pinrule 维护者 |

新文档 [`docs/CODEX_BACKEND.md`](docs/CODEX_BACKEND.md)（+ `.zh.md`）定义所有权边界、6 契约 method、Codex backend 已知 TODO 议程.

### Major — 6 契约 method 形式化

`pinrule/backends/_base.py:Backend` Protocol 形式化每个 backend 必须提供的方法. `_json_hooks.py` 提供 Claude 风格默认; backend 只 override 不同的部分:
...
之前 `pinrule/backends/protocol_adapter.py` 含 `_GEMINI_TOOL_MAP` / `_CODEX_TOOL_MAP` / `parse_apply_patch_envelope` / `_extract_codex_patch_text` / `normalize_tool_input` 等 backend 私货塞在「中立」文件. v0.10.0 把每条归到拥有该协议的 backend 文件:
...
- `parse_apply_patch_envelope` 从 `pinrule.backends.codex` re-export 给 v0.9.16 测试向后兼容

`detect_backend` 升级: 返回 canonical REGISTRY key (`claude-code` / `codex` / `gemini-cli`) 不再用 v0.9.15 简写 (`claude` / `gemini`). codex 通过 `sys.argv[0]` 路径含 `/.codex/` 识别 — 因为 codex hook payload 没可靠 backend signature 在 stdin, 但 wrapper 文件路径永远是 `~/.codex/hooks/pinrule_*.py`.
...
- `pinrule doctor` — codex-specific 段打印审批 reminder. Doctor 不能程序化验证审批状态 (codex 不暴露); 老实说而不是假装能检测.
...
2. **捕获真 hook-level payload** — 当前从 session rollout 推断, 没直接捕获. `/hooks` 审批后通过 `PINRULE_DEBUG_DUMP_PAYLOAD` dump 真 hook stdin.
...
**`pinrule/backends/protocol_adapter.py` 加 2 个函数**：
...
- `pinrule/checks/read_first.py`: 存在 `_codex_patch_files` 时遍历 — 任一 Update 路径没 Read 过都拦。Add 路径豁免（新建文件不需要 Read），Delete 跳过。**捕获多文件 patch 里只 Read 了主文件的情况。**

### 输入 shape 防御式处理

`_extract_codex_patch_text()` 同时处理裸字符串（rollout 真捕获验证）和可能的 dict-wrap 形式（`{"input": ...}` / `{"command": ...}` 等）— 因为 **hook 层** payload shape 没直接捕获到：`codex exec` non-interactive 模式即便加 `--enable hooks` 也不 fire 用户 hook（通过 `PINRULE_DEBUG_DUMP_PAYLOAD` 注入工具 + `codex features list` 双向验证）。交互式 codex（生产路径）正常 fire hook 是预期；防御式 wrap detection 让 pinrule 不管 codex hook 传哪种 shape 都能工作。

### Config DEFAULTS 静默丢字段 bug（Minor #4）

`pinrule/config.py:load()` 用 `for key in DEFAULTS` 合并用户配置 — 所以任何**不在** `DEFAULTS` 里的用户可调字段在 `~/.claude/pinrule/config.yaml` 里写了也被静默丢弃。`reinject_every_n_tokens` 在 `post_tool_use._build_smart_reinject` 里是文档化的可调字段，但漏在 `DEFAULTS` → 用户 `config.yaml` 写 `reinject_every_n_tokens: 4000` 实际被静默回退到「按模型自适应」默认。
...
- 全 6 道本地 gate 通过（pytest zh + en / ruff / mypy / vulture / wheel build+verify+smoke）
- Wheel smoke test：clean venv `pip install pinrule-0.9.16-py3-none-any.whl` → parser 正确从真 codex envelope 提取文件路径，`normalize_tool_name("apply_patch", ...)` 返回 `"Edit"`，全部 `data/signals/` 文件出货（v0.9.15 的 force-include 顺延）

完整 details: [CHANGELOG.md](https://github.com/jhaizhou-ops/pinrule/blob/v0.9.16/CHANGELOG.md)

## [0.9.15] — 2026-05-16（fix — cross-model audit (GPT-5.5) 抓到 3 个 cross-backend 协议 critical bug；pinrule 整个生命周期都假设 Claude-only 协议没被验证过）

### 为什么发这版

用户：「再来一轮 cross-audit，本机配置了 codex cli，也配置好了 gpt 5.5 模型，你委派 codex cli 做一次多 Agent 交叉评审。」

跑 `codex exec` GPT-5.5 xhigh reasoning audit pinrule。**cross-model 视角暴露了 Claude 端 audit 每轮都漏的 3 个 critical bug**：

1. **Gemini BeforeTool output shape 不兼容** — pinrule 一直输出 Claude 风格 `{hookSpecificOutput: {permissionDecision: "deny"}}`，但 [Gemini hooks docs](https://geminicli.com/docs/hooks/reference/) 要求顶层 `{decision: "deny" | "block", reason: ...}`（WebFetch 验证）。**影响**：Gemini 用户 pinrule 写 violation + stderr 警告但**危险 tool 真执行** — 所有拦截型规则（non_blocking / bypass_pinrule / read_first）只记录不真拦。

2. **Gemini tool_name 没归一化** — pinrule checks 用 Claude 风格 `Bash`/`Read`/`Edit`/`Write`/`NotebookEdit` 比较。Gemini 用 `run_shell_command` / `read_file` / `write_file` / `replace`。**影响**：Gemini 下每个 check 早 return `None` → **0 个 check 触发**。

3. **Codex `apply_patch` tool_name 不处理** — [Codex hooks docs](https://developers.openai.com/codex/hooks) 明确说编辑 hook input 真报 `tool_name: "apply_patch"`，不是 `Edit`/`Write`。pinrule 0 处处理。**影响**：Codex 用户用 `apply_patch`（主要编辑方式）会绕过 `read_first` / `long_term_fundamental` / `testset` / `evidence` checks。`last_edit_ts` 不推进 → 旧「测试通过」状态被错保留 → git commit `evidence.commit` 门被绕过。

本 session 用户拍我一次：「你没有探查就下结论这很不好。结合实际环境支持和官方文档开始排查、修复和测试吧。」之前打算直接 ask 用户拍 fix 方案，没真 verify。规则 6 read-before-write 也适用 doc — pulled `~/.codex/hooks.json` + `~/.gemini/settings.json` 真配置 + WebFetch 两家官方协议文档。这也 catch 了 codex audit 的一处误判（codex 以为 Codex 需要 legacy shape，文档实际说 Codex 同时接受新格式 — pinrule 现在 Codex output OK；只 `apply_patch` tool_name 漏了）。

### Fix — `pinrule/backends/protocol_adapter.py`（新模块）
...
Codex → Claude canonical:
  apply_patch → Edit  # 让 long_term/testset/bypass_pinrule 扫 tool_input.command 真触发
```
...
`apply_patch` Phase-2 limitation: `apply_patch` 编辑 diff 在 `tool_input.command` 不是单个 `file_path`。`long_term`/`testset`/`bypass_pinrule` 扫命令文本正确触发。但 `read_first`（需 `file_path` 比 `state.read_files`）跟 `record_edit`（单 path）目前在 `apply_patch` 上 no-op 因为没 `file_path`。多文件 diff parsing 留 **Phase 2**（让 `read_first` 真强制「patch 每个文件先读」+ `record_edit` 推每个 touched file 的 `last_edit_ts`）。adapter 模块 docstring 写清楚了。

### Critical wheel-打包 fix (第二次 codex full-repo review 抓到)

Phase 1 合并进 v0.9.15 后，用户要求再来一次 codex GPT-5.5 review — 这次评审**整个 pinrule 项目，不只 diff**。GPT-5.5 抓到一个**比 cross-backend bug 还严重的灾难性打包 bug**：

`pyproject.toml` `force-include` 列了单个 yaml 模板 + skills + locales — 但**从来没包含 `data/signals/`**。`pinrule/signals.py:40` 写死 `_REPO_ROOT / "data" / "signals"` 加载。源码树测试通过（本地 signals 目录存在），但**wheel 安装后整个 `data/signals/` 树丢失**。`compile_alternation()` 返回 never-match 正则 `(?!)` → **所有 pip 安装用户的 keyword-fallback layer 全静默失效**：`evidence` / `keep_pushing` / `non_blocking` checks 失去检测词表。

这影响**每个 pip 安装的 pinrule 用户，含 Claude Code 主流路径** — 比 cross-backend bug 更严重（cross-backend 只影响 Gemini/Codex 用户）。我自己 6 道本机门禁有 wheel verify 步骤但**只锁了 6 个 expected 文件**；`data/signals/` 子树从来没在 lockdown 列表里（规则 5 教训扩展：lockdown 只覆盖当时想到的，新加 data 子树会漏）。
...
**cross-model audit 价值是真的** — 当在地模型有系统性盲区时。Claude 写的 pinrule；Claude 本 session 已 review pinrule 12+ 轮；Claude 的盲区是：**假设 Claude 自己用的协议是通用的**。GPT-5.5 跑在 Codex CLI 上 — 不同训练 exposure 到 Gemini hooks 文档 — 拉了官方 ref 并精确指出这个假设。单模型轮次（v0.9.13 / v0.9.14）边际收益递减；cross-model 轮次打开了全新 audit surface。

这个 bug 在 pinrule 「3-backend 支持」声明的整个历史里都潜伏 — 每次 dogfooding 都是 Claude Code，所以 cross-backend 协议从来没真被测过。README 字面声称「Claude Code / Codex CLI / Gemini CLI」支持，但 Gemini 支持是 non-functional。诚实修正。
...
**fail-closed 是 pinrule 设计原则的反面** — 所有 hook 必须 fail-open（pinrule 自身内部失败永远不该卡用户）。
...
v0.9.12 收官后（v0.9.11 instrumentation bug + 元教训「规则 4 双向适用 — 验证结果不是 instrument artifact」），用户问「全面排查下，还有没有这种 bug，直接影响 pinrule 运行准确性和统计准确性的」。用 v0.9.12 bug pattern 作模板起综合 audit — Type A（字段缺失）/ Type B（聚合 off-by-one）/ Type C（race / load-modify-no-save）/ Type D（i18n 不一致）。
...
`pinrule/violations.py:370` 反序列化 `Violation()` 时加 `agent_id=d.get("agent_id")`。跟 `to_json()` 写路径对称（行 59-60）。audit/stats 视图能真按主/子 Agent 分组。

### Fix 2 — turn 窗口 cutoff: `cur - (window - 1)` 替代 `cur - window`

`pinrule/violations.py:309 recent_turns` + `pinrule/violations.py:343 count_recent_turns` + `pinrule/cli.py:836 cmd_audit` 漂移视图三处 cutoff 一致改。
...
修后 `cur - (window - 1)` 让 `window=N` 真匹配 N turn。子 Agent 报为「中等严重性统计偏差」— 真语义是「force_block 触发条件不准影响 pinrule 干预行为准确性」。
...
`pinrule/hooks/pre_tool_use.py:98-100` 之前 `state = session_state.load(...); state.catchup_pending_bg()` **不 save**。跟 v0.9.8 `update_state` 架构不一致 — 其他所有 hook 写路径用 `update_state` 原子 load-modify-save。子 Agent 报告捕到我早期判断错（v0.9.8 时 read 这块判 design choice「PreToolUse 是决策端不是 state 端」）。但 `catchup_pending_bg()` 改 `pending_bg_tasks` 跟 `recent_bash` — 不持久化让下次 hook 重复 catchup 同一 task。改用 `session_state.update_state(session_id, lambda s: s.catchup_pending_bg(), agent_id=agent_id)` 跟 post_tool_use.py 一致。
...
v0.9.11 发完 `pinrule audit --by-check`。作者本机 first-run 跑出一个抓眼球数字：**「86% violation 走 keyword-only 兜底，只 14% 来自 engine check。」** 我把这数字读成真行为信号 + 给用户解读「多数规则没附 violation_checks，engine 层需要更多投入」。

**解读是错的。** 用户问了关键 follow-up：「只 1 次触发的 `bypass_pinrule` / `evidence.completion` / `testset` 是规则设计冗余，还是该监控的没监控到？」深挖这个问题逼我去 read 真 jsonl —— 发现两条 violation 的 `trigger` 字面完全一样（都是 `check.keep_pushing.default.trigger` 的 i18n 输出），但一条有 `trigger_key` 字段、一条没有。**纯字段缺失差异，背后是同一 engine 命中信号。**

根因：`pinrule/hooks/user_prompt_submit.py:_build_strong_reminder`（v0.4.41 加的 fallback 路径 — 用户立刻接 prompt 时 stop hook 来不及跑，这条路径补写 violations）构造 `Violation` 时**漏传了 trigger_key**，而 `pre_tool_use.py` 跟 `stop.py` 两条路径都正确传了。所以经过这条 fallback 路径写入的 engine 命中 `trigger_key` 字段都是空的，v0.9.11 的 `--by-check` 视图就把它们错归到 "keyword-only" 桶。
...
静态扫 `pinrule/hooks/*.py`：每个 `Violation(...)` 或 `_V(...)` 构造调用如果含 `rule_id=...`，必须同时含 `trigger_key=...`。未来 PR 加新 hook 路径写 violation 漏传 `trigger_key` → CI 直接红。**不变量在测试套件里，不在 code review 记忆里**。
...
- 原报告 `bypass_pinrule` engine 1×。keyword-only 桶有 6 条「绕开检测 — 手动写 pinrule 内部状态」字面（`bypass_pinrule` check 的 i18n trigger）。**真 `bypass_pinrule` engine 命中估计 ~7 次**。
...
## [0.9.11] — 2026-05-16（feat — 可观察性：`pinrule audit --by-check` engine check 命中分布 + `/pinrule` 无参数默认展示这个视图）

### 为什么发这版

v0.9.10 onboarding 打磨收官后，问用户下一波推哪个方向：check 命中分布可观察性还是周报趋势可视化。用户的设计洞见：

> 「skill 的增加会造成额外的用户使用成本，我想尽可能压缩。第一个方向是不是直接做成 /pinrule 指令不带内容时候的默认输出就比较好？也就是用户输入 /pinrule 就直接输出 check 命中分布」

这避免了引入新 entry point（新 CLI 子命令或新 slash command）。`/pinrule` 是用户已知的（v0.9.10 footer 刚教过）。no-arg `/pinrule` 给个有用的默认行为复用已有 muscle memory — zero learning curve。

### 实施

**1. CLI 后端 — `pinrule audit --by-check`**（`pinrule/cli.py`）：

新 `_cmd_audit_by_check()` 按 violation 的 `trigger_key` 字段（i18n locale key，格式 `check.<name>[.<sub>].trigger`）聚合：

- **top-level 聚合**（8 个 engine check）：每个 check 函数一行，含命中次数 + 占 engine 总命中的比例（`bypass_pinrule` / `chinese_plain` / `evidence` / `keep_pushing` / ...）
...
**2. Skill — `/pinrule` no-arg 默认行为**（`skills/pinrule/SKILL.md`）：

`pinrule` skill 加 "No-argument flow" 段：用户输 `/pinrule` 不带 `$ARGUMENTS` 时 Agent 跑 `pinrule audit --by-check` 转述给用户，附简要解读（高命中 check → 哪条方向违反最多；高 keyword-only 占比 → 多数 violation 走兜底层；高假阳嫌疑 → 哪个 sub-variant 字面 pattern 可能过宽）。然后问：「想根据这数据调哪条规则，还是加一条新规则？」

这闭合 dogfood 反馈闭环：violations.jsonl → audit → 用户看到 pattern → 决定调整。**没发明新 entry point**；`/pinrule` no-arg 是「告诉我现在啥情况」的自然手势。

**3. 向后兼容**：

`pinrule audit`（不带 `--by-check`）保持现有行为（按规则聚合 + 假阳嫌疑标记 + fix 时间线 + 当前 session 漂移段）。`--by-check` flag 纯加项。

### 测试覆盖

`tests/test_cli.py` 加 2 个 case：
- `test_audit_by_check_aggregates_engine_hits` — 构造 6 条违反（3 条 `bypass_pinrule` engine + 2 条 `keep_pushing` sub-variant + 1 条 keyword-only），验证 top-level + sub-variant + keyword-only 段都出现
...
- 6 道本机门禁全过
- 真 dogfood 验证：本机 `pinrule audit --by-check` 直接跑出上面 187 条违反的真实分布

## [0.9.10] — 2026-05-16（feat — onboarding 打磨：summary 改首段不再砍半句 + 加 footer「3% token 上限 + `/pinrule` 入口」）
...
2. **少了 token 成本安心 + 加规则的 in-chat 入口提示** — 用户原话「希望加一句用户体验相关的补充：经测试，以上规则注入仅占 pinrule 每 session 会话 token 消耗总量的 3% 以内，请放心使用，体验下 Agent 长任务不飘逸的爽感。希望增改规则直接输入 /pinrule <自然语言你想增加的规则> 即可。」
...
### Fix 2 — 双语 footer 加 token 安心 + `/pinrule` 入口
...
**为什么 `/pinrule` 不违反 v0.9.9「不加指令 tip」原则**：`/pinrule` 是客户端对话框里输的 slash command（Claude Code / Codex / Gemini），不是 shell 命令需要用户开 terminal 跑。它是「自然语言录规则」skill 的 trigger — 输 `/pinrule <意图>` 等于「跟 Agent 说你想要什么规则」。这是 in-chat 协作的延续，不是「去 shell 跑这个」的 friction。

footer 走 `_resolve_locale()`（PINRULE_LOCALE env > config.yaml > `is_chinese_user()` system detect）— 中文系统用户自动看到中文 footer，英文系统用户自动看到英文 footer。

### 测试覆盖

`tests/test_cli.py` 加 2 个 case：
- `test_init_summary_footer_includes_token_cost_and_slash_pinrule` — 验证 footer 含 `3%` + `/pinrule` 字面
- `test_init_summary_footer_matches_user_locale` — **lockdown**：`PINRULE_LOCALE=zh` 时 footer 只出中文不漏英文；`PINRULE_LOCALE=en` 时只出英文不漏中文

更新 `test_init_summary_does_not_include_command_tips` 注释明确 `/pinrule <自然语言>` 是允许的（chat 里的 slash command，非 shell 命令）。
...
## [0.9.9] — 2026-05-16（feat — onboarding 改进：`pinrule init` 末尾展示默认启用规则简要列表，让 Agent 代装时能直接告知用户）
...
Agent 协助安装流程（README 顶部「让 AI 客户端帮你装」那段）走到 `pinrule install-hooks` 结束就完了。Agent 本身不知道装完启用了哪些规则 — 要么 (a) 让用户自己跑 `pinrule rule list` 看（违反「不让用户手动输指令」原则），要么 (b) Agent 自己去 read `rules.yaml`（额外工作 + 装机脚本之外的协议）。

### Fix — `pinrule init` 末尾加默认规则简要列表
...
Agent 跑 `pinrule init` 看到这段 stdout 自然 paraphrase 给用户 — onboarding 需求达成，**用户不需要手动输任何指令**。

### 设计取舍 — 刻意不加「下一步指令」清单

第一版实施带了「下一步:」段含 `pinrule rule edit / list / remove` 命令 tip。用户反馈：「我可能没说清楚，我不想让用户手动输一条指令。」删掉 tip 段。

原则：**Agent 转述完规则列表后，用户想改规则就跟 Agent 说「帮我去掉规则 X」「改下规则 Y」 — Agent 知道用 `/pinrule` skill 或 `pinrule rule edit`**。不需要给用户命令语法。
...
- `test_init_summary_does_not_include_command_tips` — 锁「不含指令 tip」不变量；下次有人重新加「下一步:」/ `pinrule rule edit` 字面到 summary 段 CI 直接红
...
第一遍方案是 expose `state_lock(sid)` contextmanager 让 6 个 hook 手动套 `with state_lock(...)`。用户拦下来：**「咱们要做长期方案，你忘了么？为什么 pinrule 没制止你走短期路线？」** — 我之前自己说过高阶函数方案 (B) 是「长期对的解」但用「v0.9.8 务实，留 v0.10/v1 走 B」搪塞自己选了 (A)。pinrule 自身检测层抓不到这层 framing（pure engineering 零 LLM 原则，design-intent 短期化抓不到字面 trigger） — **人工监督是兜底**，用户拦下来正是这个机制生效。
...
| 位置 | 改动 |
|---|---|
| `pinrule/session_state.py` | 加 `_state_lock`（fcntl.flock advisory lock，Windows no-op fallback）+ `update_state` + `read_state`。module docstring 写清楚 API 分层政策。 |
| `pinrule/hooks/post_tool_use.py:main` | 整段 modify + `_build_smart_reinject` 包进 fn；fn 返回 `additional_context` 给 stdout 输出。 |
| `pinrule/hooks/user_prompt_submit.py:_advance_turn_state` | catchup + turn++ + stop_block reset + model 探测全包 fn。 |
| `pinrule/hooks/session_start.py` | model 写入用 `update_state`。 |
| `pinrule/hooks/subagent_start.py` | 2 个独立 `update_state` 调用（主 state 模型队列 pop + 子 state 模型写 — 不同 lock key 互不阻塞）。 |
| `pinrule/hooks/pre_tool_use.py` | 段 1（Agent 模型入队）走 `update_state`。段 2（catchup-no-save）保留不动 — 已有设计「PreToolUse 是决策端不是 state 端，真正 catchup 在 PostToolUse / Stop」。 |
| `pinrule/hooks/stop.py` | `_handle_force_block` + `_handle_keep_pushing_block` 的 `stop_block_count += 1` 用 `update_state`。 |
| `pinrule/cli.py` | 2 处只读调用方（`stats` / `doctor` 视图）迁 `read_state` 让 API 意图真落地。 |
...
v0.9.2 → v0.9.7 修的是 CI 门禁 + i18n 残留 + user-facing 字符串一致性。v0.9.8 修的是**功能正确性 bug** — 影响每个多进程 pinrule 用户 — 而且修法是**把不变量编码进 API 形状**而非调用约定。这才是「long-term-fundamental」在代码里真长成的样子，不只是嘴上说说。

## [0.9.7] — 2026-05-15（fix — PINRULE_HOME 隔离 mode 下 bypass 检测失效 + v0.6.0 user-facing sticky 残留 + 加 regression 锁机制）
...
### Fix 1 — `bypass_pinrule` 检测在 `PINRULE_HOME` 隔离 mode 下失效

`pinrule/paths.py:pinrule_home()` 一直支持 `PINRULE_HOME` env override（跨用户 / dry-run / CI / 多 profile）。但 `pinrule/checks/bypass_pinrule.py:_PINRULE_STATE_PATH_RE` 硬编码 `\.claude/pinrule/...` 字面正则。后果：用户跑 `PINRULE_HOME=/tmp/foo pinrule ...` 然后 `rm /tmp/foo/session-state/*.json`（绕开尝试）— bypass-pinrule 检测**完全打不到**，正则只 match 默认 `~/.claude/pinrule/` 路径。

这跟 v0.9.6 CI verify step 是同一类 bug — 该用工厂函数的地方用了硬编码字面。单一来源原则在仓库一个角落破了。

**fix**：`_build_state_path_re()` 工厂函数按 `pinrule_home()` 动态构造正则 — 覆盖默认 mode / `PINRULE_HOME` override mode / home 子目录 mode（用户可能敲 `~/<rel>` 字面）。同时把文件名集合从 `(session-state|violations|sticky.yaml)` 扩成 `(session-state|violations|rules.yaml|sticky.yaml)` — v0.6.0+ 主名跟旧用户迁移期路径都拦。

### Fix 2 — `pinrule/cli.py:257` 硬编码提示骗 `PINRULE_HOME` 用户

`print("编辑用: ... vim ~/.claude/pinrule/config.yaml")` — 但实际创建在 `PINRULE_DIR / "config.yaml"`。`PINRULE_HOME=/tmp/foo` 时用户被指向一个不存在的文件。fix：`print(f"... vim {config_path}")` — 变量本来就在作用域里。
...
- `data/config.example.yaml:13,16` — config 模板注释（`pinrule init` 复制到 `~/.claude/pinrule/config.yaml` 用户会读）
...
### Fix 5 — `pinrule/violations.py` API contract docstring 还说 `sticky_id`
...
### 新加测试 — `tests/test_bypass_pinrule.py` PINRULE_HOME 隔离覆盖

4 个新 case：
- 默认 mode：`~/.claude/pinrule/*` / 绝对 home 路径 / 相对 fragment 都 match
- `PINRULE_HOME` override mode：override 路径下 bypass 写也 match
- `PINRULE_HOME` 在 home 子目录：用户敲 `~/<rel>` 字面也 match
...
v0.9.4 push: `mypy` 绿、`vulture` 绿、`ruff` 绿 — 但 **`pytest` 16 个测试红**。根因：测试 fixture assert 中文字面（`"默契"` / `"偏离"` / `"纯陈述"`）。我本机 `LANG=zh_CN.UTF-8` 让 `pinrule.locale_detect.is_chinese_user()` 返回 True → i18n 选 zh → fixture 通过。CI runner 默认 `en_US.UTF-8` → is_chinese_user 返回 False → i18n 选 en → 16 个 fixture fail。
...
v0.9.3 push 后（修了 vulture 阈值不匹配）CI **仍红**。第 3 个独立根因：`pinrule/signals.py:116` 的 `mypy` error，v0.8.1 加 yaml loader 时引入：
...
我从来不在本机跑 `mypy`。本机 release 前「质量门禁」只跑 `pytest + ruff`。CI 跑 `pytest + ruff + mypy pinrule/ + mypy tests/ + vulture --min-conf 60`。本机是 CI 严格子集 → 4 项 check 中有 2 项可以本机绿 CI 红。
...
CI 跑 `vulture pinrule/ --min-confidence 60`，但我本机跑 `--min-confidence 70`。60 confidence 阈值找到 5 处我本机看不到的「死代码」：

| 文件 / 行号 | 项 | 判定 |
|---|---|---|
| `pinrule/cli.py:67-68` | `EXAMPLE_RULES` / `EXAMPLE_RULES_MINIMAL` 别名 | **真死** — 0 调用者，删 |
| `pinrule/i18n.py:99` | `current_locale()`（docstring 说「for diagnostics」）| **真死** — 0 调用者，删 |
| `pinrule/i18n.py:104` | `reset_cache()`（docstring 说「for tests / config-reload」）| **真死** — 0 调用者，删 |
| `pinrule/signals.py:205` | `reset_cache()` | **vulture 假阳** — `tests/test_signals.py` import 用了（vulture 只扫 `pinrule/` 看不到 test 调用）|

### Fix

- 删 4 个真死代码
- 加 `whitelist.py` 引用 `pinrule.signals.reset_cache` 让 vulture 看到「被用」
- 改 `.github/workflows/ci.yml`：`vulture pinrule/ whitelist.py --min-confidence 60`
...
- 本机 460/460 通过
- 本机跑 CI 同款命令 `vulture pinrule/ whitelist.py --min-confidence 60` → exit 0
- `ruff` 干净
- 这次 push 的 CI 应该终于绿

## [0.9.2] — 2026-05-15（fix — `test_compact_hooks.py` 硬编码 `/Users/jhz/pinrule` 路径 → 动态解析；issue #2 来自 @fyn1320068837-source）

### 真实用户 bug 报告（同一外部贡献者的第 2 个 issue）

@fyn1320068837-source 提了 issue #2：`tests/test_compact_hooks.py` 共 **20 处硬编码 `/Users/jhz/pinrule`**（维护者本机路径），跨全部 9 个测试函数。结果：本机跑通过，但任何其他机器（含 GitHub Actions CI）都 `FileNotFoundError: '/Users/jhz/pinrule'`。
...
然后：
- 所有 `"/Users/jhz/pinrule/.venv/bin/python"` → `PYTHON`
- 所有 `cwd="/Users/jhz/pinrule"` → `cwd=PROJECT_ROOT`
...
- **`pinrule/hooks/session_start.py`**：docstring 描述方向反了（说「UserPromptSubmit 每 turn 全量, SessionStart 一次精简」）— 跟 v0.9.0 实际相反。重写匹配 v0.9.0 架构
...
`pinrule/rule.py` 加 `format_anchor_only(rule_list, recent_violations)` 渲染精简文本：`id + 第一行 preference + 偏离回顾标记`。UserPromptSubmit 每 turn 用。`format_for_injection()`（全量）仍被 SessionStart + PostToolUse 中段 reinject 用。
...
- `pinrule/rule.py:format_for_injection` 内有个 function-level `from pinrule.i18n import tr`。验证 `pinrule.i18n` 是 leaf module（无 `pinrule.*` import）— 安全上提到 module 顶部。减少函数体噪音 + 跟 module 顶部 import 惯例一致。
- `pinrule/checks/chinese_plain.py` 有个 inline magic number `< 30`（jargon 后括号闭合的最大距离）。抽成命名常量 `_JARGON_PAREN_MAX_DIST = 30`，跟已有的 `_JARGON_CONTEXT_RADIUS = 30` 并排 — 都是 module 级常量，都有解释。

### 审过但故意不改的

- cli.py 有 ~10 处 function-level `from pinrule.* import ...`。多数可安全上提（验证过无循环 import 风险），但有几处是测试 mock 友好性（如 `cmd_reset_session` 延迟 import `DEFAULT_DIR as SS_DIR` 让 `monkeypatch.setattr(pinrule.session_state, 'DEFAULT_DIR', ...)` 能拿到 patch 后的值）。大规模上提净收益小（~3 行净减），逐个分析「真 mock 友好」vs「冗余」会在边际收益上耗 review 时间。
...
`pinrule/checks/__init__.py:run_checks()` 有个 `sticky_id: str = ""` 参数，自己内联注释写「v0.5.0 deprecated alias, removed in v0.6.0」— 没真删。0 调用者传过这参数（grep 实证）。删掉参数 + 引用它的 `rule_id=rule_id or sticky_id` 兼容垫层。函数签名现在干净为 `rule_id: str = ""`。

这是 v0.8.2 抓的 3 个死代码（`PINRULE_RULE_SKILL_SRC` / `_claude_skills_dir` / `_install_pinrule_rule_skill`）同款 pattern — 注释说「v0.6.0 移除」但没真删。v0.8.4 抓到上一轮手工 grep 漏的第 4 处。

### 没改的

- CHANGELOG / HANDOFF 历史 entry 里「6 信号」字眼 — 那描述的是当时 release 的状态，归档完整性保留（rule 5）
- README「历史版本」banner 提的 v0.6.0 `pinrule.sticky` 移除 — 合理的迁移指引给 pre-v0.6 用户
...
`cli.py` 有 3 处函数体内重复 `from pinrule.rule import ... load as load_rules`（module 顶部已 import `load`）。还有 1 处 `from pinrule.violations import load_all as _load_v` shadow 别名。4 处全清：

- module 顶部改用 `from pinrule.rule import load as load_rules` + `format_for_injection`
- 函数内重复 import 删
...
- `PINRULE_RULE_SKILL_SRC` 在 `cli.py` — v0.5.x deprecated alias，注释自己写「removed in v0.6.0」但没删。0 外部使用
- `_claude_skills_dir()` 在 `cli.py` — docstring 自己写「v0.5.16 deprecated, v0.6.0 移除」但留着。0 外部使用
- `_install_pinrule_rule_skill()` 在 `cli.py` — 同款 v0.6.0 移除自述，0 调用者
...
- **模块级常量**：`STICKY_PATH`（`pinrule.rule.DEFAULT_PATH` 的 alias）→ `RULES_PATH`。cli.py 10 处 + test_cli.py 8 处
- **`pinrule doctor` 输出**：`"sticky.yaml: <path>"` 显示的实际是 `rules.yaml` 的路径 — 名跟内容打架。改成 `"rules.yaml: <path>"`。`"sticky 加载: ✓"` → `"规则加载: ✓"`
- **`pinrule audit` 输出**：表头 `'sticky_id'` → `'rule_id'`；「未触发的 sticky」段标题 → 「未触发的规则」
- **`pinrule violations clear` 输出**：filter 描述 `"sticky={id}"` → `"rule={id}"`（CLI flag `--sticky` 保留作向后兼容，跟过去 deprecation 节奏一致）
- **`pinrule rule list` 输出**：`"pinrule sticky (N/M)"` → `"pinrule 规则 (N/M)"`；局部变量 `sticky = load()` → `rules = load()`
- **Hook stderr 输出**：`pre_compact.py` / `session_start.py` / `subagent_start.py` 报错时都打印 `"sticky 加载失败"` → 改成 `"规则加载失败"`；局部变量 `sticky_list` → `rule_list`
- **`cli.py` 顶部 docstring**：删过时 `pinrule sticky <...>` 条目（命令的友好提示逻辑 L1252 还在兜底老调用）
...
`pinrule audit` 跟 `pinrule doctor` 输出是新用户「不对劲」时第一眼看到的东西。混着「sticky」/「rule」命名给人「这项目自己没跟上自己」的感觉 — 这正是 rule 9 doc-sync 纪律想防止的印象。v0.8.2 让用户面输出跟 v0.6.0 BREAKING 后的实际状态一致。
...
v0.8.0 之前，pinrule 的检测 regex（`_USER_STOP_HINT_RE` / `_AGENT_SATURATION_RE` / `_STOP_HINT_RE` / `_EXPLICIT_USER_HANDOFF_RE` / `_WEAK_CLAIM_RE`）字眼全是中文硬编码在 Python 源码里。英文用户能装 pinrule 但 `keep_pushing` 反思 hook 经常假阳 — Agent 说「Next I'll proceed to X」不被识别为推进信号、用户说「looks good / LGTM」不豁免反思、`evidence` 漏拦「should work / probably fine」类弱声明。

用户的洞察精准：**是不是工程模块全英文就行，反正 LLM 能看懂，人类也不看工程模块。** 对 pinrule 自身源码而言基本对，但 **regex 字面**匹配的是用户/Agent 实际对话语言，用什么语言取决于用户自己。所以优雅方案是把信号字眼从代码彻底剥出来，按语言分文件维护。
...
- 一行一字眼，`#` 注释 + 空行跳过
- `pinrule/signals.py` 加载某信号目录下所有语言文件，去重 + union + 编译成单 regex（长字眼优先，避免「OK」抢「OK 了」前面命中）
- 不同语言字符集不重叠（中文 vs 拉丁 vs 假名 vs 谚文）→ 天然无跨语言误命中
- LRU 缓存；字眼文件每进程加载一次

### 加新语言 = 0 Python 代码

日语 / 韩语 / 俄语 / 德语母语用户只需给每个 signal 目录贡献一个 `data/signals/<signal>/xx.txt`，pinrule 下次启动自动接进来。不需要 regex 编排技巧 — 只写「实际用户会说的话」即可。
...
pinrule「永不依赖 LLM」边界在这版更扎实 — i18n 用纯数据文件 + regex 就能做，根本不需要 LLM 在循环里。让 pinrule 快（< 60ms）的同一原则也让它 locale 可扩展，零认知成本。
...
按 rule #7（pinrule 假阳时治根）：trigger 在当前 regex 下命中是正确的，但 regex 漏了一整个语义类别。

### Fix — 扩 `_USER_STOP_HINT_RE` 加满意确认类

在 `pinrule/checks/keep_pushing.py` 加第二类停下字眼：
...
pinrule 用户叫停豁免存在的根本原因是**用户已经表态停下时不再挡道**。漏掉「满意」类意味着 hook 在用户已宣告停下后还继续催 Agent 推 — 这正是 pinrule 应该**防止**的，不是 pinrule 应该**产生**的。
...
- 砍掉已过期的 M0 milestone 段、过时的「Strict LLM authorization v1+」段（pinrule 是坚定不依赖 LLM，不是「v0 不用 LLM」）
- 文档标题从「pinrule v2」改回「pinrule」— v2 框架是 v1 归档时期的产物，不再相关
...
- `ARCHITECTURE.zh.md`：全文清 `sticky.yaml` → `rules.yaml` + `pinrule sticky list/edit/remove` → `pinrule rule …`（这些是 v0.6.0 漏网的）；注入头部文本同步到当前「[pinrule — 你跟用户的长期默契]」合作默契语气；性能数字 < 50ms → < 60ms（匹配实测）
...
- `HANDOFF.zh.md`：同名改 — 标题从「M3 六波结束」改「pinrule 内部接力文档」；当前版本行更新到 v0.7.2
...
- `pinrule/backends/HOWTO.md/.zh.md`：把内部 cross-reference `[pinrule rule #1 long-term fundamental]` 改成自然话术指向规则 slug
...
这一版是 rule 9（docs-sync-after-commit）的补课 — 按「第一次看 pinrule 的读者会觉得这是爆款级项目还是 fragmentary 项目」的标准过一遍。营销话术和老命令名都是「不够认真」的信号，去掉以后项目读起来更诚实，不是更弱。
...
v0.7.0 + v0.7.1 治根（重写 ~640 处 mimicry 跨规则模板 + locale + 文档）后，`pinrule audit` 数据确认 Check 3 在本 session 168 条 violation 里 **0 次触发**。源头清了，reactive 监控冗余。

这就是用户 v0.7.0 对 `defensive_prefix_stacking` 用过的同款逻辑：**「这显然是你对 pinrule 的应激反应，咱们要治根不要治表」**。v0.7.0 在加这个 check 前撤了；v0.7.2 撤掉三个月前同款思路漏掉的 Check 3。

### 删除

- `pinrule/checks/chinese_plain.py`：`_check_repeated_prefix()` 函数 + `_PREFIX_REPEAT_THRESHOLD` 常量 + `check()` 里 Check 3 调用（~45 行）
...
- `ruff`：0 issue
- `pinrule audit` chinese-plain 分解：Check 1（中文占比）+ Check 2（jargon）仍覆盖所有真实 case；没丢任何 Check 3 触发场景

### 价值

pinrule 核心哲学是**治根不治表**。reactive 监控容易作为「工程层兜底」对冲堆积，治根后还留着。v0.7.2 闭环了 v0.7.0 用户指令：源重写做了，对冲的 reactive 监控也能撤。
...
- Phase 8：pinrule 规则源文件 + check 注释（in-context mimicry 源头层）
- Phase 9-10：零散残留
...
- 全部 `pinrule/**/*.py` 源码注释（v0.7.0 之前 deferred）
...
## [0.7.0] — 2026-05-15（refactor — 治根：改写 pinrule 源规则文本里「真X」防御性前缀堆叠）

### 用户识别的原因

用户抓到一个实际架构失败模式：我（pinrule 监督下的 Agent）反复堆「真X」前缀（「原因 / 违反 / 任务饱和 / 实测」）作为防御性证据语言。用户诊断很犀利 — 加 `defensive_prefix_stacking` check 函数是**治表**，留下 **mimicry 源头**不动。

源头：pinrule 自己规则文本和 locale 字符串通篇用「真X」模式（如 `rules.dev.example.zh.yaml` 里「想清楚是违反 / 修原因」、`data/locales/zh.yaml` 反思提示「任务饱和」）。LLM 每个 turn 读 pinrule 头部，在响应里复制前缀风格 — in-context mimicry 把规则文本本身的语言模式复制到 Agent 表达上。
...
- 内部文档（中文）：`docs/PRD.zh.md`、`docs/ARCHITECTURE.zh.md`、`docs/V0_6_0_PLAN.zh.md`、`docs/REFACTOR_PLAN_RULE_AND_I18N.zh.md`、`docs/RULES_REDESIGN_PROPOSAL.zh.md`、`pinrule/backends/HOWTO.zh.md`

### 没做的事（正确性克制）

- **没加 `defensive_prefix_stacking` 工程层 check** — 一开始动手了，被用户「这是应激反应，要治根不治表」点醒后撤销。反应式监控只能事后捕捉 Agent 症状，留 pinrule 自己引发的 mimicry 源头不动。正确 fix 在源头文本层
- **没动 `pinrule/*.py` 源代码注释**（~200 处）— 这些不进 Agent prompt context，不驱动 mimicry。低优清理留 v0.7.1+
...
`pinrule/session_state.py` 加 `_NON_CODE_EDIT_RE` 豁免清单 — `record_edit()` 在 file 是文档 / 元数据 / 顶级仓库文本时不推 `last_edit_ts`：
...
pinrule 第一个外部贡献者 `@fyn1320068837-source` 报了他在 `henghai-backend` 工作流踩到的 bug — `docker exec container python -m pytest` + edit + commit。他最初的根因诊断（「regex 不 match docker 前缀」）是错的，但 **bug 本身是真的**。maintainer 本机端到端 docker pytest 实测在候选 A 场景（`last_edit_ts > last_test_pass_ts` 在非代码 edit 后）复现。v0.6.1 在正确层修原因。
...
- **`pinrule.sticky` 模块** — `from pinrule.sticky import ...` 现在抛 `ModuleNotFoundError`。迁移：`from pinrule.rule import ...`（exports 完全一致）。
...
- **`pinrule sticky <subcommand>` CLI** — 退 1 带提示 `💡 你是不是想用 pinrule rule？`。迁移：用 `pinrule rule list / edit / remove / add / preview`。
- **`pinrule.rule` aliases** — `Sticky` / `MAX_STICKY` / `StickyConfigError` 删了。迁移：`Rule` / `MAX_RULES` / `RuleConfigError`。
- **`pinrule.cli` aliases** — `EXAMPLE_STICKY` / `EXAMPLE_STICKY_MINIMAL` 删了（内部符号，对用户基本无影响）。

### 保留（盘上数据兼容永久保留）

这些不是废弃 alias，是处理用户盘上数据的兼容补丁，pinrule 永远保留：

- **`sticky.yaml` → `rules.yaml` 自动迁移** 在 `pinrule init` — 从 v0.4.x 升级的用户盘上仍有 `sticky.yaml`；pinrule 静默移到 `rules.yaml` 并备份 `.bak`
- **`violations.jsonl` `sticky_id` 字段兜底** — v0.4.x 历史 jsonl 行用 `sticky_id` 不是 `rule_id`；`pinrule audit` / `stats` 通过 `_extract_rule_id` 仍能正确读
- **`STICKY_PATH` 内部常量** in `pinrule.cli` — 向后兼容路径 alias 指向 `rule.DEFAULT_PATH`。测试在用；无需迁移
...
pinrule 自己代码 v0.5.13 起停用 `.sticky_id` 属性访问，v0.5.15 起停用 `from pinrule.sticky` import。v0.6.0 是**纯删除 commit** — 无 refactor 逻辑，只是删除。

### 用户脚本迁移指南

绝大部分用 pinrule 的用户脚本是 1 行机械替换：
...
- `tests/test_sticky.py` 新增 5 个 deletion-lock 测试（`test_v0600_*`）：
  - `import pinrule.sticky` 抛 `ModuleNotFoundError` ✓
  - `Violation.sticky_id` 抛 `AttributeError` ✓
  - `CheckHit.sticky_id` 抛 `AttributeError` ✓
  - `pinrule.rule.Sticky` / `MAX_STICKY` / `StickyConfigError` `hasattr() == False` ✓
  - `pinrule sticky list` subprocess 退 1 + stderr 含 `"pinrule rule"` ✓
...
| ② 功能放主语 version 放从句 | ✅ README hero / `/pinrule` section / PRD F5 都做到；ARCHITECTURE milestone 表是 patch 体（按格式可接受 — milestone 表本质就是时间线） |
...
## [0.5.18] — 2026-05-15（fix — `bypass_pinrule` 区分「读 pinrule 写别处」vs「写到 pinrule 路径」）

### dogfooding 假阳触发的根因 fix

正在看 `pinrule audit` 今天累积的违反数据时，跑 `grep deep-fix ~/.claude/pinrule/violations.jsonl > /tmp/df_audit.jsonl` 想提取几行分析 — 被 bypass_pinrule 拦「绕开检测 — 手动写 pinrule 内部状态」。按规则 7 没绕，深挖根因。

**之前的问题**：`bypass_pinrule` 老判定是 `(has_internal OR has_state_path) AND has_write` — 命令含 pinrule 路径 + 任何 redirect/write op 就拦，不管 redirect target 是不是 `/tmp/`。读 pinrule 状态到 tmp 分析是合法 audit 用途，但 rule 把「pinrule 路径出现在命令里」跟「写到 pinrule 路径」混为一谈。

**修法**：通过 `_BASH_REDIR_TARGET_RE`（v0.5.9 起放在 `description_context.py` 共享）提取 redirect target，看任一 target 是否匹配 `_PINRULE_STATE_PATH_RE`。新规则: `(has_internal OR has_state_path) AND write_to_pinrule_state`，其中 `write_to_pinrule_state = has_python_write OR (任一 redirect target 是 pinrule 路径)`。

**行为对比**（4 个新回归测试验证）：

| 命令 | v0.5.17 | v0.5.18 |
|---|---|---|
| `grep ~/.claude/pinrule/violations.jsonl > /tmp/x` | ❌ 拦 (假阳) | ✓ 豁免 |
| `cat ~/.claude/pinrule/violations.jsonl \| python3 -m json.tool > /tmp/pretty.json` | ❌ 拦 | ✓ 豁免 |
| `echo '{}' >> ~/.claude/pinrule/violations.jsonl` | ✓ 拦 | ✓ 拦（写 pinrule）|
| `python -c "open('.claude/pinrule/x', 'w').write(...)"` | ✓ 拦 | ✓ 拦（python 写接口）|
| `echo 'last_test_pass_ts=999' > /tmp/inject.txt` | ✓ 拦 | ✓ 豁免（target 是 /tmp 不是 pinrule 路径 — 跟 state_path 维度对称）|

`has_internal`（字段名引用）维度也对称收紧：写 `last_test_pass_ts=...` 到 `/tmp/` 不影响 pinrule 状态，现在豁免。同样字符串写到 `~/.claude/pinrule/...` 仍拦因为 redirect target 是 pinrule 路径。

### 为啥这事重要

这是 pinrule 自己 false-positive 拦合法的 audit 工作 — 正是规则 7 写来防的「pinrule 过度纠正 → 用户被迫绕」失败模式。Catch trigger 没绕，深挖 regex，修判定器。两个新 case lock 住新豁免和保留拦截（`test_v0518_read_pinrule_state_write_tmp_exempted` + `test_v0518_redirect_target_is_pinrule_path_still_blocked`）。

### 验证

- `tests/test_bypass_pinrule.py` 加 5 个回归测试覆盖: read-pinrule-写-tmp 豁免、pipe-to-python 豁免、write-to-pinrule 仍拦、internal-field-name + write-tmp 豁免（跟 state_path fix 对称）、internal-field-name + write-pinrule 仍拦
- `pytest`：416/416 通过（411 + 新 5）
- `ruff`：0 issues
- 4 个之前 `test_*_real_bypass_*` 仍绿 — fix 没松开写检测

## [0.5.17] — 2026-05-15（docs — README narrative 重写：`/pinrule <NL>` skill 提升为顶级 section，不再是 patch 式提及）

### 这版动机

v0.5.16 ship 了能用的 skill 但 README 仍然把它当成「Customize 章节内的 patch 式提及」— 「Agent 替你写规则」能力是一行 aside，「Agent 守规则」能力独占整个 hero。本版按用户原则重写 README narrative 让 pinrule 两面闭环在 landing page 上平起平坐：

> 「对外说明文档一定不要只是打补丁，要很「爆款」的融入整体说明，重要亮点和功能说明展示好。」

### 改了啥（README + README.zh.md 对称）

**1. Hero opening 重写** — 之前是单段「监督 Agent」+ 违规率数字。现在明确把 pinrule framing 为「同一闭环的两面」：🛡️ 钉规则 / Agent 守 + ✨ 大白话告诉 pinrule / Agent 替你写。两面各配具体一行。

**2. 目录** — 加 `/pinrule 自然语言录入规则` 作为顶级 entry，跟 install / 原理 / 自定义并列。

**3. 痛点表格** — 加第 7 行 v0.5.16 解决的痛点（「想加规则但 yaml 太重 / 措辞 Agent 不响应」），让 value-prop 用跟其他 6 个痛点同样的对照表格出现。

**4. Quick install 段** — 加一行 callout 说 `pinrule init` 自动装 skill 到三家 backend，让用户从 install 起就知道开箱即用不需要额外步骤。

**5. 新顶级 section `/pinrule <自然语言>` — Agent 替你写规则** — 替换 v0.5.15 在 Customize 内 patch 的 20 行「推荐路径」子段。新 section 55+ 行：7 步流程可视化、「skill 替你做的事」6 行表（语气 / 格式 / 重叠 / scope / locale / modify）、「三家 backend 一个命令」装机表、升级流程（`pinrule install-skill --force` / `--backend`）。
...
- `ruff`：0 issues
- 手工 sanity：TOC anchor `#pinrule-自然语言--agent-替你写规则` resolve；首次读者落到 README 的章节切分合理

### 触发

本 release 由用户输 `/pinrule 每次commit以后必须更新所有 github 文档至最新版本...要很「爆款」的融入整体说明` 触发 — pinrule skill 第一次现场端到端使用加了 rule 10（`docs-sync-after-commit`），本 commit 是新加规则的第一次立即应用。

## [0.5.16] — 2026-05-15（feat — `/pinrule <自然语言>` skill 可用，多 backend 装机）

### 这版为啥重要

session 内深度 audit（用户问「`/pinrule rule X` 能不能简化成 `/pinrule X`」触发）发现 **v0.5.1 起 pinrule skill 从未触发过**。根因：Claude Code skill 机制要求 `<name>/SKILL.md` 目录形式（不是裸 `<name>.md`）+ `name:` frontmatter 字段 + 单 token 命令（不是 `/pinrule rule` 多词）。v0.5.1 ~ v0.5.15 全部按错的假设 ship — 手工 CLI 测试能工作，但 skill 自动触发从来没工作。
...
| Codex CLI | `~/.agents/skills/pinrule/SKILL.md`（注意 `~/.agents/` 不是 `~/.codex/`）| Markdown | `/skills` menu / `$pinrule <args>` inline / auto |
| Gemini CLI | `~/.gemini/skills/pinrule/SKILL.md` + `~/.gemini/commands/pinrule.toml`（双轨）| Markdown（skill）+ TOML（commands）| skill 路径 auto-trigger + commands 路径显式 `/pinrule <args>` |

### 改了啥

**1. 仓库 skill source 重组** — `skills/pinrule-rule.md`（裸文件，错）→ `skills/pinrule/SKILL.md`（正确目录形式）。加 `name: pinrule` + `description: ...` frontmatter。skill body 内所有 `/pinrule rule X` 引用改 `/pinrule X` 跟简化触发命令对齐。

**2. 新模块 `pinrule/skill_packaging.py`** — 格式转换处理：
...
**3. `Backend` Protocol 扩展** 加 `skill_install_targets(skill_name="pinrule") -> list[tuple[Path, str]]`。每个 backend 声明自己装路径 + 内容格式。3 个 backend 实现：
- `ClaudeCodeBackend` → 1 个目标（Markdown）
- `CodexBackend` → 1 个目标（Markdown，`~/.agents/` 路径）
- `GeminiCLIBackend` → 2 个目标（Markdown skill + TOML commands）

**4. CLI 多 backend 支持**：
- `_install_pinrule_skill_multi_backend(force, backend_filter)` — 中央装机函数，遍历所有 detected backend，按格式写每个目标
- `cmd_install_skill(force, backend)` — `pinrule install-skill` 默认装到所有；`--backend claude-code|codex|gemini-cli` 指定单家
- `cmd_init` — 自动装到所有 backend，每个目标打印 `创建 [<backend>] pinrule skill: <path>`
- `cmd_doctor` — 报多 backend skill 状态（✓ 最新 / ⚠ 跟当前版本不一致 / 未装），每个 (backend, path) 一行

**5. `pyproject.toml`** — `force-include` 改 `skills/pinrule/SKILL.md`，`pip install pinrule` 装对文件。
...
**这是 pinrule skill 第一次被 Claude Code 看到。** v0.5.1 ~ v0.5.15 它一直默默躺在错的路径上。
...
- 老的 `~/.claude/skills/pinrule-rule.md`（v0.5.12-15 装的裸文件）是死重，可以 `rm`
- 新 skill 下次 `pinrule init` 或 `pinrule install-skill` 自动装
- `/pinrule rule X` 命令从来没工作过（虽然 doc 说能），新的 `/pinrule X` 在 Claude Code 里能（其他家尽力）
- Codex / Gemini 支持是 best-effort — Codex 用 `/skills` menu 或 `$pinrule` inline；Gemini 显式 `/pinrule` 走 TOML commands 路径

### v0.5.1 到 v0.5.15 文档说的 vs 现实（sticky #4 诚实披露）

v0.5.1 release notes 说「Claude Code skill template at `skills/pinrule-rule.md` for natural-language rule input」并描述 `/pinrule rule <NL>` 触发。**端到端从来没工作过** 直到本版。skill 流程只在用户手工调底层 `pinrule rule add --from-yaml` CLI 时工作 — 自然语言 → skill 自动 refine 那条路径是空气。对前面误导的 doc 道歉。

## [0.5.15] — 2026-05-15（chore — v0.6.0 准备：起草计划稿 + 内部 `pinrule.sticky` → `pinrule.rule` import 迁移）

### 这版动机

v0.5.13 audit 号称「清完所有 `.sticky_id` callsite」但只清了属性级。起草 v0.6.0 计划时 follow-up audit 发现更深一层 miss：pinrule 自己源码里**还有 11 处 `from pinrule.sticky import ...`**（cli.py 4 处 + hooks/*.py 6 处 + 自指）— 加上 4 个测试文件里的平行 import。v0.6.0 删 `pinrule/sticky.py` 前，pinrule 自己得先不 import 它。本版修这个。
...
- **Group A** — 内部脚手架（只 pinrule 自己引用的 alias）。零外部影响。
- **Group B** — public API 破坏性改动（`pinrule.sticky` 模块 / `.sticky_id` @property / `pinrule sticky` CLI alias）。v0.5.0 起一直 deprecated；v0.6.0 是悬崖。
- **Group C** — 盘上数据 migration（`sticky.yaml` → `rules.yaml`、老 `violations.jsonl` 的 `sticky_id` 字段兜底）。**永远保留** — 这些处理用户数据不是 API 表面。

含执行顺序、测试覆盖期待、风险评估、2 个开放问题（`pinrule sticky` CLI alias 要不要多活一个 release 周期；非中文用户的 `chinese_plain_no_jargon` 默认行为是否在 v0.6.0 范围 — 都暂定「不」延后再定）。

**2. v0.6.0 前置 import 迁移**（本版执行）

把 `from pinrule.sticky import X` → `from pinrule.rule import X`，覆盖：

- `pinrule/cli.py`（4 处）
- `pinrule/hooks/post_tool_use.py`、`hooks/stop.py`、`hooks/pre_tool_use.py`、`hooks/subagent_start.py`、`hooks/user_prompt_submit.py`、`hooks/pre_compact.py`、`hooks/session_start.py`（7 个 hook 文件共 7 处）
- `tests/test_violations.py`、`test_sticky.py`、`test_paths.py`、`test_cli.py`、`test_post_tool_use_reinject.py`（5 个测试文件）
- `test_post_tool_use_reinject.py` 里 `mock.patch("pinrule.sticky.load", ...)` → `mock.patch("pinrule.rule.load", ...)`（4 处 patch）— Python module aliasing 意味着 patch alias namespace 不会传递到对应 module，如果消费者直接 import 对应 module 的话

### 验证

- `pytest`：410/410 通过
- `pytest -W error::DeprecationWarning`：410/410 通过 — **pinrule 自己代码和测试里 0 处 `pinrule.sticky` deprecation warning** 触发
- `ruff`：0 issues
- `grep -rn "from pinrule.sticky" pinrule/ tests/` 只剩 `pinrule/sticky.py` shim 自己 docstring 里写的（shim 存在目的就是被 import，本身不 import 自己）

### v0.6.0 就绪状态

本版后，v0.6.0 删 `pinrule/sticky.py` 不会破任何内部 callsite。4 个 class/property alias（`MAX_STICKY` / `Sticky` / `StickyConfigError` / `EXAMPLE_STICKY*`）也是 — 0 内部使用者。`CheckHit` + `Violation` 上 `.sticky_id` @property 自 v0.5.13 起就 0 内部使用者。`pinrule sticky <subcommand>` CLI alias 在 `cli.py:1183` 是个 entry-point 分支，0 内部使用者。

简单说：v0.6.0 可以是纯删除 commit，不需要 refactor。

## [0.5.14] — 2026-05-15（docs — `pinrule-rule` skill 教会 Agent 用现有命令组合做 modify，不加新 CLI）

### 这版动机

dogfooding 发现 gap：Agent 走完 skill Step 2 决策表说「modify 现有规则」，但 skill 在这里断了 — 提到 `pinrule rule edit` 但那命令是启动 `$EDITOR` 给用户手编（Agent 无法自动化）。Agent 没有清晰路径用现有 CLI 完成「modify」，导致我（正在 dogfooding 的 Agent）提议加新命令 `pinrule rule replace`。用户立刻 pushback：不要扩大 CLI 表面，把现有命令教清楚。
...
- **Step 2 下新加 "How to modify an existing rule (replace / merge / extend scope)" section**：
  ...
  - 明确说「为啥不用 `pinrule rule edit`」— 那是用户逃生口不是 Agent 路径
...
用户本 session 立场原话：「不希望给用户增加一堆不常用的 skill」。Modify = remove + add，现有命令组合够用。加 `pinrule rule replace` 就是表面 bloat 无能力增量 — Agent 缺的只是 recipe 写在 skill 里.
...
用户的 `~/.claude/pinrule/sticky.yaml` 里 `lighthearted-vibe` 规则被改写：作用域从「加 pinrule 规则对话时」扩到「整体说话方式」，对偶半句从 mild「该严肃就严肃」升级为「具体问题分析要认深刻」。这次改写是 dogfooding 触发，暴露了本版修的 skill gap.
...
`testset.py` / `bypass_pinrule.py` / `non_blocking.py` 各自独立定义同款 regex `r"\b(?:python\d?|node|ruby|perl)\s+-[ce]\b"`。v0.5.9 把平行的 `_BASH_REDIR_TARGET_RE` 提到 `description_context.py` 但漏了这个。

**修复**：在 `pinrule/checks/common.py` 加 `is_python_c_command(cmd: str) -> bool` helper（这里是对位 home — 跟 `_SHELL_INTERPRETER_RE` / `_HEREDOC_RE` 等其他 Bash 解析工具放一起）。3 个 check 全 import + 调用 `is_python_c_command(cmd_raw)` 代替本地 pattern。

### F2 — `pinrule doctor` 没报 skill 装机状态

v0.5.12 加了 `pinrule install-skill`，但 `cmd_doctor` 只报 hook 装机不报 skill。新装用户跑 `pinrule doctor` 看不到 `/pinrule rule <NL>` 流程是否接通。

**修复**：`cmd_doctor` 现在报 `pinrule-rule skill` 三态：
- "存在 ✓ 最新" — 装好且跟仓库 source 一致
- "存在 ⚠ 跟当前 pinrule 版本不一致" — 装着但过时（建议 `pinrule install-skill` 升级）
- "未装" — 没装（建议 `pinrule install-skill`）
...
## [0.5.12] — 2026-05-15（feat — `pinrule init` 自动装 `pinrule-rule` skill + 新加 `pinrule install-skill` 命令）

### feat — `/pinrule rule <NL>` 流程对新用户开箱即用

v0.5.11 audit 发现的 gap：`skills/pinrule-rule.md` 在仓库里但没自动装到 `~/.claude/skills/pinrule-rule.md`，第一次用 pinrule 的用户在 Claude Code 里输 `/pinrule rule add a new rule about X` 触发不到 — skill 得手工 copy。本版补齐。

### 改动

- **`pinrule init` 末尾自动装 skill** 到 `~/.claude/skills/pinrule-rule.md`。首次跑打印「创建 pinrule-rule skill: <path>」+ `/pinrule rule <NL>` 用法提示。
- **新加 `pinrule install-skill [--force]` 子命令** 给 v0.5.12 之前装过 pinrule 的用户（或想升级 skill 比如 v0.5.11 clarity audit 之后）。不带 `--force` 时冲突非破坏 — 用户改过本地 skill → 新版写到 `pinrule-rule.md.new` 提示用户对比/合并。`--force` 强制覆盖。
- **`pyproject.toml` `force-include`** 把 `skills/pinrule-rule.md` 打进 wheel 让 `pip install pinrule` 也能用。
- **`pinrule --help`** 列出新的 `install-skill` 子命令带简短用法。
...
- `tests/test_cli.py` 新增 5 个回归测试：
  - `test_v0512_init_auto_installs_pinrule_rule_skill` — 首次跑装好 ✓
...
## [0.5.11] — 2026-05-15（docs — `skills/pinrule-rule.md` 清晰度 audit，补 5 个 gap）

### docs — `/pinrule rule` skill template 5 个清晰度 gap 修复

Dogfooding 驱动的 audit。用自然语言录入流程跑了一遍 `/pinrule rule` 之后，发现 5 个陌生 Agent 容易默默猜错的地方：

1. **Step 1 漏 anchor-vs-scope 歧义识别** — 用户原话「在 X 场景下要 Y」通常意思是「X 是引发例子」而不是「Y 只在 X 时生效」，但 pinrule v2 是 always-on 注入（无 scene routing）。skill 现在要求 Agent 直接把这个歧义明说出来对齐而不是默默猜作用域。还加了「one-off vs long-term」识别清单（`"for this PR" → one-off` / `"I always want" → long-term`），让「这事到底该不该入 pinrule」的判断有具体抓手。
...
4. **缺 locale-aware tone 指引** — v0.5.2 i18n 之后 pinrule 双 locale，但 skill 全英文示例。加了明确规则「用户跟你说哪种语言就用哪种写 preference；violation_checks 函数名保持英文不变」。中文 locale Agent 被指向 `data/rules.dev.example.zh.yaml` 作为参考模式。

5. **Step 7「啥时候生效」埋在底部** — 原 skill 在末尾独立 `## Restart Claude Code after pinrule rule add` section，容易漏。把「下个 UserPromptSubmit 起注入」notice 搬进 Step 7 内联第 4 条，同时把「建议删改」步骤改具体（直接点名冗余的具体规则对，不要泛泛「review for duplicates」）。删了底部独立 section。

底部 `## Common mistakes to avoid` 列表加 3 条对应 gap 1 / 4 / 3 的反例，让快速扫一眼也能 catch 高影响失败模式。

### 发现但 v0.5.11 没修

audit 顺手发现 `skills/pinrule-rule.md` **没被 `pinrule init` 自动装到** `~/.claude/skills/pinrule-rule.md` — 用户得手工 copy。意思是当前 `/pinrule rule <NL>` 流程只有手工装 skill 的用户能用。本 release 是纯文档版不在 scope，但值得 v0.5.12 加个 `pinrule install-skill` 或扩 `pinrule init`。
...
## [0.5.10] — 2026-05-15（docs — `pinrule --help` 补 `rule add` / `rule preview` 子命令列表）

### docs — `pinrule --help` 之前藏着 `pinrule rule add` / `pinrule rule preview`

用户授权的 dogfooding 测试（第一次端到端跑 v0.5.1 `pinrule rule` 流程）发现 `pinrule --help` 仍然只列 `pinrule sticky list/edit/remove` — v0.5.1 加的 `rule add` / `rule preview` / `rule list/edit/remove` 子命令已经完整实装且 dispatch 正常，但顶层 help 看不到。第一次用 pinrule 的用户输 `pinrule --help` 完全不知道 `pinrule rule add` 存在。

本版修 `pinrule/cli.py` 顶部 docstring：
- 列全 4 个 `rule` 子命令（`list` / `edit` / `remove` / `add` / `preview`）及其 flags (`--from-yaml <file>` / `--from-stdin`)
- 标注 `pinrule sticky` 为 v0.6.0 移除的 deprecated alias
- 末尾加 Claude Code `/pinrule rule <自然语言>` skill 工作流指引

实装从 v0.5.1 起一直工作；本版是纯文档修复。

### 端到端验证 (16 个 test case)

- `pinrule rule preview --from-stdin` 合法 yaml → schema check + 注入预览渲染 ✓
- `pinrule rule preview` 错误路径 (缺 id / yaml 文件不存在) → `exit 1` 带 `❌` 信息 ✓
- `pinrule rule add --from-stdin` 合法 yaml → schema 校验 + id 唯一性 + 上限 + REGISTRY 检查 + 写入 + 反馈 ✓
- `pinrule rule add --from-yaml <file>` 合法 yaml → 同流程 ✓
- `pinrule rule add` 重复 id → `exit 1` ✓
- `pinrule rule add` 未知 `violation_checks` 函数 → `exit 1` 带可用函数清单 ✓
- `pinrule rule add` schema 错 (缺 preference) → `exit 1` ✓
- `pinrule rule add` 无效 yaml → `exit 1` ✓
- `pinrule rule add` 无 flag → `exit 1` 带 usage + `/pinrule rule` skill 提示 ✓
- `pinrule rule` 无子命令 → `exit 1` 带子命令列表 ✓
- `pinrule rule foobar` 未知子命令 → `exit 1` ✓
- `pinrule rule list` 新加规则可见 ✓
- `pinrule rule remove <id>` 删 ✓
- `pinrule rule remove <id>` 然后 `pinrule rule add` 同 id → 成功 ✓
...
v0.5.4 i18n 后副作用：`pinrule audit` 按 `trigger` 字面分组，用户 zh locale 跑一周切到 en locale 后会看到「同行为分两组 counter 计数」。audit「top trigger」分析失真。
...
## [0.5.5] — 2026-05-15（fix — testset check 补 python -c 豁免，跟 non_blocking / bypass_pinrule 对齐）

### fix — testset.py 漏 python -c 字符串字面豁免（dogfooding 触发）

v0.5.3 自测时触发：probe 脚本 `python -c "r = check(content='gold_cases.append(x)')"` 被 testset check 错拦 — 把引号内的 `gold_cases.append(x)` 字面当成反喂调用。根因：受 `python -c` 影响的 3 个 check 里，只有 `testset.py` 漏了 `_LANG_C_HEAD_RE` 豁免（`non_blocking.py` 在 v0.4.18 加了、`bypass_pinrule.py` 在 v0.4.13 加了）。
...
`trigger` 字段写入 `~/.claude/pinrule/violations.jsonl` 用作 audit log 分类标签，是 v0.5.3 留下的最后一个双语缺口。v0.5.4 收尾：8 个 check 模块 28 处 trigger 全部走 `tr()`，跟 fix namespace 平行。

- 14 处 trigger 直接调用 — `chinese_plain` / `non_blocking` / `evidence` / `keep_pushing` / `read_first` / `bypass_pinrule`（含 `{term}` / `{cmd}` / `{word}` / `{tool}` / `{file_path}` / `{target}` 插值）
...
- **`pinrule/checks/chinese_plain.py`**（3 处）— `ratio` / `jargon` / `repeated_prefix`。注意：chinese_plain check 本身是中文用户专属，英文 default 装机时通过规则模板选择移除
- **`pinrule/checks/non_blocking.py`**（4 处）— `python_block` / `sleep` / `wait` / `long_task`（含 `{cmd}` 插值）
- **`pinrule/checks/evidence.py`**（3 处）— `commit` / `completion` / `weak_claim`
- **`pinrule/checks/keep_pushing.py`**（2 处）— `stop_hint` / `default`
- **`pinrule/checks/read_first.py`**（1 处，含 `{file_path}` 插值）
- **`pinrule/checks/bypass_pinrule.py`**（1 处）
- **`pinrule/checks/long_term.py`**（pattern 表内 7 处）— `long_id_branch` / `blacklist_literal` / `uppercase_const_list` / `commit_hack` / `git_skip_verify` / `todo_marker` / `patch_intent`
- **`pinrule/checks/testset.py`**（pattern 表内 7 处）— `reverse_feed` / `detail_writeback` / `cross_split_copy` / `detail_append` / `split_hardcode` / `hash_branch` / `case_list_hash`
...
- `CheckHit.trigger` 字段 — 内部 audit log 分类标签，写入 `~/.claude/pinrule/violations.jsonl`。不在 Agent 注入路径上，优先级低，后续小版本配合 trigger-key namespace 设计一并迁移

## [0.5.0] — 2026-05-15（**major breaking change** — sticky → rule 全代码库改名）

> **用户原话**：「将整个 pinrule 所有代码和文件的 sticky 字样改成 rule」+
> 「直接做成 `/pinrule rule XXX` 的命令」+「希望支持其他主要语言」
...
- **模块**：`pinrule/sticky.py` → `pinrule/rule.py`（git mv 保留 history），老 `pinrule/sticky.py` 改成 compat shim 含 DeprecationWarning
- **字段**：`Violation.sticky_id` → `Violation.rule_id`（property `sticky_id` alias 保留），`CheckHit.sticky_id` → `CheckHit.rule_id`
- **CLI**：`pinrule sticky list/edit/remove` → `pinrule rule list/edit/remove`，老 `pinrule sticky` 作为 deprecated alias
- **配置文件**：`~/.claude/pinrule/sticky.yaml` → `~/.claude/pinrule/rules.yaml`，老用户跑 `pinrule init` 自动迁移 + backup 为 `sticky.yaml.bak`
...
- `from pinrule.sticky import Sticky / StickyConfigError / MAX_STICKY` 仍工作（DeprecationWarning 提示迁移）
- `pinrule sticky list` 仍跑（同样输出 + DeprecationWarning）
- `~/.claude/pinrule/sticky.yaml` 仍可读（pinrule.rule.DEFAULT_PATH fallback 找到）
...
- **README v5 用户驱动深度优化**（2026-05-15 仓库公开后第二轮，作者亲自给 12 个具体调整方向）— 用户视角落地：
  ...
  - 删全文 pinrule v1 引用（FAQ + 相关项目），用户不需要知道还有 v1
...
`pinrule/hooks/subagent_stop.py` v0.4.30 起的 `hookSpecificOutput.additionalContext`
...
事件 Claude Code UI 自身会显示，pinrule 不需要重复 echo。

顺手清死代码（sticky_list 加载不再用 → 删 `from pinrule.sticky import load`）。

### Fix 2 — PreCompact schema 合规

`pinrule/hooks/pre_compact.py` 同 SubagentStop 思路 — v0.4.29 起输出
...
pinrule 长期以为 hook 生效但 Agent 没看到。
...
`pinrule/hooks/stop.py:295-301` 删幽灵代码（`hookSpecificOutput` 输出）。
...
`reason` 字段 — 仍是 v0.4.41 老版本「pinrule stop hook 反思提醒 ... 请自检 ...」
指控式 + 双重否定句式。改成合作回顾语气：
...
- 测试 392/392 ✓（一处 test_stop_hook_respects_block_max assertion 更新为
  接受 `{}` passthrough 输出）
- ruff ✓ / mypy pinrule+tests ✓ / vulture 0 死代码
- `pinrule --version` = v0.4.43 ✓
- manual run Stop hook：干净 `{"decision": "block", "reason": "..."}` schema
  合规输出，干预原因外 passthrough `{}`，不再被 Claude Code 报「Expected schema」

### 后续

pinrule stop hook 早期所有 hookSpecificOutput 设计（v0.4.x 早期阶段）都已检查 —
...
**批次 1 — 3 处包装文本**：
- `pinrule/sticky.py:format_for_injection` 头部「请始终遵守」→「合作默契」+
  加「这不是规则也不是审判」破除监督感；违反标记 ⚠️ → 〔上一回应这条
  有偏离，本 turn 看看能否更对齐〕合作回顾标记
- `pinrule/hooks/user_prompt_submit.py` 强提醒段「命中检测」→「上一回应
  没对齐默契」+ 收尾「立即按 fix 不要再犯」→「不需要为这条特意补偿过度」
- `pinrule/hooks/post_tool_use.py` 锚定刷新「sticky 易稀释」技术词 →
...
- chinese_plain 3 / non_blocking 4 / evidence 3 / keep_pushing 2 /
  long_term 7 / testset 7 / read_first 1 / bypass_pinrule 1
...
- `~/.claude/pinrule/sticky.yaml` + `data/sticky.dev.example.yaml` 移除
  violation_keywords + violation_checks（保留 preference 文本提醒）
- `pinrule/checks/chinese_plain.py` + REGISTRY 注册保留供恢复
- `tests/test_sticky.py` 加 soft_only 例外不强制 chinese-plain 有 check

### Task 4 — stats / audit / doctor 跨 session 数据分开

权威 source：`pinrule/session_state.py:get_current_session_id()` 按 mtime
选主 Agent session-state 文件，比 `violations[-1].session_id` 推更权威 —
当前 session 可能完全没产生违反但仍是当前活跃。

- `pinrule/cli.py:cmd_stats` 加「本 ses」列对照「历史」列
- `pinrule/cli.py:cmd_audit` 显示当前 session id 前 8 字（如 `c6d3eb4a...`）
- `pinrule/cli.py:cmd_doctor` 用 `get_current_session_id()` fallback
...
4 件套全过：ruff ✓ / mypy pinrule+tests ✓ / vulture 0 死代码 / pytest 392 ✓。
...
- `pinrule/hooks/stop.py` 加 `_read_last_user_prompt(transcript_path)` 镜像 `_read_last_assistant_response`，抽公共 `_read_last_message_text(path, msg_type)` 函数复用 reverse scan jsonl 路径
- `pinrule/checks/__init__.py` `run_checks` 加 `user_prompt` 入参透传给 check 函数
- `pinrule/checks/keep_pushing.py` `check()` 加 `user_prompt: str = ""` 入参 + 新 `_USER_STOP_HINT_RE` 匹配 sticky #8 例外字面（不用啦 / 不用了 / 休息吧 / 明天再说 / 先到这 / 算了 / 停一下 / 停下 / 别推了 / 别继续 / 不再推 / 够了 / 到此为止 / 收尾吧 / 睡吧 / 晚安 / 好了好了 / 走火入魔 等）
...
pinrule 自身设计闭环 — sticky #8 例外条件文本里写的「用户明确叫停」字面清单**在工程层 enforced**，不是文本声明而已。今晚多次 dogfooding 触发暴露这盲区，v0.4.41 让 sticky #8 例外清单从「文本声明」变「工程层豁免」。
...
**1. 反思阈值 3 → 2**：
- `pinrule/config.py` `stop_block_max_per_turn` 默认 3 → 2
- `pinrule/hooks/stop.py` 两处 fallback 默认 3 → 2
...
- `_DOTTED_IDENT_RE` 剥含点号工程标识符（`pre_tool_use.py` / `state.model` / `pinrule.hooks.session_start` / `extract_model_from_transcript()`）
- `_PATH_LITERAL_RE` 剥路径字面（`/path/to/file` / `~/.claude/pinrule/...`）
...
pinrule 路径：reverse scan transcript jsonl 找最后一条非合成 model 字面。

### 实施

- `pinrule/model_threshold.py` 加 `extract_model_from_transcript(transcript_path)` 函数 — regex 扫 raw 内容比逐行 json.parse 快 10x，reverse 取最后一个非 `<synthetic>` 实际值
- `pinrule/hooks/post_tool_use.py` + `pinrule/hooks/user_prompt_submit.py` 改用 transcript_path 路径替代之前 payload.get("model")（v0.4.36 / v0.4.38 实施）
- `pinrule/hooks/session_start.py` 保留 payload.model 直接拿（向后兼容）
...
主 Agent 跑 Agent tool (model=sonnet) 派子 Agent
  ↓
主 PreToolUse(Agent, model=sonnet) → pinrule 入队 main_state.pending_subagent_models
  ↓
SubagentStart(agent_id=uuid) → pinrule pop 队首 → 写子 Agent state.model = "sonnet"
  ↓
...
- `pinrule/session_state.py`: `SessionState` 加 `pending_subagent_models: list[str]` 字段 + load/save 序列化
- `pinrule/hooks/pre_tool_use.py`: if `tool_name == "Agent"` and `tool_input.get("model")` → `main_state.pending_subagent_models.append(model)` + save
- `pinrule/hooks/subagent_start.py`: load 主 state → pop 队首 → 写子 Agent state.model + save 主 state（清队列）+ save 子 state
...
- `pinrule/hooks/session_start.py` 加从 payload 读 model 写 state.model — Claude Code 本地协议下唯一暴露 model 的事件
...
生效证据：
```
echo '{"source":"startup","session_id":"test","model":"claude-opus-4-7"}' | python -m pinrule.hooks.session_start
cat ~/.claude/pinrule/session-state/test.json | grep model
...
- `pinrule/model_threshold.py` 新模块：`threshold_for_model(model: str | None) -> int` 关键词匹配 + 7 条守护测试覆盖（opus / sonnet / haiku / 老模型 / 未知 fallback / 大小写 / 关键词优先级）
- `pinrule/session_state.py` `SessionState` 加 `model: str | None = None` 字段 + load/save 序列化
- `pinrule/hooks/post_tool_use.py`：从 payload `model` 字段更新 `state.model` + `_build_smart_reinject` 阈值优先级 = sticky.yaml 显式配置 > `threshold_for_model(state.model)` 按模型 > DEFAULT 60K
...
## [0.4.34] — 2026-05-15（feat — 子 Agent 独立 pinrule 监控架构 + v0.4.32 叙事对齐 + v3 第七步验证完成）

### 触发

用户洞察：「子 Agent 的行为能不能起一个临时 pinrule 监控并精准注入到子 Agent 的运行过程中，不影响主 Agent，并且子 Agent 结束运行就自动销毁。这本来就是两个完全不同的进程彼此互不干扰才对。」
...
- pinrule 当前 `pre_tool_use.py:64` 只读 `session_id` 没读 `agent_id` → 根因
...
- `pinrule/session_state.py`: `_state_path / load / save` 加 `agent_id` 可选参数 — 给的话路径加 `__<agent_id>` 后缀；新加 `purge_subagent_state(session_id, agent_id)` 销毁 + `SessionState` 加 `agent_id: str | None = None` 字段
- `pinrule/violations.py`: `Violation` 加 `agent_id` 字段 + `to_json` 序列化（None 不写省 jsonl 体积 + 向后兼容） + `detect()` 接受 `agent_id` 参数透传
- `pinrule/hooks/pre_tool_use.py`: 读 `agent_id = payload.get("agent_id")` + `state = session_state.load(session_id, agent_id=agent_id)` + Violation 写 agent_id
- `pinrule/hooks/post_tool_use.py`: 同上路由
- `pinrule/hooks/stop.py`: 同上路由
- `pinrule/hooks/subagent_stop.py`: 加 `purge_subagent_state(session_id, agent_id)` 销毁子 Agent 临时 state + 文案改「临时 state 已自动销毁」
...
pinrule 8K 阈值不是「模型开始忘」的判据，是「sticky 在 attention 里被新上下文**稀释**到该重新锚定」的判据 — 价值是**抗稀释**不是**抗遗忘**。Liu 2023 数据撑当代阈值依据是错的，但 8K 抗稀释频率在工程层仍合理。

文档 / 注释改：
- `pinrule/session_state.py` `tool_byte_seq` 字段注释加 v0.4.34 叙事对齐说明
- `pinrule/hooks/post_tool_use.py` `_build_smart_reinject` docstring 改「衰减」→「稀释」
...
**根本结论**：**路径 A 生效** — 子 Agent 内 Bash 被主 PreToolUse hook 拦 + 写主 violations.jsonl + 主 Stop hook 也会扫子 Agent 完成响应文本。**pinrule 当前架构已自动监管子 Agent 内 tool 调用**，不需要写新 SubagentStop transcript scan 机制（路径 B）。
...
v0.4.32 commit + tag + push 都成功，但 `gh release create` 命令被自己的 deep-fix-not-bypass check 拦了 — release notes 里描述用户场景的 markdown 反引号包字面 `` `cat ~/.claude/pinrule/session-state/xxx.json` `` 被错当 shell command substitution 保留扫到外层 cmd。
...
- 复现脚本 `gh release create --notes "$(cat <<'EOF' ...`cat ~/.claude/pinrule/session-state/...` ... EOF)"` strip 后**完全干净** ✓
- 测试 362 → 364 全过 + ruff 干净 + vulture 0 死代码
- v0.4.32 同时 release 触发 v0.4.33 根因 fix

## [0.4.32] — 2026-05-15（fix — bypass_pinrule `json.dumps` 假阳 + feat — 中段注入 token 启发式频率优化）
...
2. **bypass_pinrule 假阳拦 cat 读** — 用户调试时 `cat ~/.claude/pinrule/session-state/xxx.json | python -c "import json; d = json.load(...); print(json.dumps(d))"` 被 deep-fix-not-bypass 拦了。但这是纯 read-only 输出，没写任何 pinrule 文件。

### 根因 + 根因 fix

**bypass_pinrule 假阳根因**：`_PYTHON_OR_SHELL_WRITE_RE` 里 `json\.dump` regex **没加 word boundary** `\b` → `json.dumps`（序列化为字符串纯输出）被误判 `json.dump`（写 file-like）。同样 `p\.write` 也缺边界（`p.writeable` 类字面会假阳）。

修：`r"json\.dump\b|p\.write\b"` 加 `\b`，加 3 条守护测试（cat 读 / json.dumps 假阳 / json.dump 写对偶）。

**中段注入频率根因 + 升级**（用户决策驱动）：

pinrule turn 定义 = `state.turn_count += 1` 在 user_prompt_submit hook = **1 turn = 1 次 user 提问 + Agent 全部响应**（哪怕跑 100 个 tool call）。所以「最近 5 turn 内触发 sticky 就注入」在长任务里**永远窗口内**，每个 PostToolUse 都注入 → 频率 60+/turn → Agent 表达扭曲。
...
- `pinrule/session_state.py` 加 `tool_byte_seq` + `last_reinject_byte_seq` 两字段 + load/save 序列化
- `pinrule/hooks/user_prompt_submit.py` 每 turn 起手 `tool_byte_seq=0` + `last_reinject_byte_seq=0` 归零
- `pinrule/hooks/post_tool_use.py` 加 `_estimate_tokens(tool_input, tool_response)` = `len // 3` 启发式 + 累积 + 阈值判定 + 注入后重置 last_reinject_byte_seq 节流
...
- `pinrule/hooks/subagent_start.py` 用 `ensure_ascii=False` 输出中文 +
  passthrough 抽公共函数 + 文本表达跟其他 hook 风格统一（去 emoji + 用
  `[pinrule 子 Agent 继承父 session 的核心方向]` 格式跟 SessionStart baseline
  对齐）
...
## [0.4.30] — 2026-05-14（feat — pinrule v3 第六步：SubagentStart/Stop 装机 + 删 PostCompact 幽灵代码）
...
- **PostCompact 协议层不支持 `additionalContext`** — v0.4.29 留的 post_compact.py
  整段是「幽灵代码」（输出会被 Claude Code 静默丢），不是 pinrule 实现 bug 是
...
- `pinrule/hooks/post_compact.py` **删** — 幽灵代码，留着是技术债
- `pinrule/hooks/subagent_start.py` 装 — 子 Agent 启动时注入 sticky baseline
  到子 Agent 上下文（注入位置是子 Agent 不是主 Agent）让子 Agent 跑长任务
  也按这些方向
- `pinrule/hooks/subagent_stop.py` 重写 — 早期 stub 用 substring match 扫子
...
- `pinrule/backends/claude_code.py` `_HOOK_EVENTS` 加 SubagentStart / SubagentStop —
...
## [0.4.29] — 2026-05-14（feat — pinrule v3 第五步：PreCompact 落盘 + 两端夹击 compact 失忆 / CI 修）
...
同时 CI 4 个 job 全失败 — ruff 4 个错（pinrule 早期 stub 文件 unused import / var）：
- `pinrule/hooks/subagent_start.py:29` unused `agent_id`
...
**2. pinrule v3 第五步落地** — PreCompact 升级（早期 stub 用 `continue: false`
想阻止 compact，错。compact 是 Claude Code 保护机制 pinrule 不该干扰。改成纯落盘 +
注入 reminder）：
...
不阻止 compact — compact 是 Claude Code 保护长 session 不爆 token 的机制，pinrule
...
已装用户跑：`pinrule install-hooks --backend claude-code` 重装加新 PreCompact 入口。
...
## [0.4.28] — 2026-05-14（feat — pinrule v3 第四步：SessionStart 注入 sticky baseline）

### 触发

用户问：「claude 的 hook 接口还有好多个，研究下其他几个还没有使用的 hook 接口
有哪些对咱们 pinrule 有价值值得探索一下」。子 Agent 研究 9 个未用 hook 协议后
...
pinrule v2 当前仅 UserPromptSubmit 每 turn 注入完整 sticky。但 session 起手时（包括
...
`pinrule/hooks/session_start.py` 已存在但是早期 stub（只输出摘要文字不注入 sticky
...
`pinrule/backends/claude_code.py` `_HOOK_EVENTS` 加 `"SessionStart": "session_start"`
让 install-hooks 装 wrapper 到 `~/.claude/settings.json`。Codex / Gemini 协议
没对应 event 是 Claude Code 特有 — `test_backends_all_have_4_common_pinrule_wrappers`
...
4 种 source 跑通：
```
source=startup  → [pinrule session 起手 sticky baseline — source=startup] + 8 sticky
source=resume   → [pinrule session 恢复 — sticky baseline 重新加载] + 8 sticky
source=clear    → [pinrule session 起手 sticky baseline — source=clear] + 8 sticky
source=compact  → [pinrule 上下文 compact 后重起 — 这些核心方向必须留在记忆里]
                  + 8 sticky
...
已装升级：用户跑 `pinrule install-hooks --backend claude-code` 重装 — 加新 SessionStart
...
回滚 v0.4.26 的两类改造：
- `pinrule/checks/long_term.py` 5 处 suggested_fix 改回原命令式
- `pinrule/checks/non_blocking.py` 4 处 suggested_fix 改回原命令式

保留：
- `pinrule/checks/keep_pushing.py` 反思式（2 处）
- `pinrule/checks/chinese_plain.py` 反思式（2 处）
- `pinrule/hooks/stop.py` force_block reason 反思式
...
## [0.4.26] — 2026-05-14（feat — pinrule v3 第三步：4 类价值观规则反思式语气改造）
...
pinrule 之前所有 suggested_fix / force_block reason 都是**命令式语气**「不要 X」
...
**「价值观引导用对话，工程纪律用命令」** — pinrule 从「监督员」往「同事教练」演化，
...
pinrule v2 → v3 渐进式演化清单已经完整。

## [0.4.25] — 2026-05-14（feat — pinrule v3 第二步：audit 字面多样性元行为监测）
...
`pinrule/cli.py` `cmd_audit` 加字面多样性列：
...
## [0.4.24] — 2026-05-14（feat — pinrule v3 第一步：PostToolUse 中段 sticky reinject 锚定）

### 突破

dogfooding 实测验证 Claude Code PostToolUse hook 接受 `additionalContext`
字段 — system-reminder 显示 `[pinrule 中段提醒 — 最近 turn 触发过的 sticky
别再犯]` 字面。**pinrule v3 proactive 锚定路径打通**。
...
`pinrule/hooks/post_tool_use.py` 加 `_build_smart_reinject()`：
...
system-reminder 显示 `[pinrule 中段提醒]` + 当前最近触发过的 3 条 sticky
（non-blocking / chinese-plain / loud-failure）。这是 pinrule v3 第一次**真**
中段锚定。
...
第一步**（锚定让 Agent 自然不忘 sticky）。这是 pinrule v2 → v3 演化的起点。
...
v0.4.22 commit 那次被 pinrule 自己拦了（命令字面含 `time.sleep(60)` 实际阻塞 pattern
...
pinrule 自己的 pre_tool_use hook 拦命令导致 commit 失败 → 但 shell `&&` 链
继续跑后面 tag/push/release → 产生「tag 指向错 commit」幽灵 release。这是
shell `&&` 短路行为跟 pinrule 拦截语义的冲突。

下次 commit + tag + release 类链式命令应该用 `set -e` 或者拆开跑保证前一步
失败不继续。或者 pinrule hook 应该返回 exit code 让 shell `&&` 短路。
...
| Fix | 漏拦 case | 严重度 |
|---|---|---|
| v0.4.13 deep-fix | `python -c "os.system('rm pinrule')"` → None | **绕过漏拦** ⚠️ |
...
1. **`bypass_pinrule.py` 加 python 调 shell 绕过接口**：`os.system / subprocess.
...
- CLI 加 `--format md` flag。组合用：`pinrule audit --with-fix-timeline --format md`
...
`pinrule/checks/keep_pushing.py` 加新豁免（紧接 `_PUSH_SIGNAL_RE.search(tail)`
豁免后）：
...
`pinrule audit` 显示 keep-pushing-no-stop 修前 26 / 修后 6 — v0.4.12 部分修
后仍触发 6 次（最近 5 turn 11 次），都是「response 纯陈述完结无推进」类
pinrule 自标 ⚠️ 可能假阳。dogfooding 看真 snippet 找出 3 类剩余假阳：
...
`pinrule/checks/keep_pushing.py`：
...
件治理：pinrule 自测 `_SLEEP_RE` 探针 `python3 -c "for c in ['sleep 5']: ..."`
...
保留 `python -c` 内容（设计上拦 `bash -c 'rm pinrule'` 类绕过），但 python
代码里的 `sleep` / `wait` 字面是 identifier / 字符串数据不是 shell 调用。

### Fix

`pinrule/checks/non_blocking.py` 加 `_LANG_C_HEAD_RE`（跟 bypass_pinrule.py
v0.4.13 完全一致）：
...
- 仅 pinrule 仓库 cwd + git 可用时启用（dev 工具，不破坏跨用户场景）
- fail open — 不在 pinrule 仓库 / git 不可用 → 静默不报错，正常 audit 视图
...
327 测试全过；跑 `pinrule audit --with-fix-timeline` 输出完整 timeline 标记。
...
`pinrule/hooks/stop.py` 的 force_block 逻辑（line 210-213）：
...
`pinrule/checks/chinese_plain.py` jargon 扫描全部从 `natural` 改用
...
pinrule 内部状态」。深挖：`_WRITE_OP_RE` 的 `> c` 命中了 python 代码里的
比较运算符 `> cutoff`，因为 `strip_shell_quoted_literals` 保留 `python -c`
内容（设计上为拦截「`bash -c 'rm pinrule'`」类 indirect 绕过）。

### Fix

`pinrule/checks/bypass_pinrule.py` 拆 `_WRITE_OP_RE` 成两类：
...
2. `python -c "open(pinrule).write('{}')"` → 命中 ✓（真 python 写绕过）
3. `echo '{}' > ~/.claude/pinrule/session-state.json` → 命中 ✓（shell 绕过）
4. `pinrule violations clear` → None ✓（CLI 合法操作）

321 测试全过；加 3 个守护 case 在 `tests/test_bypass_pinrule.py`。

## [0.4.12] — 2026-05-14（patch — keep-pushing 假阳治理 + scripts/verify-installed.sh）

### 触发

`.venv/bin/pinrule stats` 看：`keep-pushing-no-stop` 最近 5 turn 触发 **10
...
- `我去看 pinrule check ...` — 「我去 + 看/查」是近 future 推进信号
  但 `_PUSH_SIGNAL_RE` 漏覆盖（只有「我现在/立刻/马上 + 动词」）

### Fix

`pinrule/checks/keep_pushing.py` 两处 regex 扩：
...
加 `scripts/verify-installed.sh`：对比 pyproject 版本 vs `.venv/bin/pinrule
...
v0.4.10 刚发完一句汇报响应立即触发**第 6 次** force_block：pinrule 自己的
...
`pinrule/checks/chinese_plain.py` 增 `_KEBAB_SNAKE_IDENT_RE`：
...
`pinrule/checks/chinese_plain.py` 增 3 个剥离正则在算 ratio 前应用：
...
用户体验影响：pinrule 装完后 codex 不会自动调度 wrapper，**第一次 TUI 必须
手动审批 4 个 pinrule wrapper**。
...
> /hooks             # 交互审批 pinrule 4 个 wrapper
> /quit              # 退出
# 之后再跑任何 codex 命令 pinrule hook 触发
```
...
**状态**：
- pinrule 装在 `~/.codex/hooks.json` 对应 codex **CLI** — 用 `codex` 终端命令
...
- pinrule 在 codex 协议下 5/5 生效（模拟 codex payload 跑 4 wrapper 全过 +
...
- **`cmd_install_hooks` 默认 `claude-code` backend 不查 `client_installed()`
  静默装 hook 配置** — 之前同事没装 Claude Code 跑 `pinrule install-hooks`
...
- **P3**：pyproject classifier 去掉 Windows 声明（pinrule wrapper 用 Unix
  shebang Windows shell 不识别，未实测过先不声明）+ README 前置要求加
  「Windows 建议 WSL」。
- **P4**：README 新增「装完立即做：自定义 sticky 偏好」段 — 明示 pinrule
...
## [0.4.6] — 2026-05-14（patch — `pinrule uninstall` 一键卸装 alias）

### Added

- **`pinrule uninstall`** — `pinrule uninstall-hooks --backend all` 的一键 alias。
  陌生用户想完全卸装 pinrule 时不用记 backend flag 长串，一句 `pinrule uninstall`
  就清所有 backend（Claude Code / Codex / Gemini）+ 删 wrapper + 从客户端
  配置移除 pinrule entry，保留他人 hook（vibe-island / rtk 等）共存。
...
## [0.4.5] — 2026-05-14（patch — PINRULE_HOME 环境变量 + sub-agent 评审驱动改进）

「同事即将首装」我 spawn 一个 sub-agent 扮演陌生用户跑首装清单**实测试**，
找到 5 条问题。本版修最关键 P0：

### Added — `PINRULE_HOME` 环境变量支持

之前 `~/.claude/pinrule` 路径写死 5 个模块（cli / sticky / violations /
session_state / config）— dry-run / CI / 多 profile 都污染默认 home。
sub-agent 评审作为 v2 边界 bug 标出。

新建 `pinrule/paths.py:pinrule_home()` 单一来源 + 所有模块用它。`PINRULE_HOME`
env 隔离用法：

```bash
PINRULE_HOME=/tmp/pinrule-test pinrule init            # 不动 ~/.claude/pinrule/
PINRULE_HOME=~/pinrule-profile-A pinrule sticky list   # 多 profile
```

加 4 条 subprocess 测试守护（用新 Python 进程让 env 在 import pinrule 之前
...
- 清单加 `pinrule init` 第 5 步明示（之前禁止 init 但 init 是必要步骤）
...
「同事即将首装 pinrule」消息触发 README 站陌生用户视角重审 + 触发 3 个真
问题修。这是 dogfooding → real-user 转折点的标志性 patch。

### Fixed

- **`pinrule --version` 输出错版本号** — `__init__.py` 硬写 `__version__ = "0.1.0"`
  跟 pyproject 双维护失同步，bump 到 0.4.3 后 `--version` 还是输出 v0.1.0
  让用户疑惑。修：`__init__.py` 用 `importlib.metadata.version("pinrule")`
  单一来源读 pyproject metadata（editable install 后重 `pip install -e .`
  让 metadata 同步）。加 `test_version_matches_pyproject` 守护防回归。
- **`pinrule install-hooks --help` 漏列 `gemini-cli` backend** — `--backend
...
- 加 **⚠️ 关键最后一步「装完必须重启 AI 客户端」** — Claude Code / Codex /
  Gemini CLI 都是 session 启动时一次性读 hook 配置不重载，跑中 session pinrule
  不触发。新用户最容易踩坑。
- 加「维护跟卸载」段警告 wrapper 硬写 venv 路径 — 删 / 移动 / 重建 `.venv`
  前必须先 `pinrule uninstall-hooks --backend all`，否则 hook 指向不存在的
  python 让 AI 客户端启动报错。
...
## [0.4.2] — 2026-05-14（patch — dogfooding 实测发现 bypass_pinrule 假阳）

### Fixed

`bypass_pinrule._WRITE_OP_RE` 误识别 `2>/dev/null` 类 stderr 重定向为写操作。

**实测触发场景**：跑 `python -c "...session_state.load(...)" 2>/dev/null`
只读 inspection pinrule 内部状态时被误拦 — 命令含 pinrule 状态路径字面
（`~/.claude/pinrule/session-state`） + `2>/dev/null` 之前被
...
丢弃目标，普通文件路径不放过）；`echo bad > ~/.claude/pinrule/session-state/abc.json`
写 pinrule 状态仍要拦。
...
- **`pinrule/backends/_json_hooks.py`** 通用基类，提供 90% 共用实现：
  client_installed / hooks_dir / settings_path / load_settings / save_settings /
  is_pinrule_entry / 默认 build_event_entry / 默认 pre_install_setup（空）。
...
- 4 件套全绿：299 测试 / ruff / mypy（pinrule + tests） / vulture 0 issue。

## [0.4.0] — 2026-05-14（minor — 第三个 backend：Gemini CLI 适配）

### Added — Gemini CLI 装机支持

pinrule 三家 AI 编程客户端 backend 全打通：Claude Code（v0.1.0）+ Codex CLI
（v0.3.0）+ Gemini CLI（v0.4.0）。三个客户端 hook 协议跟 pinrule 实测兼容,
装机 / 卸装 / hook 触发 catch 违反全跑通。

- **`pinrule/backends/gemini_cli.py`**：新 `GeminiCLIBackend` 实现，
  `~/.gemini/settings.json` 配置（含 `hooks` 字段），默认启用（不像 Codex 要
  feature flag）。
- **`pinrule install-hooks --backend gemini-cli`** 装机；`--backend all` 自动
  装本机三家全检测到的客户端。
- **Stop hook 跨协议跨 backend 适配**：pinrule `stop.py` 现在适配 3 个不同字段名：
...
pinrule backend 抽象设计巧妙处理：**`hook_events()` key 是 backend 实际 event 名
（写进各家配置文件），value 是 pinrule 内部 wrapper basename**。这样 4 个 wrapper
（user_prompt_submit / pre_tool_use / post_tool_use / stop）跨 3 backend 完全
复用，pinrule hook 入口模块代码 0 改动。

| pinrule wrapper | Claude Code | Codex | Gemini CLI |
|---|---|---|---|
...
- 装机：`~/.gemini/settings.json` 6 个 vibe-island event + 4 个 pinrule event 共存 ✓
- AfterAgent 模拟 payload 跑 pinrule stop.py → catch「我先打个补丁」违反 +
  decision=block + reason 输出 ✓
- 卸装：pinrule 4 entry 清除，vibe-island 7 个 entry 完整保留 ✓
...
pinrule 从「Claude Code 专用」升级为「多 AI 编程客户端通用」框架。新增 Codex
CLI 适配 — 协议跟 Claude Code **几乎一对一兼容**，实测装机 / 卸装 / hook 真
触发全跑通。

- **`pinrule/backends/` 多 backend 抽象**：`Backend` Protocol 定义客户端无关的
...
- **`pinrule install-hooks --backend codex|claude-code|all`**：默认 claude-code
  向后兼容；`--backend codex` 装 Codex；`--backend all` 装本机检测到的所有客户端。
- **`pinrule uninstall-hooks --backend ...`**：同样支持，验证保留他人 hook
  （vibe-island 等共存插件）。
- **`pinrule doctor` 跨 backend 显示**：每个客户端 ✓/✗ 检测，含 hook 装机状态。
...
- Codex 6 个 hook event vs Claude Code 4 个 — 共有 UserPromptSubmit / PreToolUse /
  PostToolUse / Stop（pinrule 用这 4 个）；Codex 额外有 SessionStart / PermissionRequest
- Codex stdin payload **没** `transcript_path`，但 Stop hook 直接给
  `last_assistant_message` — pinrule 自动适配
...
- `cli.py` 旧硬编码 helper 函数（`_settings_path` / `_save_settings` /
  `_remove_pinrule_entries` / `_add_pinrule_entries` / `_check_hooks_installed` /
  `_pinrule_event_entry` / `_PINRULE_HOOK_EVENTS` / `_SettingsParseError` 等）
...
- 实测装机：`pinrule install-hooks --backend codex` 写 `~/.codex/hooks.json`，
...
新模块 `pinrule/locale_detect.py` — 跟其他 app（VS Code / Slack / Chrome）安装
时一样的「按系统语言偏好自动选」做法。
...
- `pinrule init`（无 flag）→ **自动按系统语言偏好选**：中文用户装 7 条完整
  含 chinese_plain；非中文 / 检测不到装 5 条精简。检测结果会打印让用户知道。
- `pinrule init --minimal` / `--no-minimal` 强制覆盖自动选择
...
## [0.2.3] — 2026-05-14（patch — pinrule init --minimal flag）

### Added

- **`pinrule init --minimal`** 显式 flag 装 5 条中性核心模板（评审 C Agent
  第二轮指出 minimal 模板存在但默认 7 条对英语母语用户仍是持续假阳源）。
  - 评审建议过「`pinrule init` 检测系统 locale 自动选」— 实测后否决：
    `locale.getlocale()` 在 macOS 默认返回 `en_US` 但用户可能是中文，
    自动猜错率高。改用显式 flag 让用户自己选（显式优于隐式）。
  - 默认 `pinrule init` 仍装 7 条向后兼容；末尾打印 `--minimal` 提示让英文
    用户知道有选项。
...
- `PINRULE_DEBUG_TRACE` 加 2 条守护测试（评审第二轮指出 v0.2.1 补了
  `PINRULE_DEBUG` 但姊妹变量 `PINRULE_DEBUG_TRACE` 没测过 — sticky #4 违反）。
...
- **`ARCHITECTURE.md` 加「配置」章节** — v0.2.0 README 重组让链接指向
  ...
  （`PINRULE_NO_NOTIFY` / `PINRULE_DEBUG` / `PINRULE_DEBUG_TRACE`）。
- **mypy 类型化** — 之前我说「会改 200+ 行」推迟，**跑后只有 3 个 error**
  10 分钟修完（`testset.py` / `long_term.py` underscore 变量名跨类型重用 →
  `_label`；`cli.py:_pinrule_event_entry` dict 异质 value → `dict[str, object]`
  显式标注）。mypy 加进 `[project.optional-dependencies].dev` + CI 步骤守护。

### Test / Quality

- `run_checks` `PINRULE_DEBUG=1` 门控加 3 条守护测试 — 之前加了功能没验证过
...
- **`data/sticky.dev.minimal.example.yaml`** 中性 5 条核心 sticky 模板：
  ...
  - 评审 C Agent 痛点：默认 7 条违反 CLAUDE.md「不针对当前用户作弊」
    ...
    ~/.claude/pinrule/sticky.yaml` 切换。
  - 默认 `pinrule init` 仍装 7 条（向后兼容现有 0.1.x 用户）。
...
`pinrule/checks/common.py:strip_shell_quoted_literals` 三个违反假阴漏报修复 ——
...
pinrule v2 的第一个可发布版本，经历多轮 dogfooding + 4 个 Opus 4.7 评审 Agent
交叉评审 + 1-2 小时质量打磨。
...
- 每次 install-hooks 额外写带时间戳备份（保留初次 `.before-pinrule` + ts 版本）
...
- `bypass_pinrule._WRITE_OP_RE` 移除 `cp/mv/rm`（用户备份 pinrule 状态文件 /
...
**代码质量 refactor**：
- `pinrule/checks/_types.py` 抽 `CheckHit` / `CheckFn` Protocol —— 消除 8 处
  子模块函数体内反向 `from pinrule.checks import CheckHit` 循环依赖代码味道
...
- `run_checks` 加 `PINRULE_DEBUG=1` 门控的 stderr trace（check 函数抛异常时
...
[Unreleased]: https://github.com/jhaizhou-ops/pinrule/compare/v0.4.9...HEAD
[0.4.9]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.4.9
[0.4.8]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.4.8
[0.4.7]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.4.7
[0.4.6]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.4.6
[0.4.5]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.4.5
[0.4.4]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.4.4
[0.4.3]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.4.3
[0.4.2]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.4.2
[0.4.1]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.4.1
[0.4.0]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.4.0
[0.3.0]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.3.0
[0.2.4]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.2.4
[0.2.3]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.2.3
[0.2.2]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.2.2
[0.2.1]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.2.1
[0.2.0]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.2.0
[0.1.1]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.1.1
[0.1.0]: https://github.com/jhaizhou-ops/pinrule/releases/tag/v0.1.0
