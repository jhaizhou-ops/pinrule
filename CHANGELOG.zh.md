...
## [0.16.15] — 2026-05-17 — 修 `PINRULE_HOME` sandbox 漏洞: Claude skill 装机路径走 `pinrule_install_root()` (之前硬编码 `Path.home()` 让 sandbox 承诺破口); 加 3 个 lockdown 测试覆盖三家 backend 的 skill 路径 (854 测).
