...
def run_tool_gate(payload: dict) -> int:
    """Run karma pre-tool checks; print allow/deny JSON to stdout."""
    ...
    if tool_name == "Agent":
        ...
        if sub_model:
            try:
                ...
                print(f"karma tool gate: 入队子 Agent model 失败 ({e})", file=sys.stderr)
    ...
    try:
        ...
        print(f"karma tool gate: update_state fallback ({e})", file=sys.stderr)
    ...
    if check_hits:
        ...
        print(f"🛑 karma: {top.rule_id} (tool={tool_name}) — {top.trigger}", file=sys.stderr)
    else:
...
def main_stdin() -> int:
    try:
        ...
        print(f"karma tool gate: JSON 解析失败 ({e})", file=sys.stderr)
        print(emit_allow({}))
        return 0
