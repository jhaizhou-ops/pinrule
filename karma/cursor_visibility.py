"""Make karma rule IDs visible in Cursor Composer context.
...
_SKILL_DIR = "karma-rules-catalog"
_SKILL_FILENAME = "SKILL.md"


def build_rules_catalog_skill_body(rule_ids: list[str], karma_home: Path) -> str:
    """Skill body: only rule ids + one-line instruction (kept short for skill picker)."""
    lines = [
        "---",
        "name: karma-rules-catalog",
        "description: >",
        "  Active karma rule IDs (auto-synced). When the user asks for karma rule ids,",
        "  visibility, or rules.yaml contents without reading files, list every id below verbatim.",
        "---",
        "",
        f"# karma active rule IDs (`KARMA_HOME={karma_home}`)",
        "",
        "If asked whether karma rules are visible, answer **yes** and list **all** ids:",
        "",
    ]
    for rid in rule_ids:
        lines.append(f"- `{rid}`")
...
def sync_claude_skills_catalog(*, karma_home: Path | None = None) -> tuple[Path | None, list[str]]:
    """Write ~/.claude/skills/karma-rules-catalog/SKILL.md for Cursor visibility."""
    from karma.rule import RuleConfigError, load

    logs: list[str] = []
    if karma_home is None:
        from karma.paths import karma_home as _kh
        karma_home = _kh()
    ...
    dest.write_text(
        build_rules_catalog_skill_body(rule_ids, karma_home),
        encoding="utf-8",
    )
...
def sync_all_visibility_layers(*, karma_home: Path | None = None) -> list[str]:
    """Run all Composer-visible sync paths."""
    from karma.cursor_rules_sync import build_mdc_content, sync_cursor_rules
    from karma.rule import RuleConfigError, format_for_injection, format_rule_id_catalog, load
    ...
    _, skill_logs = sync_claude_skills_catalog(karma_home=karma_home)
