...
│  ├── pinrule_user_prompt_submit.py   ← Inject rules per msg │
│  ├── pinrule_pre_tool_use.py         ← Real-time intercept  │
│  ├── pinrule_post_tool_use.py        ← State tracking       │
│  └── pinrule_stop.py                 ← Scan violations      │
└───────────────────────────────────────────────────────────┘
                       │
...
- `violation_checks` — function names from `pinrule/checks/__init__.py:REGISTRY`
...
- Format as `[pinrule — Your long-term agreement with the user]` + numbered rules
...
### Speaking side: `pinrule/i18n.py` + `data/locales/{en,zh}.yaml`

What pinrule **says to the Agent** — hook injection text, suggested_fix strings, audit labels. `tr(key, **fmt)` lookup with `{placeholder}` interpolation, fail-open on missing keys. Locale resolution chain:

```
PINRULE_LOCALE env > config.yaml `locale` field > auto-detect (chinese ratio) > en fallback
```
...
### Listening side: `pinrule/signals.py` + `data/signals/<name>/{zh,en}.{txt,yaml}` (v0.8.0 → v0.8.2)

What pinrule **listens for in dialogue** — detection phrases for `keep_pushing` / `evidence` checks. Two storage formats:
...
```bash
# Initialize
pinrule init                       # Create ~/.claude/pinrule/ + copy rule templates

# Rule management
pinrule rule list                  # List all rules
pinrule rule edit                  # Open rules.yaml with $EDITOR
...
`~/.claude/pinrule/config.yaml` adjusts thresholds without modifying code. `pinrule doctor` shows current effective values. All missing fields use `pinrule/config.py:DEFAULTS` (fail open); you can configure only the fields you care about.
...
`pinrule init` copies `data/config.example.yaml` template.

### Debug environment variables

- `PINRULE_NO_NOTIFY=1` — Disable desktop notifications (CI / mute scenarios)
...
pinrule state defaults to `~/.claude/pinrule/` (contains `rules.yaml` / `violations.jsonl` / `session-state/` / `config.yaml`). The `PINRULE_HOME` env var changes the path — useful for dry-run / CI / multi-profile isolation:

```bash
PINRULE_HOME=/tmp/pinrule-test pinrule init           # Doesn't touch ~/.claude/pinrule/
PINRULE_HOME=~/pinrule-profile-A pinrule rule list    # Multi-profile isolation
```

Note: paths freeze at module-level constant import time, so `PINRULE_HOME` must be set **before** launching the pinrule process. Hook-wrapper-invoked pinrule doesn't read this env (wrapper doesn't pass env), so actual usage primarily uses `~/.claude/pinrule/`.
...
- All data local in `~/.claude/pinrule/`
- No data uploaded
- No LLM calls (pinrule v2 strictly no LLM, even v1+ won't introduce)
- Users can `rm -rf ~/.claude/pinrule/` anytime to clear state
...
| v0.5.1 `pinrule rule add` / `rule preview` CLI + Claude Code skill template for natural-language rule input | ✅ |
| v0.5.2 i18n MVP — `pinrule/i18n.py` + 5 hook injection paths switchable en/zh | ✅ |
...
| v0.5.7 `trigger_key` field on `CheckHit` + `Violation` — locale-agnostic `pinrule audit` grouping | ✅ |
| v0.5.8 + v0.5.9 Bash heredoc → description-context path exemption, lifted into `description_context.py` shared layer | ✅ |
| v0.5.10 `pinrule --help` lists `rule add` / `rule preview` subcommands (doc-only) | ✅ |
| v0.5.11 `skills/pinrule-rule.md` clarity audit — 5 gaps closed (anchor-vs-scope / overlap decision table / inline draft review / locale-aware tone / when-takes-effect) | ✅ |
| v0.5.12 `pinrule init` auto-installs `pinrule-rule` skill + new `pinrule install-skill [--force]` command | ✅ |
| v0.5.13 audit-driven dedup — shared `is_python_c_command` helper + 34 `.sticky_id` callsites → `.rule_id` + `pinrule doctor` reports skill status | ✅ |
...
| v0.5.15 v0.6.0 preparation — plan doc `docs/V0_6_0_PLAN.md` + internal 11+4 `from pinrule.sticky` import migration to `from pinrule.rule` so v0.6.0 can ship as pure deletion commit | ✅ |
| v0.5.16 `/pinrule <natural language>` skill actually works — first release; multi-backend install (Claude Code / Codex / Cursor) with Markdown → TOML format adaptation for Gemini commands path; v0.5.1-15 honest disclosure (wrong install path → skill never triggered before) | ✅ |
| v0.5.17 README narrative rewrite — `/pinrule <NL>` skill promoted to top-level section instead of patch-style mention; PRD F5 rewritten; ARCHITECTURE + HANDOFF synced to v0.5.16 reality | ✅ |
| v0.5.18 `bypass_pinrule` false-positive fix (dogfood-found) — redirect target must actually be a pinrule path to count as bypass, not just "command mentions pinrule path + any write op"; symmetric tightening for `has_internal` field-name dimension | ✅ |
...
| **v0.6.0** ⚠️ BREAKING — Remove `pinrule.sticky` module, `.sticky_id` @property on `CheckHit`+`Violation`, `pinrule sticky` CLI subcommand, and `pinrule.rule`/`pinrule.cli` internal aliases (`Sticky` / `MAX_STICKY` / `StickyConfigError` / `EXAMPLE_STICKY*`). Data-compat shims (`sticky.yaml`→`rules.yaml` auto-migration, `violations.jsonl` `sticky_id` field fallback) stay permanently. Deprecation cycle: 18 v0.5.x releases. Pure-deletion commit — no logic refactor needed thanks to v0.5.13/15 internal cleanup. 5 deletion-lock tests added. | ✅ |
...
| v0.7.0 treat-root-cause refactor — rewrite "真X" defensive prefixes in pinrule source rule texts. User caught Agent stacking "真X" mimicry from pinrule's own rule injection headers (in-context mimicry). Reverted attempted `defensive_prefix_stacking` engine check (treat-symptom) in favor of cleaning the source. ~140 occurrences rewritten across rule templates + locale + user-facing docs. | ✅ |
...
| **v0.8.5 3rd code review pass — 2 high-value cleanups + clean-state confirmation**. User-requested 3rd audit round. Tools clean (vulture/ruff/455 tests); manual audit found 2 high-value items: `rule.py:format_for_injection` function-level `from pinrule.i18n import tr` hoisted to module top (i18n is leaf module, no circular risk); `chinese_plain.py:L179` inline magic `< 30` extracted as `_JARGON_PAREN_MAX_DIST = 30` constant. Honestly skipped middle/low-value polish (cli.py 10 function-level imports — some serve test-mock friendliness; 4 cli long functions — coordinators with no dead code). Doc consistency audit: test count 455 + signal count 7 + 0 dead links across 16 key docs. v0.8.x series ends in 3-way-confirmed clean state. | ✅ |
...
User's own words: "We keep pushing — that IS the observation period." Every push has Claude running with pinrule installed; every commit goes through hook interception. M3 accumulated 30+ real violations; all 7-8 rules triggered.

pinrule isn't "develop first, observe later" — it's "development is dogfooding."
