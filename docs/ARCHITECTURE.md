...
| v0.5.16 `/karma <natural language>` skill actually works — first release; multi-backend install (Claude Code / Codex / Cursor) with Markdown → TOML format adaptation for Gemini commands path; v0.5.1-15 honest disclosure (wrong install path → skill never triggered before) | ✅ |
...
| **v0.10.1 first codex-owned PR merged (shell-as-Read) + karma-side wiring + cross-backend contract tests**. v0.10.0 ownership split proved itself within hours: Codex CLI opened PR #3 implementing `CodexBackend.normalize_tool_input()` shell-read detection only touching codex-owned files (`karma/backends/codex.py` + new `tests/test_codex_backend.py`), boundary discipline perfect. karma-maintainer did the explicit-callout counterpart: `karma/hooks/post_tool_use.py` consumes canonical `tool_input["read_file_paths"]` list — backend-neutral (any future backend emitting this field benefits). New integration test locks the full chain. End-to-end: codex agent shell-reads now register as Read in karma state → subsequent `apply_patch` no longer false-positive denied. Plus `tests/contract/test_backend_contract.py` adds 14 abstract contract tests × N backends = 42 auto-validations via pytest parametrize. CI vulture --min-confidence 60 false-positive on `shlex.shlex.whitespace_split` added to `whitelist.py`. 568/568 both locales. **Meta-pattern confirmed**: ownership split is right answer — Codex CLI captured real session rollouts as evidence (better signal than any Claude audit), karma-maintainer work small + focused because boundary clear in advance. | ✅ |

| **v0.10.2 second codex-owned PR (#4) closes major gaps to Claude code parity**. Codex backend now covers 5 of codex 0.130's 6 events (SessionStart / UserPromptSubmit / Pre&PostToolUse / Stop). Three additions: SessionStart event registered with real captured payload fully compatible with Claude shape (codex fires before first prompt not at TUI startup — functional correctness preserved); `exec_command → Bash` map + `cmd → command` field copy so `record_bash` / `is_test_cmd` / `last_test_pass_ts` work on codex; **`trust_karma_hooks()` auto-writes `trusted_hash` to `~/.codex/config.toml` for karma-owned wrappers** eliminating manual `/hooks` approval (karma's worst v0.10.0-era onboarding friction). Safety scoped: only karma `is_karma_entry`-verified wrappers, hash-algorithm-change falls back to `/hooks` "modified" not silent drift. karma maintainer counterpart: README alert box flipped to "auto-trust" + bilingual docs v0.10.2 entries. 575/575 both locales. **Ownership split validated 2 PRs in a row** — codex evidence-backed (real session rollouts) + on-time + delivers bonus beyond explicit ask. | ✅ |
