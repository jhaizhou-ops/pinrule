...
┌───────────────────────────────────────────────────────────┐
│  ~/.pinrule/                                         │
│  ├── rules.json              ← User-maintained core rules │
...
`pinrule/checks/__init__.py:REGISTRY` maps rules.json's `violation_checks` strings → functions:
...
### Listening side: `pinrule/signals.py` + `data/signals/<name>/{zh,en}.{txt,py}` (v0.8.0 → v0.8.2; v0.17.0 migrated YAML → Python module)
...
- **`.py` module Cartesian DSL**: `push_signals` — `DATA` dict with `templates` (`{subject}` / `{verb}` placeholders) + vocabulary lists + non-Cartesian `phrases`
...
- `pinrule_home()` — data dir (rules.json etc.). All 5 modules (rule / violations / session_state / config / cli) read this.
...
| **v0.6.0** ⚠️ BREAKING — Remove `pinrule.sticky` module, `.sticky_id` @property on `CheckHit`+`Violation`, `pinrule sticky` CLI subcommand, and `pinrule.rule`/`pinrule.cli` internal aliases (`Sticky` / `MAX_STICKY` / `StickyConfigError` / `EXAMPLE_STICKY*`). Data-compat shims (`rules.json`→`rules.json` auto-migration, `violations.jsonl` `sticky_id` field fallback) stay permanently. Deprecation cycle: 18 v0.5.x releases. Pure-deletion commit — no logic refactor needed thanks to v0.5.13/15 internal cleanup. 5 deletion-lock tests added. | ✅ |
