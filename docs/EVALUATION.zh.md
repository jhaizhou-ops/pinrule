...
- `pinrule` 包 import + 从 `~/.pinrule/rules.json` 加载规则
- hook handler 分发
...
这些数字是 pinrule 开发期间作者自用 dogfood — 30 个工作 session 跨 ~2 周, 手工数 `~/.pinrule/session-state/*.json` (每个文件记 `tool_byte_seq` 累积 / 漂移事件 / reinject 触发) + Claude Code 对话日志.

**目前没有 `pinrule audit --token-ratio` 这个 helper** — 自动化在 roadmap 但还没用户提需求, 暂保持手工. 如果你想测自己的数据, 真输入源:

- `~/.pinrule/session-state/<session_id>.json` — 每 session pinrule 状态 (read/edit 文件 / byte 累积 / reinject 标记)
- 你 AI 客户端的对话日志 — 总 token 数的权威源 (pinrule 自己不存这个)

如果你测了自己数据想要可复现脚本, [提 Issue](https://github.com/jhaizhou-ops/pinrule/issues) — 这就是触发真做这个 helper 的信号.
...
- **目前没测客户端侧开销** — AI 客户端本身（Claude / Codex / Cursor / Hermes）起 hook 子进程 + 读 stdout 也有延迟，pinrule 的 `scripts/measure_perf.py` 只测 pinrule 自己那部分。
