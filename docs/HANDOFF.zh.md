...
| **v0.12.2 砍 sticky.yaml legacy fallback (无 migration 需要)** | pinrule v2 pre-launch, 没公开 v0.5.0 之前用户需要 migrate. `rule.py:_LEGACY_STICKY_PATH` + `_resolve_default_path()` fallback + cli.py:228-240 migration block (12 行) 全砍. 砍的 block 含潜伏 bug — `rules_path.name == "rules.yaml"` 条件永远 False 因为 `_resolve_default_path()` 在 legacy 文件存在时 resolve 到 sticky.yaml. 死代码 + 潜伏 bug 一起清. backends/cursor.py:post_install_message "或手工编辑 sticky.yaml" → "rules.yaml" (唯一还在 user-facing 文案里 sticky.yaml 提及). 保留: 开发者 `#` 注释 historical context. bypass_pinrule check 仍 match `.claude/pinrule` path fragment (任何 pinrule state 文件意外编辑都拦, 不管文件名). | v0.12.2 |
...
3. **v0.4.13 deep-fix `shutil.rmtree('~/.claude/pinrule')` 整目录漏拦** —
   ...
   sticky.yaml）不匹配整目录 `.claude/pinrule/`。
