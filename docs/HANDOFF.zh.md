...
| **v0.14.0 Cursor native 12 events + 共享 ~/.karma home (cursor agent dogfood ship)** | 第一个 contributor agent (Cursor desktop Agent, Composer Cursor 1.7+) 36 files 大改 PR #10. **共享 rules library**: 默认 `KARMA_HOME=~/.karma` 跨 Claude/Cursor/Codex 三家, `karma init` 自动 migrate `~/.claude/karma` → `~/.karma`. doctor warns split legacy homes. **Cursor native surface**: 12 hook events 含 native gate (beforeShellExecution / beforeMCPExecution / beforeReadFile / afterAgentResponse / preCompact / subagentStart-Stop 等), `_tool_gate` 跨 backend 通用 shell/MCP gate. **Composer 双层 visibility**: `karma sync-cursor-rules` 命令 + `~/.cursor/rules/karma-sticky.mdc` (`alwaysApply`) 作 hook stdout 注入不可靠时的起手保险层. `[rule-id]` in `format_for_injection`. **Maintainer rebase + 7 处 cleanup**: stop hook 双 JSON 输出真 bug (audit_agent_response 返 bool intervened 信号), non_blocking.py None-safety, dead code, vulture whitelist forward-compat. 806 pytest 绿 (从 824 -18 砍 Gemini-specific 测试). main HEAD `d88d664` PR #10 squash-merge. **本机 dogfood 大坑**: ~/.karma 撞用户另一个无关项目 (daemon.sock + popups/), 用户拍板 mv 备份让位置. | v0.14.0 |
