...
分层注入让 UserPromptSubmit 每 turn 只 ~490 token, 100 turn 跨 1M Opus context 累积约 8.2%; SessionStart 跟 PostToolUse 中段 reinject 各承担一次全量, 把规则信号集中在 Agent 注意力高点 + 衰减拐点.
...
| **v0.10.4 优先用 codex payload.model + OpenAI/Codex 阈值表**. karma 中段 reinject 原来 Claude-only — `gpt-5.5` (1M context) fallback 到 DEFAULT 40K 太密. 新 `model_from_payload(payload)` 统一查找: payload.model 优先 (Codex 官方 hooks doc 明确说这是稳定 signal, transcript_path 不是), transcript fallback. 接入 3 个 hook. 加 11 个 OpenAI/Codex 阈值: `gpt-5.5/5.4 → 120K, gpt-5.3-codex/5.2-codex → 80K, mini/nano/spark → 40K-30K`. Claude 行为不变. Codex `/model` 中途切换立刻识别. **老实说不做**: PreCompact / SubagentStart/Stop / PermissionRequest 在 codex 上仍不可 hook (API limitation 不是 karma scope). 中段 reinject 是跨平台替代. 15 个新测试. 595/595 双 locale (原 580). | ✅ |
