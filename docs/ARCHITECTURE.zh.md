...
|---|---|---|
| 数据目录（rules.yaml / violations.jsonl / session-state/ / config.yaml） | `~/.pinrule/` | `/tmp/foo/.pinrule/`（走 `pinrule_home()`） |
...
- `pinrule_home()` — 数据目录（rules.yaml 等）。所有 5 个 module（rule / violations / session_state / config / cli）都读这个。
