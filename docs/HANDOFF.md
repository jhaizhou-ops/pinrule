...
🔜 Next session — `karma audit` data continues to drive refinement; v0.8.1 redesigns `_PUSH_SIGNAL_RE` to also be i18n-friendly (likely small DSL or hybrid): chinese-plain residual jargon, keep-pushing pattern tuning. The treat-root philosophy from v0.7.x is now the default lens for new check additions.
