...
- 用户在 `~/.claude/pinrule/rules.yaml` 定义 5-10 条（软上限 10，超过 12 拒绝加载）
...
**触发方式**：用户在 Claude / Codex / Cursor 任一输 `/pinrule <自然语言>`。skill 走 7 步：识别意图 → 检查现有规则重叠 → 内联起草 yaml → `pinrule rule preview` schema 校验 → 跟用户确认 → `pinrule rule add` 写入 → 反馈报告
...
- `pinrule rule add --from-yaml <file>` / `pinrule rule add --from-stdin` — 程序化写规则，含 schema + id 唯一性 + REGISTRY 校验
- `pinrule rule preview --from-yaml/--from-stdin` — dry-run 校验 + 头部注入预览
...
- locale 解析链：`PINRULE_LOCALE` env > `config.yaml` `locale` 字段 > `pinrule.locale_detect.is_chinese_user()` 自动检测 > `en` fallback
...
- `pinrule init` 按 detected locale 选规则模板（中文用户装 `rules.dev.example.zh.yaml`，其他用英文 default）

**听话端 — 检测信号**（v0.8.0 → v0.8.2）：
- `pinrule/signals.py` 含 `load_phrases()`（`.txt` 平面字眼）+ `load_patterns()`（`.yaml` cartesian 模板）+ `compile_alternation()` union 编译（长字眼优先，`re.escape` 字面 vs raw regex 模板）
...
- **配置系统**（`pinrule/config.py` + `~/.claude/pinrule/config.yaml`）— 所有阈值集中可调（notify 开关 / rotation / purge / escalate）；fail open（文件缺失 / 字段为 null 用 DEFAULTS）
