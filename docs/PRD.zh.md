...
- 用户在 `~/.pinrule/rules.yaml` 定义 5-10 条（软上限 10，超过 12 拒绝加载）
...
- **配置系统**（`pinrule/config.py` + `~/.pinrule/config.yaml`）— 所有阈值集中可调（notify 开关 / rotation / purge / escalate）；fail open（文件缺失 / 字段为 null 用 DEFAULTS）
