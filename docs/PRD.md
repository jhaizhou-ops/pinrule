# pinrule Product Requirements Document

**[🇬🇧 English (current)](./PRD.md) · [🇨🇳 中文](./PRD.zh.md)**

## User pain points (empirical)

pinrule's design starts from a **long-term pain point**, in the user's own words:
...
| **pinrule v1** | Tried auto-distillation + retrieval — but the real pain point is "persistence" not "recall," directionally misaligned |

## pinrule design philosophy
...
pinrule doesn't learn new rules. The user manually lists "core directions," pinrule only ensures they're **always at the position of highest Agent attention**.
...
pinrule uses **"behavioral-violation detection"** as feedback:
...
pinrule does only **"core direction persistence + violation detection"** — one thing.
...
Per-turn injection: anchor only lists session-violated rules (median 1 rule ≈ 60 tokens; clean session = 0 anchor passthrough). **Real dogfood measure: ~2% of conversation context in typical session.**
...
- **Engine layer (M2+ reinforced)**: 8 violation_check functions doing precise regex detection for each rule
  ...
  - bypass_pinrule_detection: Bash commands with pinrule internal sensitive literals + write operations
...
- `pinrule stats` — Each rule's violation count + last trigger time
- `pinrule violations recent [N]` — Last N violation details
- `pinrule doctor` — Check environment (rule validity + all hook install status + skill install status, Claude 8 events)
- `pinrule audit` — Per-rule top trigger frequency + locale-agnostic grouping (v0.5.7+)
- `pinrule install-hooks / uninstall-hooks` — Auto-write/clean settings.json (idempotent + backup + preserve other hooks)
- `pinrule install-skill [--force]` — Install / upgrade the `pinrule-rule` Claude skill (`pinrule init` runs this automatically; standalone command for upgrades)
...
- Codex: `~/.agents/skills/pinrule/SKILL.md` (note: `~/.agents/`, not `~/.codex/` — shared namespace with Anthropic per OpenAI design)
- `pinrule init` auto-installs to all three; `pinrule install-skill [--force] [--backend <name>]` for upgrades; `pinrule doctor` reports per-backend skill status


### F6. Internationalization (v0.5.2+ injection text; v0.8.0+ detection signals) ✅

pinrule has **two-way i18n**: speaking-side (what pinrule injects into the Agent prompt) and listening-side (what regex phrases pinrule uses to detect signals in user / Agent dialogue).
...
- **Unified description-context exemption** (`pinrule/checks/description_context.py`) — `.md` / `.yaml` / `.json` / tests/ / `/tmp/` / probe-sample naming files: "description trigger patterns" don't count as execution intent
...
pinrule v0 doesn't pursue accuracy numbers — pursues **whether the author self-using actually feels "Agent makes fewer directional errors in long tasks"**.

Observation metrics:
1. **Violation trigger frequency in long tasks** — comparison before vs. after pinrule install
...
If after a week of self-use there's no obvious improvement → pinrule's hypothesis is wrong, needs redesign.
...
- **No LLM dependency, ever** — pinrule is firmly pure-engineering (regex / keywords / counting). Not just for v0 — this is a permanent boundary
