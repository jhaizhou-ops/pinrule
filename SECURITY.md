...
|---|---|---|
| **`rules.json` contains malicious regex** | User copies untrusted rule template containing ReDoS patterns that hang the hook | pinrule fails loud on `re.compile` errors during rule loading; users should only copy templates from trusted sources |
...
| **`violations.jsonl` contains sensitive strings** | Bash command containing secrets triggers pinrule detection → secret enters `~/.pinrule/violations.jsonl` snippet field | Don't put plaintext secrets in Bash commands; pinrule doesn't perform secret detection on disk (not its responsibility) |
| **`pre_compact_snapshot.md` contains sensitive content** | Pre-compact rule state dumped to `~/.pinrule/pre_compact_snapshot.md` containing your `rules.json` preference content | Don't put secrets in `rules.json` `preference` field; file is 600 permissions + home directory protected by default |
| **Cross-session state contamination** | Multiple session JSON files in `~/.pinrule/session-state/` containing read_files / edit_files / recent_bash summaries | Auto-cleaned after 30 days (configurable via `session_state_max_age_days`); doesn't sync cross-user / cross-machine |
...
- **Can't detect actual legitimacy of `rules.json` content** — if a user writes "encourage Agent to write insecure code" type preferences, pinrule will still inject as-is. **Users are responsible for `rules.json` content**
