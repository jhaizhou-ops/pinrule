...
| 场景 | 风险 | 缓解 |
|---|---|---|
| **`rules.yaml` 含恶意 regex** | 用户复制不可信来源的规则模板，含 ReDoS pattern 卡死 hook | pinrule 加载规则时 `re.compile` 失败会 fail loud；只从可信源复制模板 |
...
| **`violations.jsonl` 含敏感字符串** | 你的 Bash 命令含 secret 触发 pinrule 检测时，secret 进 `~/.claude/pinrule/violations.jsonl` snippet 字段 | 不要在 Bash 命令里明文 secret；pinrule 落盘前不做 secret 检测（不是它的职责） |
| **`pre_compact_snapshot.md` 含敏感内容** | compact 前规则状态落盘到 `~/.claude/pinrule/pre_compact_snapshot.md`，含你 `rules.yaml` 写的 preference | 不要在 `rules.yaml` 的 `preference` 字段写 secret；这文件 600 权限 + 家目录默认保护 |
| **跨 session 状态污染** | `~/.claude/pinrule/session-state/` 多个 session JSON 含 read_files / edit_files / recent_bash 摘要 | 30 天自动清理（`session_state_max_age_days` 可调）；不跨用户跨机器同步 |
...
- **检测不了 `rules.yaml` 内容本身的合法性** — 如果用户写了「鼓励 Agent 写不安全代码」类 preference，pinrule 仍照样注入。**`rules.yaml` 内容由用户负责**
